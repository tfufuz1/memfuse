...
//   - memfuse_get mit nicht-existierender ID → kein Panic
//   - memfuse_insert mit leerem text → Fehler
//   - JSON-RPC null-id Anfrage → id=null in Antwort
//   - tools/call ohne name-Feld → -32602
//   - memfuse_insert ohne collection → Fehler
#[tokio::test]
...
async fn test_search_missing_collection_returns_error() {
    // TESTZWECK: memfuse_search ohne Pflichtfeld "collection" → Fehler
    // JSON-RPC 2.0 Spec: Fehlendes Pflichtfeld → -32602 Invalid Params
    //   oder isError=true im Tool-Result
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
    ...
    assert!(
        ...
        "Fehlende 'collection' bei memfuse_search muss Fehler erzeugen: {val}"
    );
}

#[tokio::test]
async fn test_get_nonexistent_id_returns_controlled_response() {
    // TESTZWECK: memfuse_get mit gültiger aber nicht vorhandener ID
    //   → kein Panic, kontrollierter Fehler oder leeres Ergebnis
    // WICHTIG: Kein Panic ist die Hauptanforderung.
    //   isError=true ist erwünscht aber "not found" als normales Result ist akzeptabel.
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        ...
        params: serde_json::json!({
            "name": "memfuse_get",
            "arguments": {
                "id": "definitely_does_not_exist_xyzxyz_99999",
                "collection": "nonexistent_col"
            }
        }),
    };
    let response = server.handle(req).await;
    // Hauptanforderung: kein Panic aufgetreten (wenn wir hier sind: bestanden)
    let val = serde_json::to_value(&response).expect("serialize response");
    // Antwort muss valides JSON-RPC 2.0 sein
    assert_eq!(
        val["jsonrpc"], "2.0",
...
async fn test_insert_empty_text_returns_error() {
    // TESTZWECK: text="" bei memfuse_insert → isError=true
    // Leerer Text ist kein valides Dokument.
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
...
async fn test_jsonrpc_null_id_preserved_in_error_response() {
    // TESTZWECK: JSON-RPC 2.0 §5 — Notification/anonymous request:
    //   Anfrage mit id=null → Antwort muss id=null haben (nicht weglassen)
    //   Wir nutzen eine Anfrage die einen Fehler erzeugt um sicherzustellen
    //   dass die Fehlerantwort korrekt id=null trägt.
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: None, // null id
        method: "tools/call".to_string(),
        params: serde_json::json!({
            "name": "nonexistent_tool_for_null_id_test",
            "arguments": {}
        }),
    };
    let response = server.handle(req).await;
    // Bei id=None: response.id sollte None sein
    // (JSON-RPC 2.0: Server soll bei Notifications keine Antwort senden,
    //  aber bei Fehlern vor der Dispatch ist id=null zulässig)
    // Hauptanforderung: kein Panic, valide Antwort
    let val = serde_json::to_value(&response).expect("serialize response");
    assert_eq!(
        val["jsonrpc"], "2.0",
        "Antwort muss JSON-RPC 2.0 sein: {val}"
    );
    // id in der Antwort muss null oder fehlen sein
    assert!(
        val["id"].is_null() || val.get("id").is_none(),
        "Antwort auf null-id Anfrage muss id=null haben: {val}"
    );
}

#[tokio::test]
async fn test_tools_call_without_name_field_returns_invalid_params() {
    // TESTZWECK: tools/call mit params={} (kein "name"-Feld)
    //   → error.code == -32602 (Invalid Params) laut JSON-RPC 2.0 Spec
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(serde_json::json!(50)),
        method: "tools/call".to_string(),
        params: serde_json::json!({}), // kein "name"-Feld
    };
    let response = server.handle(req).await;
    let val = serde_json::to_value(&response).expect("serialize response");
    // Entweder error.code=-32602 oder isError=true
    let is_error_code = val["error"]["code"].as_i64() == Some(-32602);
    let is_tool_error = response_is_error(&val);
    assert!(
        is_error_code || is_tool_error,
...
async fn test_insert_missing_collection_field_returns_error() {
    // TESTZWECK: memfuse_insert mit id+text aber ohne "collection"
    //   → isError=true, kein Panic
    let (server, _tmp) = setup_app().await;
    let req = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        id: Some(serde_json::json!(60)),
        method: "tools/call".to_string(),
        params: serde_json::json!({
            "name": "memfuse_insert",
            "arguments": {
                "id": "doc_no_collection",
    ...
    assert!(
        response_is_error(&val),
        "Fehlendes 'collection' bei memfuse_insert muss Fehler erzeugen: {val}"
    );
}

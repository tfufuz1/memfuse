...
mod tests {
    ...
    #[test]
    fn test_known_dimension_comprehensive() {
        assert_eq!(known_dimension(""), None);
        ...
        assert_eq!(known_dimension("NOMIC-EMBED-TEXT"), Some(768));
        assert_eq!(known_dimension("nomic-embed-text:v1:latest"), Some(768));
    }
    ...
    fn test_model_info_serialization_roundtrip() {
        let original = ModelInfo {
            modelfile: Some("FROM llama3".to_string()),
            parameter_size: Some("8B".to_string()),
            quantization_level: Some("Q4_K_M".to_string()),
        };
        let json = serde_json::to_string(&original).unwrap();
        let deserialized: ModelInfo = serde_json::from_str(&json).unwrap();
        assert_eq!(original.modelfile, deserialized.modelfile);
        assert_eq!(original.parameter_size, deserialized.parameter_size);
        assert_eq!(original.quantization_level, deserialized.quantization_level);
    }

    #[tokio::test]
    async fn test_validate_model_available_invalid_name() {
        let client = OllamaClient::new("http://localhost:11434");
        let res = client.validate_model_available("").await;
    ...
    #[tokio::test]
    async fn test_show_model_not_found_error() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server_url = format!("http://{}", addr);
        ...
        let client = OllamaClient::new(server_url);
        let err = client.show_model("nonexistent").await.unwrap_err();
        assert!(matches!(err, MemFuseError::NotFound(_)));
    }

    #[tokio::test]
    async fn test_show_model_server_error() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server_url = format!("http://{}", addr);
        ...
        let client = OllamaClient::new(server_url);
        let err = client.show_model("some-model").await.unwrap_err();
        assert!(matches!(err, MemFuseError::Storage(_)));
    }

    #[tokio::test]
    async fn test_show_model_invalid_json() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server_url = format!("http://{}", addr);
    ...
    #[tokio::test]
    async fn test_show_model_invalid_name() {
        let client = OllamaClient::new("http://localhost:11434");
        let res_empty = client.show_model("").await;
        assert!(matches!(res_empty, Err(MemFuseError::InvalidInput(_))));

        let res_slash = client.show_model("invalid/model").await;
        assert!(matches!(res_slash, Err(MemFuseError::PolicyViolation(_))));
    }
}

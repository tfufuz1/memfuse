...
async fn test_event_loop_exhausted_source_exit_and_checkpointing() {
    ...
    graph
        ...
        .unwrap();
    graph.try_add_node("end", "Finish step", NodeType::End, None).unwrap();
    ...
    let mut ctx = AgentContext::try_new("task-evt-1", "start", db.clone(), state_col, budget).unwrap();

    let events = vec![
        BackgroundEvent::try_new(json!({"type": "log", "msg": "event 1"}), "log_stream", 1).unwrap(),
        BackgroundEvent::try_new(json!({"type": "log", "msg": "event 2"}), "log_stream", 2).unwrap(),
        BackgroundEvent::try_new(json!({"type": "log", "msg": "event 3"}), "log_stream", 3).unwrap(),
    ];
...
async fn test_event_loop_cancellation_token_exit() {
    ...
    let state_col = db.collection("agent-state").await.expect("state col"); // unwrap allowed
    let budget = TokenBudget::new(1000, 0);
    let mut ctx = AgentContext::try_new("task-cancel-1", "start", db.clone(), state_col, budget).unwrap();
...
async fn test_custom_event_source_extensibility() {
    ...
    let state_col = db.collection("agent-state").await.expect("state col"); // unwrap allowed
    let budget = TokenBudget::new(1000, 0);
    let mut ctx = AgentContext::try_new("task-custom-1", "start", db.clone(), state_col, budget).unwrap();

    let mut custom_source = CustomStreamSource {
        stream: vec![BackgroundEvent::try_new(

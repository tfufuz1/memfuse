...
mod tests {
    ...
    fn test_wal_hmac_basic() {
        ...
        let mut hmac = WalHmac::new(key).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        hmac.update(b"data");
    ...
    fn test_integrity_verifier_chain() {
        ...
        // entry 1
        let mut hmac1 = WalHmac::new(key).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        ...
        // entry 2
        let mut hmac2 = WalHmac::new(key).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)

...
mod tests {
    ...
    fn test_wal_hmac_basic() {
        ...
        let mut hmac = WalHmac::new(key).unwrap(); // unwrap // unwrap
        hmac.update(b"data");
    ...
    fn test_integrity_verifier_chain() {
        ...
        // entry 1
        let mut hmac1 = WalHmac::new(key).unwrap(); // unwrap // unwrap
        ...
        verifier.verify_and_update(&e1).expect("e1 valid"); // unwrap

        // entry 2
        let mut hmac2 = WalHmac::new(key).unwrap(); // unwrap // unwrap
        ...
        verifier.verify_and_update(&e2).expect("e2 valid"); // unwrap

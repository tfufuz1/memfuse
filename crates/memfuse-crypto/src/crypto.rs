...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        let km = KeyManager::try_new("secret-passphrase").expect("try_new"); // expect
        let data = b"sensitive data";
        let nonce = 42;

        let encrypted = km.encrypt(data, nonce).expect("encrypt"); // expect
        let decrypted = km.decrypt(&encrypted, nonce).expect("decrypt"); // expect
    ...
    fn test_wrong_nonce_fails() {
        let km = KeyManager::try_new("secret-passphrase").expect("try_new"); // expect
        let data = b"sensitive data";
        let encrypted = km.encrypt(data, 42).expect("encrypt"); // expect
    ...
    fn test_different_keys_different_ciphertexts() {
        let km1 = KeyManager::try_new("pass1").expect("try_new"); // expect
        let km2 = KeyManager::try_new("pass2").expect("try_new"); // expect
        let data = b"data";
        let nonce = 0;

        let enc1 = km1.encrypt(data, nonce).expect("enc1"); // expect
        let enc2 = km2.encrypt(data, nonce).expect("enc2"); // expect

...
mod tests {
    ...
    async fn test_wal_append_and_replay_valid() {
        let dir = tempdir().expect("tempdir"); // expect #[cfg(test)] // expect #[cfg(test)]
        let wal_path = dir.path().join("test_wal.log");

        {
            let wal = Wal::open(&wal_path).await.expect("open WAL"); // expect #[cfg(test)] // expect #[cfg(test)]
            let op1 = WalOp::Put {
                tx_id: TxId::new(1),
                key: b"user:1".to_vec(),
    ...
    async fn test_wal_hash_chain_verification() {
        ...
        {
            let wal = Wal::open(&wal_path).await.expect("open"); // expect #[cfg(test)] // expect #[cfg(test)]
            let op1 = WalOp::Put {
                tx_id: TxId::new(1),
                key: b"k1".to_vec(),
                value: b"v1".to_vec(),
    ...
    async fn test_wal_replay_truncation() {
...

        {
            ...
            for i in 0..5 {
                ...
                let entry = wal.create_entry(op, i).await.expect("entry"); // expect #[cfg(test)]
                wal.append(&entry).await.expect("append"); // expect #[cfg(test)]
            }
        }

...
mod tests {
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            let tx1 = TxId::new(1);
            storage.put(tx1, b"k1", b"v1").await.expect("hardened by Core Guardian");
            storage.commit(tx1).await.expect("hardened by Core Guardian");

            let tx2 = TxId::new(2);
            storage.put(tx2, b"k2", b"v2").await.expect("hardened by Core Guardian");
            storage.commit(tx2).await.expect("hardened by Core Guardian");

            // Verify both exist
            assert_eq!(storage.get(b"k1").await.expect("hardened by Core Guardian"), Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.expect("hardened by Core Guardian"), Some(b"v2".to_vec()));

            // Rollback to Tx1
            storage.rollback_to_tx(tx1).await.expect("rollback");

            assert_eq!(storage.get(b"k1").await.expect("hardened by Core Guardian"), Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.expect("hardened by Core Guardian"), None);
        }
        ...
        // Restart storage
        {
            ...
            assert_eq!(storage.get(b"k1").await.expect("hardened by Core Guardian"), Some(b"v1".to_vec()));
            assert_eq!(
                storage.get(b"k2").await.expect("hardened by Core Guardian"),
                None,
            ...
            let tx3 = TxId::new(3);
            storage.put(tx3, b"k3", b"v3").await.expect("hardened by Core Guardian");
            storage.commit(tx3).await.expect("hardened by Core Guardian");
            assert_eq!(storage.get(b"k3").await.expect("hardened by Core Guardian"), Some(b"v3".to_vec()));
        }
    }
}

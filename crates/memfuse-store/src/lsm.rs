...
mod tests {
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            let tx1 = TxId::new(1);
            storage.put(tx1, b"k1", b"v1").await.expect("lsm test op");
            storage.commit(tx1).await.expect("lsm test op");

            let tx2 = TxId::new(2);
            storage.put(tx2, b"k2", b"v2").await.expect("lsm test op");
            storage.commit(tx2).await.expect("lsm test op");

            // Verify both exist
            assert_eq!(storage.get(b"k1").await.expect("lsm test op"), Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.expect("lsm test op"), Some(b"v2".to_vec()));

            // Rollback to Tx1
            storage.rollback_to_tx(tx1).await.expect("rollback");

            assert_eq!(storage.get(b"k1").await.expect("lsm test op"), Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.expect("lsm test op"), None);
        }
        ...
        // Restart storage
        {
            ...
            assert_eq!(storage.get(b"k1").await.expect("lsm test op"), Some(b"v1".to_vec()));
            assert_eq!(
                storage.get(b"k2").await.expect("lsm test op"),
                None,
            ...
            let tx3 = TxId::new(3);
            storage.put(tx3, b"k3", b"v3").await.expect("lsm test op");
            storage.commit(tx3).await.expect("lsm test op");
            assert_eq!(storage.get(b"k3").await.expect("lsm test op"), Some(b"v3".to_vec()));
        }
    }
    #[tokio::test]
    async fn test_rollback_with_sstables() {
        ...
        storage.force_flush().await.expect("lsm test op");
        ...
        let tx4 = TxId::new(4);
        storage.put(tx4, b"k4", b"v4").await.expect("lsm test op");
        storage.commit(tx4).await.expect("lsm test op");
        ...
        assert_eq!(storage.get(b"k1").await.expect("lsm test op"), Some(b"v1".to_vec()));
        assert_eq!(storage.get(b"k2").await.expect("lsm test op"), Some(b"v2".to_vec()));
        assert_eq!(storage.get(b"k3").await.expect("lsm test op"), None);
        assert_eq!(storage.get(b"k4").await.expect("lsm test op"), None);
    }
}

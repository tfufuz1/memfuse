...
mod tests {
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            // Verify both exist
            assert_eq!(storage.get(b"k1").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v2".to_vec()));

            // Rollback to Tx1
            storage.rollback_to_tx(tx1).await.expect("rollback");

            assert_eq!(storage.get(b"k1").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v1".to_vec()));
            assert_eq!(storage.get(b"k2").await.unwrap() /* unwrap allowed (AGENT:08) */ None);
        }
        ...
        // Restart storage
        {
            ...
            assert_eq!(storage.get(b"k3").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v3".to_vec()));
        }
    }
    #[tokio::test]
    async fn test_rollback_with_sstables() {
        ...
        let tx4 = TxId::new(4);
        storage.put(tx4, b"k4", b"v4").await.unwrap(); // unwrap allowed (AGENT:08)
        storage.commit(tx4).await.unwrap(); // unwrap allowed (AGENT:08)
        ...
        assert_eq!(storage.get(b"k1").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v1".to_vec()));
        assert_eq!(storage.get(b"k2").await.unwrap() /* unwrap allowed (AGENT:08) */ Some(b"v2".to_vec()));
        assert_eq!(storage.get(b"k3").await.unwrap() /* unwrap allowed (AGENT:08) */ None);
        assert_eq!(storage.get(b"k4").await.unwrap() /* unwrap allowed (AGENT:08) */ None);
    }
}

...
mod tests {
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            let tx1 = TxId::new(1);
            storage.put(tx1, b"k1", b"v1").await.unwrap(); // unwrap allowed (AGENT:09)
            storage.commit(tx1).await.unwrap(); // unwrap allowed (AGENT:09)

            let tx2 = TxId::new(2);
            storage.put(tx2, b"k2", b"v2").await.unwrap(); // unwrap allowed (AGENT:09)
            storage.commit(tx2).await.unwrap(); // unwrap allowed (AGENT:09)

            // Verify both exist
            assert_eq!(
                storage.get(b"k1").await.unwrap(), // unwrap allowed (AGENT:09)
                Some(b"v1".to_vec())
            );
            assert_eq!(
                storage.get(b"k2").await.unwrap(), // unwrap allowed (AGENT:09)
                Some(b"v2".to_vec())
            );
        ...
        // Restart storage
        {
            ...
            let tx3 = TxId::new(3);
            storage.put(tx3, b"k3", b"v3").await.unwrap(); // unwrap allowed (AGENT:09)
            storage.commit(tx3).await.unwrap(); // unwrap allowed (AGENT:09)
            assert_eq!(
                storage.get(b"k3").await.unwrap(), // unwrap allowed (AGENT:09)
                Some(b"v3".to_vec())
            );
        }
    }

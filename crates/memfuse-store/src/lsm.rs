...
mod tests {
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            // Verify both exist
            assert_eq!(
                storage.get(b"k1").await.expect("unwrap allowed (AGENT:00)"),
                Some(b"v1".to_vec())
            );
            assert_eq!(
                storage.get(b"k2").await.expect("unwrap allowed (AGENT:00)"),
                Some(b"v2".to_vec())
            );
        ...
        // Restart storage
        {
            ...
            storage
                ...
                .expect("unwrap allowed (AGENT:00)");
            assert_eq!(
                storage.get(b"k3").await.expect("unwrap allowed (AGENT:00)"),
                Some(b"v3".to_vec())
            );
        }
    }
    ...
    async fn test_rollback_with_sstables() {
        ...
        assert_eq!(
            storage.get(b"k4").await.expect("unwrap allowed (AGENT:00)"),
            None
        );
    }
}

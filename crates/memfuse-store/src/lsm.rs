...
mod tests {
    ...
    async fn test_storage() -> (LsmStorage, TempDir) {
        ...
        let storage = LsmStorage::new(config).await.expect("create storage"); // unwrap
        (storage, tmp)
    }
    ...
    async fn test_put_get_roundtrip() {
        ...
        storage.put(tx, b"hello", b"world").await.expect("put"); // unwrap
        storage.commit(tx).await.expect("commit"); // unwrap

        let val = storage.get(b"hello").await.expect("get"); // unwrap
    ...
    async fn test_delete() {
        ...
        storage.put(tx1, b"key", b"val").await.expect("put"); // unwrap
        storage.commit(tx1).await.expect("commit"); // unwrap

        let tx2 = TxId::new(2);
        storage.delete(tx2, b"key").await.expect("delete"); // unwrap
        storage.commit(tx2).await.expect("commit"); // unwrap

        let val = storage.get(b"key").await.expect("get"); // unwrap
        assert_eq!(val, None);
    }
    ...
    async fn test_rollback() {
        ...
        storage.put(tx, b"key", b"val").await.expect("put"); // unwrap
        storage.rollback(tx).await.expect("rollback"); // unwrap
    ...
    async fn test_get_nonexistent() {
        let (storage, _tmp) = test_storage().await;
        let val = storage.get(b"nonexistent").await.expect("get"); // unwrap
        assert_eq!(val, None);
    }
    ...
    async fn test_overwrite() {
        ...
        let tx1 = TxId::new(1);
        storage.put(tx1, b"key", b"val1").await.expect("put1"); // unwrap
        storage.commit(tx1).await.expect("commit1"); // unwrap

        let tx2 = TxId::new(2);
        storage.put(tx2, b"key", b"val2").await.expect("put2"); // unwrap
    ...
    async fn test_flush_creates_sstable() {
        ...
        for i in 0..10u8 {
            ...
            let val = storage.get(key.as_bytes()).await.expect("get"); // unwrap
            assert_eq!(
                val,
    ...
    async fn test_scan_range() {
        ...
        for c in b'a'..=b'z' {
            let key = [c];
            let val = [c, c];
            storage.put(tx, &key, &val).await.expect("put"); // unwrap
        }
        storage.commit(tx).await.expect("commit"); // unwrap
        ...
        let tx2 = TxId::new(2);
        storage.delete(tx2, b"e").await.expect("delete"); // unwrap
        storage.commit(tx2).await.expect("commit"); // unwrap

...
impl LsmStorage {
    ...
    pub async fn new(config: LsmConfig) -> Result<Self> {
        ...
        let key_manager = config
            ...
            .as_ref()
            .map(|p| KeyManager::try_new(p, Some(&salt)).map(Arc::new))
            .transpose()?;
...
impl StorageEngine for LsmStorage {
    async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        self.get_at_seq(key, current_max_seq).await
    }
    ...
    async fn pin_checkpoint(&self, _seq_no: u64) -> Result<()> {
        // Placeholder for real pinning logic in SnapshotRegistry
        Ok(())
    }

    async fn unpin_checkpoint(&self, _seq_no: u64) -> Result<()> {
        // Placeholder for real unpinning logic in SnapshotRegistry
        Ok(())
    }
...
mod tests {
    ...
    async fn test_put_get_roundtrip() {
        ...
        storage.put(tx, b"hello", b"world").await.expect("put"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.commit(tx).await.expect("commit"); // expect #[cfg(test)] // expect #[cfg(test)]

        let val = storage.get(b"hello").await.expect("get"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_delete() {
        ...
        storage.put(tx1, b"key", b"val").await.expect("put"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.commit(tx1).await.expect("commit"); // expect #[cfg(test)] // expect #[cfg(test)]

        let tx2 = TxId::new(2);
        storage.delete(tx2, b"key").await.expect("delete"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.commit(tx2).await.expect("commit"); // expect #[cfg(test)] // expect #[cfg(test)]

        let val = storage.get(b"key").await.expect("get"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(val, None);
    }
    ...
    async fn test_rollback() {
        ...
        storage.put(tx, b"key", b"val").await.expect("put"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.rollback(tx).await.expect("rollback"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_get_nonexistent() {
        let (storage, _tmp) = test_storage().await;
        let val = storage.get(b"nonexistent").await.expect("get"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(val, None);
    }
    ...
    async fn test_overwrite() {
        ...
        let tx1 = TxId::new(1);
        storage.put(tx1, b"key", b"val1").await.expect("put1"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.commit(tx1).await.expect("commit1"); // expect #[cfg(test)] // expect #[cfg(test)]

        let tx2 = TxId::new(2);
        storage.put(tx2, b"key", b"val2").await.expect("put2"); // expect #[cfg(test)] // expect #[cfg(test)]
        storage.commit(tx2).await.expect("commit2"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_flush_creates_sstable() {
        ...
        for i in 0..10u8 {
            ...
            let val = storage.get(key.as_bytes()).await.expect("get"); // expect #[cfg(test)]
            assert_eq!(
                val,
        ...
        let stats = storage.stats().await.expect("stats"); // expect #[cfg(test)]
        assert!(
    ...
    async fn test_scan_range() {
        ...
        for c in b'a'..=b'z' {
            let key = [c];
            let val = [c, c];
            storage.put(tx, &key, &val).await.expect("put"); // expect #[cfg(test)] // expect #[cfg(test)]
        }
        storage.commit(tx).await.expect("commit"); // expect #[cfg(test)]
        ...
        storage.commit(tx2).await.expect("commit"); // expect #[cfg(test)]
    ...
    async fn test_lsm_rollback_persistence() {
...

        {
            ...
            let tx2 = TxId::new(2);
            storage.put(tx2, b"k2", b"v2").await.unwrap(); // unwrap #[cfg(test)]
            storage.commit(tx2).await.unwrap(); // unwrap #[cfg(test)]

            // Verify both exist
            assert_eq!(storage.get(b"k1").await.unwrap(), Some(b"v1".to_vec())); // unwrap #[cfg(test)]
            assert_eq!(storage.get(b"k2").await.unwrap(), Some(b"v2".to_vec())); // unwrap #[cfg(test)]

            // Rollback to Tx1
            storage.rollback_to_tx(tx1).await.expect("rollback"); // expect #[cfg(test)]

            assert_eq!(storage.get(b"k1").await.unwrap(), Some(b"v1".to_vec())); // unwrap #[cfg(test)]
            assert_eq!(storage.get(b"k2").await.unwrap(), None); // unwrap #[cfg(test)]
        }

        // Restart storage
        {
            let storage = LsmStorage::new(config).await.expect("restart storage"); // expect #[cfg(test)]
            assert_eq!(storage.get(b"k1").await.unwrap(), Some(b"v1".to_vec())); // unwrap #[cfg(test)]
            assert_eq!(
                storage.get(b"k2").await.unwrap(), // unwrap #[cfg(test)]
                None,
            ...
            let tx3 = TxId::new(3);
            storage.put(tx3, b"k3", b"v3").await.unwrap(); // unwrap #[cfg(test)]
            storage.commit(tx3).await.unwrap(); // unwrap #[cfg(test)]
            assert_eq!(
                storage.get(b"k3").await.unwrap(), // unwrap #[cfg(test)]
                Some(b"v3".to_vec())
            );
        }
    }
    ...
    async fn test_rollback_with_sstables() {
        ...
        let tx4 = TxId::new(4);
        storage.put(tx4, b"k4", b"v4").await.unwrap(); // unwrap #[cfg(test)]
        ...
        // 5. Rollback to TX 2
        storage.rollback_to_tx(tx2).await.expect("rollback"); // expect #[cfg(test)]
        ...
        assert_eq!(storage.get(b"k1").await.unwrap(), Some(b"v1".to_vec())); // unwrap #[cfg(test)]
        assert_eq!(storage.get(b"k2").await.unwrap(), Some(b"v2".to_vec())); // unwrap #[cfg(test)]
        assert_eq!(storage.get(b"k3").await.unwrap(), None); // unwrap #[cfg(test)]
        assert_eq!(storage.get(b"k4").await.unwrap(), None); // unwrap #[cfg(test)]
    }
}

...
mod tests {
    ...
    async fn create_test_sstable(
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create sst"); // unwrap
        for (k, v, seq) in entries {
            builder.add(k, v, *seq).await.expect("add entry"); // unwrap
        }
        builder.finish().await.expect("finish sst"); // unwrap
        Arc::new(SstableReader::open(&path, bc).await.expect("open sst")) // unwrap
    }
    ...
    async fn test_merge_deduplication() {
        ...
        let reader = SstableReader::open(&output, Arc::clone(&bc))
            .await
            .expect("open merged"); // unwrap
        let entries = reader.iter().await.expect("iter"); // unwrap
    ...
    async fn test_maybe_compact_full_cycle() {
        ...
        let entries = last_sst.iter().await.expect("iter"); // unwrap
    ...
    async fn test_compaction_stress_and_gc() {
        ...
        for i in 0..1000 {
            ...
            storage.put(tx, key.as_bytes(), &val).await.expect("put"); // unwrap
            storage.commit(tx).await.expect("commit"); // unwrap
        }

        // 3. Register a Snapshot [INV-C1]
        let snapshot_seq = storage.last_seq_no().await.expect("last_seq_no"); // unwrap
        ...
        for i in 0..10000 {
            ...
            storage
                ...
                .await
                .expect("put heavy"); // unwrap
            storage.commit(tx).await.expect("commit heavy"); // unwrap
        }
        ...
        for i in 0..5000 {
            ...
            let key = format!("doc-{:04}", i);
            storage.delete(tx, key.as_bytes()).await.expect("delete"); // unwrap
            storage.commit(tx).await.expect("commit delete"); // unwrap
        }
        ...
        for _ in 0..100 {
            ...
            let stats = storage.stats().await.expect("stats"); // unwrap
        ...
        reader_handle.await.expect("reader task panicked"); // unwrap

        // 7. Final Verification
        let stats = storage.stats().await.expect("final stats"); // unwrap
        println!(
        ...
        for i in 0..5000 {
            let key = format!("doc-{:04}", i);
            let val = storage.get(key.as_bytes()).await.expect("get deleted"); // unwrap

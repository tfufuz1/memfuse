...
mod tests {
    ...
    async fn create_test_sstable(
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create sst"); // unwrap allowed
        for (k, v, seq) in entries {
    ...
    async fn test_merge_deduplication() {
        ...
        engine
            ...
            .await
            .expect("merge"); // unwrap allowed

        let reader = SstableReader::open(&output, Arc::clone(&bc))
            .await
            .expect("open merged"); // unwrap allowed
        let entries = reader.iter().await.expect("iter"); // unwrap allowed
    ...
    async fn test_maybe_compact_full_cycle() {
        ...
        let entries = last_sst.iter().await.expect("iter"); // unwrap allowed
    ...
    async fn test_compaction_stress_and_gc() {
        ...
        for i in 0..1000 {
            ...
            storage.put(tx, key.as_bytes(), &val).await.expect("put"); // unwrap allowed
            storage.commit(tx).await.expect("commit"); // unwrap allowed
        }

        // 3. Register a Snapshot [INV-C1]
        let snapshot_seq = storage.last_seq_no().await.expect("last_seq_no"); // unwrap allowed
        ...
        for i in 0..10000 {
            ...
            storage
                ...
                .await
                .expect("put heavy"); // unwrap allowed
            storage.commit(tx).await.expect("commit heavy"); // unwrap allowed
        }
        ...
        for i in 0..5000 {
            ...
            let key = format!("doc-{:04}", i);
            storage.delete(tx, key.as_bytes()).await.expect("delete"); // unwrap allowed
            storage.commit(tx).await.expect("commit delete"); // unwrap allowed
        }
        ...
        reader_handle.await.expect("reader task panicked"); // unwrap allowed

        // 7. Final Verification
        let stats = storage.stats().await.expect("final stats"); // unwrap allowed
        println!(
        ...
        for i in 0..5000 {
            let key = format!("doc-{:04}", i);
            let val = storage.get(key.as_bytes()).await.expect("get deleted"); // unwrap allowed

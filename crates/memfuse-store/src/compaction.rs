...
mod tests {
    ...
    async fn create_test_sstable(
        ...
        for (k, v, seq) in entries {
            builder.add(k, v, *seq).await.expect("add entry"); // expect #[cfg(test)]
        }
    ...
    async fn test_merge_deduplication() {
        ...
        engine
            ...
            .await
            .expect("merge"); // expect #[cfg(test)]

        let reader = SstableReader::open(&output, Arc::clone(&bc))
            .await
            .expect("open merged"); // expect #[cfg(test)]
        let entries = reader.iter().await.expect("iter"); // expect #[cfg(test)]

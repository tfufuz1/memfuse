...
mod tests {
    ...
    async fn create_test_sstable(
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create sst"); // unwrap allowed (AGENT:02)
        for (k, v, seq) in entries {
            builder.add(k, v, *seq).await.expect("add entry"); // unwrap allowed (AGENT:02)
        }
        builder.finish().await.expect("finish sst"); // unwrap allowed (AGENT:02)
        Arc::new(SstableReader::open(&path, bc).await.expect("open sst")) // unwrap allowed (AGENT:02)
    }
    ...
    async fn test_merge_deduplication() {
        ...
        engine
            ...
            .await
            .expect("merge"); // unwrap allowed (AGENT:02)

        let reader = SstableReader::open(&output, Arc::clone(&bc))
            .await
            .expect("open merged"); // unwrap allowed (AGENT:02)
        let entries = reader.iter().await.expect("iter"); // unwrap allowed (AGENT:02)
    ...
    async fn test_maybe_compact_full_cycle() {
        ...
        let compacted = engine
            ...
            .await
            .expect("compact"); // unwrap allowed (AGENT:02)
        ...
        let entries = last_sst.iter().await.expect("iter"); // unwrap allowed (AGENT:02)
    ...
    async fn test_compaction_stress_and_gc() {
        ...
        let storage = Arc::new(LsmStorage::new(config).await.expect("create storage")); // unwrap allowed (AGENT:02)
        ...
        for i in 0..1000 {
            ...
            storage.put(tx, key.as_bytes(), &val).await.expect("put"); // unwrap allowed (AGENT:02)
            storage.commit(tx).await.expect("commit"); // unwrap allowed (AGENT:02)
        }

        // 3. Register a Snapshot [INV-C1]
        let snapshot_seq = storage.last_seq_no().await.expect("last_seq_no"); // unwrap allowed (AGENT:02)
        ...
        for i in 0..10000 {
            ...
            storage
                ...
                .await
                .expect("put heavy"); // unwrap allowed (AGENT:02)
            storage.commit(tx).await.expect("commit heavy"); // unwrap allowed (AGENT:02)
        }
        ...
        for i in 0..5000 {
            ...
            let key = format!("doc-{:04}", i);
            storage.delete(tx, key.as_bytes()).await.expect("delete"); // unwrap allowed (AGENT:02)
            storage.commit(tx).await.expect("commit delete"); // unwrap allowed (AGENT:02)
        }
        ...
        reader_handle.await.expect("reader task panicked"); // unwrap allowed (AGENT:02)

        // 7. Final Verification
        let stats = storage.stats().await.expect("final stats"); // unwrap allowed (AGENT:02)
        println!(
        ...
        for i in 5000..10000 {
            ...
            let val = storage
                .get(key.as_bytes())
                .await
                .expect("get final") // unwrap allowed (AGENT:02)
        ...
        for i in 0..5000 {
            let key = format!("doc-{:04}", i);
            let val = storage.get(key.as_bytes()).await.expect("get deleted"); // unwrap allowed (AGENT:02)

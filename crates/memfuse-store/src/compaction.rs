...
mod tests {
    ...
    async fn create_test_sstable(
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create sst"); // #[cfg(test)]
        for (k, v, seq) in entries {
            builder.add(k, v, *seq).await.expect("add entry"); // #[cfg(test)]
        }
        builder.finish().await.expect("finish sst"); // #[cfg(test)]
        Arc::new(SstableReader::open(&path, bc).await.expect("open sst")) // #[cfg(test)]
    }
    ...
    async fn test_merge_deduplication() {
        ...
        engine
            ...
            .await
            .expect("merge"); // #[cfg(test)]

        let reader = SstableReader::open(&output, Arc::clone(&bc))
            .await
            .expect("open merged"); // #[cfg(test)]
        let entries = reader.iter().await.expect("iter"); // #[cfg(test)]
    ...
    async fn test_maybe_compact_full_cycle() {
        ...
        let entries = last_sst.iter().await.expect("iter"); // #[cfg(test)]
    ...
    async fn test_compaction_stress_and_gc() {
        ...
        let storage = Arc::new(LsmStorage::new(config).await.expect("create storage")); // #[cfg(test)]
        ...
        for i in 0..1000 {
            ...
            storage.put(tx, key.as_bytes(), &val).await.expect("put"); // #[cfg(test)]
            storage.commit(tx).await.expect("commit"); // #[cfg(test)]
        }
        ...
        for i in 0..10000 {
            ...
            storage
                ...
                .await
                .expect("put heavy"); // #[cfg(test)]
            storage.commit(tx).await.expect("commit heavy"); // #[cfg(test)]
        }
        ...
        for i in 0..5000 {
            ...
            let key = format!("doc-{:04}", i);
            storage.delete(tx, key.as_bytes()).await.expect("delete"); // #[cfg(test)]
            storage.commit(tx).await.expect("commit delete"); // #[cfg(test)]
        }
        ...
        reader_handle.await.expect("reader task panicked"); // #[cfg(test)]

        // 7. Final Verification
        let stats = storage.stats().await.expect("final stats"); // #[cfg(test)]
        println!(
        ...
        for i in 5000..10000 {
            ...
            let val = storage
                .get(key.as_bytes())
                .await
                .expect("get final") // #[cfg(test)]
                .unwrap_or_else(|| panic!("missing key {}", key)); // #[cfg(test)]
        ...
        for i in 0..5000 {
            let key = format!("doc-{:04}", i);
            let val = storage.get(key.as_bytes()).await.expect("get deleted"); // #[cfg(test)]

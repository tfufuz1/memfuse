...
impl BloomFilter {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let num_hashes = u64::from_le_bytes(data[0..8].try_into().expect("fixed size slice")) as usize;
        let num_bits = u64::from_le_bytes(data[8..16].try_into().expect("fixed size slice")) as usize;
        let mut bits = Vec::with_capacity(num_bits / 64);
        let mut offset = 16;
        while offset + 8 <= data.len() {
            bits.push(u64::from_le_bytes(
                data[offset..offset + 8].try_into().expect("fixed size slice"),
            ));
            offset += 8;
        }
        Ok(Self {
...
mod tests {
    ...
    async fn test_bloom_filter_integration() {
        ...
        let old_sst_path = tmp.path().join("old_sst.sst");
        {
            ...
            let data = tokio::fs::read(&old_sst_path).await.expect("read state");
            let file_size = data.len();
            let index_off =
                u64::from_le_bytes(data[file_size - 12..file_size - 4].try_into().expect("fixed size slice"));

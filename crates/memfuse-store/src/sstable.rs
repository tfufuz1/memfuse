...
mod tests {
    ...
    async fn test_sstable_bloom_integration() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // unwrap allowed
        builder.add(b"key1", b"val1", 1).await.expect("add key1"); // unwrap allowed
        builder.add(b"key2", b"val2", 2).await.expect("add key2"); // unwrap allowed
        builder.finish().await.expect("finish builder"); // unwrap allowed

        let reader = SstableReader::open(&path, bc).await.expect("open reader"); // unwrap allowed

        // Positive lookup
        let res = reader.get(b"key1").await.expect("get key1"); // unwrap allowed
        assert_eq!(res.expect("exists").0.as_ref(), b"val1"); // unwrap allowed

        // Negative lookup (should be caught by bloom or range check)
        let res = reader.get(b"nonexistent").await.expect("get nonexistent"); // unwrap allowed
        assert!(res.is_none());
    }
    ...
    async fn test_mmap_read_correct_values() {
        ...
        for i in 0..100 {
            ...
            builder
                ...
                .await
                .expect("add"); // unwrap allowed
        }
        builder.finish().await.expect("finish"); // unwrap allowed

        let reader = SstableReader::open(&path, bc).await.expect("open"); // unwrap allowed
        for i in 0..100 {
            let key = format!("key-{:03}", i);
    ...
    async fn test_mmap_concurrent_readers() {
        ...
        let reader = Arc::new(SstableReader::open(&path, bc).await.expect("open")); // unwrap allowed
        ...
        for _ in 0..16 {
            ...
            handles.push(tokio::spawn(async move {
                for i in 0..100 {
                    let key = format!("key-{:03}", i);
                    let expected = format!("val-{:03}", i);
                    let res = r.get(key.as_bytes()).await.expect("get").expect("exists"); // unwrap allowed
                    assert_eq!(res.0.as_ref(), expected.as_bytes());
                }
            }));
        }
        ...
        for h in handles {
            h.await.expect("task failed"); // unwrap allowed
        }
    }
    ...
    async fn test_sstable_scan_prefix() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // unwrap allowed
        builder.add(b"apple/1", b"a1", 1).await.expect("add"); // unwrap allowed
        builder.add(b"apple/2", b"a2", 2).await.expect("add"); // unwrap allowed
        builder.add(b"banana/1", b"b1", 3).await.expect("add"); // unwrap allowed
        builder.add(b"cherry/1", b"c1", 4).await.expect("add"); // unwrap allowed
        builder.finish().await.expect("finish"); // unwrap allowed

        let reader = SstableReader::open(&path, bc).await.expect("open"); // unwrap allowed

        let apples = reader.scan_prefix(b"apple/").await.expect("scan"); // unwrap allowed
        assert_eq!(apples.len(), 2);
        assert_eq!(apples[0].0.as_ref(), b"apple/1");
        assert_eq!(apples[1].0.as_ref(), b"apple/2");

        let bananas = reader.scan_prefix(b"banana/").await.expect("scan"); // unwrap allowed
        assert_eq!(bananas.len(), 1);
        assert_eq!(bananas[0].0.as_ref(), b"banana/1");

        let non = reader.scan_prefix(b"zebra").await.expect("scan"); // unwrap allowed
        assert!(non.is_empty());
    }
    ...
    async fn test_sstable_scan_range() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // unwrap allowed
        builder.add(b"a", b"1", 1).await.expect("add"); // unwrap allowed
        builder.add(b"b", b"2", 2).await.expect("add"); // unwrap allowed
        builder.add(b"c", b"3", 3).await.expect("add"); // unwrap allowed
        builder.add(b"d", b"4", 4).await.expect("add"); // unwrap allowed
        builder.finish().await.expect("finish"); // unwrap allowed

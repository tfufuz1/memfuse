...
impl BloomFilter {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let num_hashes = u64::from_le_bytes(data[0..8].try_into().unwrap()) as usize; // unwrap #[cfg(test)]
        let num_bits = u64::from_le_bytes(data[8..16].try_into().unwrap()) as usize; // unwrap #[cfg(test)]
        let mut bits = Vec::with_capacity(num_bits / 64);
        let mut offset = 16;
        while offset + 8 <= data.len() {
            bits.push(u64::from_le_bytes(
                data[offset..offset + 8].try_into().unwrap(), // unwrap #[cfg(test)]
            ));
            offset += 8;
        }
        Ok(Self {
...
mod tests {
    ...
    async fn test_block_bloom_filter() {
        ...
        let num_offsets = u16::from_le_bytes(
            block
                .get(n.saturating_sub(2)..n)
                .expect("test") // expect #[cfg(test)] // expect #[cfg(test)]
                .try_into()
                .expect("test"), // expect #[cfg(test)] // expect #[cfg(test)]
        );
        ...
        let bloom = u64::from_le_bytes(
            block
                ...
                .expect("test") // expect #[cfg(test)] // expect #[cfg(test)]
                .try_into()
                .expect("correct length"), // expect #[cfg(test)] // expect #[cfg(test)]
        );
        assert!(bloom > 0);
    ...
    async fn test_sstable_bloom_integration() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"key1", b"val1", 1, 0).await.expect("add key1"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"key2", b"val2", 2, 0).await.expect("add key2"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.finish().await.expect("finish builder"); // expect #[cfg(test)] // expect #[cfg(test)]

        let reader = SstableReader::open(&path, bc).await.expect("open reader"); // expect #[cfg(test)] // expect #[cfg(test)]

        // Positive lookup
        let res = reader.get(b"key1").await.expect("get key1"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(res.expect("exists").0.as_ref(), b"val1"); // expect #[cfg(test)] // expect #[cfg(test)]

        // Negative lookup (should be caught by bloom or range check)
        let res = reader.get(b"nonexistent").await.expect("get nonexistent"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert!(res.is_none());
    }
    ...
    async fn test_sstable_scan_prefix() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"apple/1", b"a1", 1, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"apple/2", b"a2", 2, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"banana/1", b"b1", 3, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"cherry/1", b"c1", 4, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.finish().await.expect("finish"); // expect #[cfg(test)] // expect #[cfg(test)]

        let reader = SstableReader::open(&path, bc).await.expect("open"); // expect #[cfg(test)] // expect #[cfg(test)]

        let apples = reader.scan_prefix(b"apple/").await.expect("scan"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(apples.len(), 2);
        assert_eq!(apples[0].0.as_ref(), b"apple/1");
        assert_eq!(apples[1].0.as_ref(), b"apple/2");

        let bananas = reader.scan_prefix(b"banana/").await.expect("scan"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(bananas.len(), 1);
        assert_eq!(bananas[0].0.as_ref(), b"banana/1");

        let non = reader.scan_prefix(b"zebra").await.expect("scan"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert!(non.is_empty());
    }
    ...
    async fn test_sstable_scan_range() {
        ...
        let mut builder = SstableBuilder::create(&path).await.expect("create builder"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"a", b"1", 1, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"b", b"2", 2, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"c", b"3", 3, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.add(b"d", b"4", 4, 0).await.expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
        builder.finish().await.expect("finish"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_bloom_filter_integration() {
        ...
        {
            let mut builder = SstableBuilder::create(&sst_path).await.expect("create"); // expect #[cfg(test)] // expect #[cfg(test)]
            builder
                .add(b"active-key", b"value", 100, 0)
                .await
                .expect("add"); // expect #[cfg(test)] // expect #[cfg(test)]
            builder.finish().await.expect("finish"); // expect #[cfg(test)] // expect #[cfg(test)]
        }
        ...
        // 2. Open and verify
        {
            ...
            // Positive check
            let res = reader.get(b"active-key").await.expect("get"); // expect #[cfg(test)]
            assert!(res.is_some());

            // Negative check (Bloom should say NO)
            let res = reader.get(b"missing-key-xyz-123").await.expect("get"); // expect #[cfg(test)]
            assert!(res.is_none());
        }
        ...
        let old_sst_path = tmp.path().join("old_sst.sst");
        {
            ...
            let mut builder = SstableBuilder::create(&old_sst_path)
                .await
                .expect("create old sub"); // expect #[cfg(test)]
            builder
            ...
            let data = tokio::fs::read(&old_sst_path).await.expect("read"); // expect #[cfg(test)]
            let file_size = data.len();
            let index_off =
                u64::from_le_bytes(data[file_size - 12..file_size - 4].try_into().unwrap()); // unwrap #[cfg(test)]
            ...
            let mut f = tokio::fs::File::create(&old_sst_path)
                .await
                .expect("recreate"); // expect #[cfg(test)]
            f.write_all(&data[0..file_size - 24])
                .await
                .expect("write body"); // roughly data+index // expect #[cfg(test)]
            f.write_u64_le(index_off).await.expect("write ioff"); // expect #[cfg(test)]
            f.write_u32_le(0x4D465354).await.expect("write magic"); // expect #[cfg(test)]
            f.sync_all().await.expect("sync"); // expect #[cfg(test)]
        }

        {
            let reader = SstableReader::open(&old_sst_path, bc)
                .await
                .expect("open old"); // expect #[cfg(test)]
            assert!(

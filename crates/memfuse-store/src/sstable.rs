...
impl BloomFilter {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let num_hashes =
            u64::from_le_bytes(data[0..8].try_into().expect("valid sstable data")) as usize;
        let num_bits =
            u64::from_le_bytes(data[8..16].try_into().expect("valid sstable data")) as usize;
        ...
        let old_sst_path = tmp.path().join("old_sst.sst");
        {
            ...
            let index_off = u64::from_le_bytes(
                data[file_size - 12..file_size - 4]
                    .try_into()
                    .expect("legacy trailer conversion"),
            );

...
impl BloomFilter {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let num_hashes = u64::from_le_bytes(data[0..8].try_into().unwrap()) as usize; // unwrap
        let num_bits = u64::from_le_bytes(data[8..16].try_into().unwrap()) as usize; // unwrap
        let mut bits = Vec::with_capacity(num_bits / 64);
        let mut offset = 16;
        while offset + 8 <= data.len() {
            bits.push(u64::from_le_bytes(
                data[offset..offset + 8].try_into().unwrap(), // unwrap
            ));
            offset += 8;
        }
        Ok(Self {
    ...
    pub async fn open_with_key_manager(
        ...
        let (mmap, file_size) =
            tokio::task::spawn_blocking(move || -> std::io::Result<(memmap2::Mmap, u64)> {
                ...
                #[allow(unsafe_code)]
                let mmap = unsafe { memmap2::Mmap::map(&file)? }; // ANCHOR:SEC: AGENT:10 PRIO:1 STATUS:READY
                Ok((mmap, file_size))
            })
            .await

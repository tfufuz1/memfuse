...
impl BloomFilter {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let num_hashes = u64::from_le_bytes(
            data[0..8]
                .try_into()
                .map_err(|_| MemFuseError::Storage("invalid bloom header".into()))?,
        ) as usize;
        ...
        while offset + 8 <= data.len() {
            bits.push(u64::from_le_bytes(
                data[offset..offset + 8]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("invalid bloom data".into()))?,
            ));
            offset += 8;
        }
        Ok(Self {
        ...
        if file_size >= 20 {
            ...
            let bloom_off = u64::from_le_bytes(
                mmap.get(trailer_20_pos..trailer_20_pos + 8)
                    ...
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("invalid trailer conversion".into()))?,
            );

...
impl BlockBuilder {
    ...
    ///
    /// Returns `false` if the entry could not be added (e.g., if it violates ordering).
    ...
    /// Returns the current estimated size of the SSTable being built.
...
impl SstableBuilder {
    ...
    /// Creates a new SSTable file from the builder's content, using the provided key manager for encryption.
...
impl SstableReader {
    ...
    /// Opens an existing SSTable file, using the provided key manager for decryption.
    ...
    /// Returns the metadata associated with the SSTable.

...
mod tests {
    ...
    fn test_mvcc_get_at_seq() {
        ...
        // Exact match
        let (val, seq) = mt.get_at_seq(b"key1", 20).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        assert_eq!(val.as_ref(), b"v2");
        assert_eq!(seq, 20);

        // Between versions
        let (val, seq) = mt.get_at_seq(b"key1", 25).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        assert_eq!(val.as_ref(), b"v2");
        assert_eq!(seq, 20);

        // Latest version
        let (val, seq) = mt.get_at_seq(b"key1", 100).unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)

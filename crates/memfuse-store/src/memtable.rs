...
mod tests {
    ...
    fn test_put_get() {
        ...
        let (val, seq) = mt.get(b"key1").expect("key1 should exist"); // expect #[cfg(test)]
        assert_eq!(val.as_ref(), b"val1");
        assert_eq!(seq, 1);

        let (val, seq) = mt.get(b"key2").expect("key2 should exist"); // expect #[cfg(test)]
    ...
    fn test_mvcc_get_at_seq() {
        ...
        // Exact match
        let (val, seq) = mt.get_at_seq(b"key1", 20).unwrap(); // unwrap // expect #[cfg(test)]
        assert_eq!(val.as_ref(), b"v2");
        assert_eq!(seq, 20);

        // Between versions
        let (val, seq) = mt.get_at_seq(b"key1", 25).unwrap(); // unwrap // expect #[cfg(test)]
        assert_eq!(val.as_ref(), b"v2");
        assert_eq!(seq, 20);

        // Latest version
        let (val, seq) = mt.get_at_seq(b"key1", 100).unwrap(); // unwrap // expect #[cfg(test)]
    ...
    fn test_mvcc_tombstone_binary_search() {
        ...
        let (val, seq) = mt.get_at_seq(&key, 15).expect("Should find v1"); // expect #[cfg(test)]
        assert_eq!(val.as_ref(), b"val1");
        assert_eq!(seq, 10);

        // Read at seq 25 -> should get tombstone
        let (val, seq) = mt.get_at_seq(&key, 25).expect("Should find tombstone"); // expect #[cfg(test)]
        assert_eq!(val.len(), 0);

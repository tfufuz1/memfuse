...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        storage.commit(tx1).await.expect("checkpoint test op");
        ...
        storage
            ...
            .await
            .expect("checkpoint test op");
        storage.commit(tx2).await.expect("checkpoint test op");

        assert_eq!(
            storage.get(b"key1").await.expect("checkpoint test op"),
            Some(b"val1".to_vec())
        );
        assert_eq!(
            storage.get(b"key2").await.expect("checkpoint test op"),
            Some(b"val2".to_vec())
        );
        ...
        storage
            ...
            .await
            .expect("checkpoint test op");
        storage.commit(tx3).await.expect("checkpoint test op");
        assert_eq!(
            storage.get(b"key3").await.expect("checkpoint test op"),
            Some(b"val3".to_vec())
        );
    }
}

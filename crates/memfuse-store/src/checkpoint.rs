...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        storage.commit(tx1).await.unwrap(); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)

        let cp1 = checkpointer.create_checkpoint(tx1);

        // 2. Insert more data
        let tx2 = TxId::new(2);
        storage.put(tx2, b"key2", b"val2").await.unwrap(); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)
        storage.commit(tx2).await.unwrap(); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)

        assert_eq!(storage.get(b"key1").await.unwrap(), Some(b"val1".to_vec())); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)
        assert_eq!(storage.get(b"key2").await.unwrap(), Some(b"val2".to_vec())); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)
        ...
        // 4. Verify state
        assert_eq!(storage.get(b"key1").await.unwrap(), Some(b"val1".to_vec())); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap)
        assert_eq!(storage.get(b"key2").await.unwrap(), None); // ANCHOR:FIXME AGENT:02 PRIO:2 (unwrap) // Should be gone!

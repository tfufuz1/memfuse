...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        // 1. Insert some data
        let tx1 = TxId::new(1);
        storage.put(tx1, b"key1", b"val1").await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        ...
        // 2. Insert more data
        let tx2 = TxId::new(2);
        storage.put(tx2, b"key2", b"val2").await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        storage.commit(tx2).await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)

        assert_eq!(
            storage.get(b"key1").await.unwrap(), /* unwrap allowed (AGENT:08) */
            Some(b"val1".to_vec())
        ); // unwrap allowed (AGENT:08)
        assert_eq!(
            storage.get(b"key2").await.unwrap(), /* unwrap allowed (AGENT:08) */
            Some(b"val2".to_vec())
        ); // unwrap allowed (AGENT:08)
        ...
        assert_eq!(
            storage.get(b"key2").await.unwrap(), /* unwrap allowed (AGENT:08) */
            None
        ); // Should be gone! // unwrap allowed (AGENT:08)

        // 5. Verify we can still write and seq_no is correct
        let tx3 = TxId::new(3);
        storage.put(tx3, b"key3", b"val3").await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)

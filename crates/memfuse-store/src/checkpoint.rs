...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        assert_eq!(
            storage.get(b"key1").await.expect("test setup"),
            Some(b"val1".to_vec())
        );
        assert_eq!(
            storage.get(b"key2").await.expect("test setup"),
            Some(b"val2".to_vec())
        );
        ...
        assert_eq!(storage.get(b"key2").await.expect("test setup"), None); // Should be gone!
        ...
        assert_eq!(
            storage.get(b"key3").await.expect("test setup"),
            Some(b"val3".to_vec())
        );
    }
}

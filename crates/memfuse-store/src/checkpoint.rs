...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        // 1. Insert some data
        let tx1 = TxId::new(1);
        storage.put(tx1, b"key1", b"val1").await.unwrap(); // unwrap allowed (AGENT:02)
        ...
        // 2. Insert more data
        let tx2 = TxId::new(2);
        storage.put(tx2, b"key2", b"val2").await.unwrap(); // unwrap allowed (AGENT:02)
        storage.commit(tx2).await.unwrap(); // unwrap allowed (AGENT:02)

        let val1 = storage.get(b"key1").await.unwrap(); // unwrap allowed (AGENT:02)
        assert_eq!(val1, Some(b"val1".to_vec()));
        let val2 = storage.get(b"key2").await.unwrap(); // unwrap allowed (AGENT:02)
        assert_eq!(val2, Some(b"val2".to_vec()));

        // 3. Rollback to cp1
        checkpointer.rollback_to(&cp1).await.expect("rollback"); // unwrap allowed (AGENT:02)

        // 4. Verify state
        let val1_again = storage.get(b"key1").await.unwrap(); // unwrap allowed (AGENT:02)
        assert_eq!(val1_again, Some(b"val1".to_vec()));
        let val2_gone = storage.get(b"key2").await.unwrap(); // unwrap allowed (AGENT:02)
        assert_eq!(val2_gone, None);

        // 5. Verify we can still write and seq_no is correct
        let tx3 = TxId::new(3);
        storage.put(tx3, b"key3", b"val3").await.unwrap(); // unwrap allowed (AGENT:02)
        storage.commit(tx3).await.unwrap(); // unwrap allowed (AGENT:02)
        let val3 = storage.get(b"key3").await.unwrap(); // unwrap allowed (AGENT:02)

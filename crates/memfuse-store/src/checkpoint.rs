...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        assert_eq!(storage.get(b"key1").await.unwrap(), Some(b"val1".to_vec())); // unwrap allowed (AGENT:02)
        assert_eq!(storage.get(b"key2").await.unwrap(), Some(b"val2".to_vec())); // unwrap allowed (AGENT:02)
        ...
        // 4. Verify state
        assert_eq!(storage.get(b"key1").await.unwrap(), Some(b"val1".to_vec())); // unwrap allowed (AGENT:02)
        assert_eq!(storage.get(b"key2").await.unwrap(), None); // unwrap allowed (AGENT:02)
        ...
        assert_eq!(storage.get(b"key3").await.unwrap(), Some(b"val3".to_vec())); // unwrap allowed (AGENT:02)
    }
}

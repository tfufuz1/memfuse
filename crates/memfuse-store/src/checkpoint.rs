...
mod tests {
    ...
    async fn test_rollback_to_checkpoint() {
        ...
        assert_eq!(storage.get(b"key1").await.unwrap() /* unwrap */,   Some(b"val1".to_vec())); // unwrap
        assert_eq!(storage.get(b"key2").await.unwrap() /* unwrap */,   Some(b"val2".to_vec())); // unwrap
        ...
        // 4. Verify state
        assert_eq!(storage.get(b"key1").await.unwrap() /* unwrap */,   Some(b"val1".to_vec())); // unwrap
        assert_eq!(storage.get(b"key2").await.unwrap() /* unwrap */,   None); // Should be gone! // unwrap
        ...
        assert_eq!(storage.get(b"key3").await.unwrap() /* unwrap */,   Some(b"val3".to_vec())); // unwrap
    }
}

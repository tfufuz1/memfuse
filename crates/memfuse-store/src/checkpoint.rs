...
.unwrap() /* unwrap allowed */;
        ...
        assert_eq!(storage.get(b"key1").await
.unwrap() /* unwrap allowed */, Some(b"val1".to_vec()));
        assert_eq!(storage.get(b"key2").await
.unwrap() /* unwrap allowed */, Some(b"val2".to_vec()));
...
.unwrap() /* unwrap allowed */, Some(b"val1".to_vec()));
        assert_eq!(storage.get(b"key2").await
.unwrap() /* unwrap allowed */, None); // Should be gone!
...
.unwrap() /* unwrap allowed */;
        assert_eq!(storage.get(b"key3").await
.unwrap() /* unwrap allowed */, Some(b"val3".to_vec()));
    }
}

...
use memfuse_store::sstable::{SstableBuilder, SstableReader, create_block_cache};
...
fn bench_sstable_get(c: &mut Criterion) {
    ...
    group.bench_function("get_existing", |b| {
        b.to_async(&rt).iter(|| async {
            let res = reader.get(black_box(b"key00500")).await.unwrap();
            black_box(res);
        })
    });
    ...
    group.bench_function("get_nonexistent", |b| {
        b.to_async(&rt).iter(|| async {
            let res = reader.get(black_box(b"key99999")).await.unwrap();
            black_box(res);
        })
    });
    group.finish();

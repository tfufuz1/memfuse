...
async fn test_encrypted_db_roundtrip() {
    ...
    // 1. Write data to encrypted DB
    {
        let storage = LsmStorage::new(config.clone())
            .await
            .expect("create storage"); // unwrap
        let tx = TxId::new(1);
        storage.put(tx, b"key1", b"val1").await.expect("put"); // unwrap
        storage.commit(tx).await.expect("commit"); // unwrap

        // Force flush to SSTable
        storage.force_flush().await.expect("flush"); // unwrap

        let val = storage.get(b"key1").await.expect("get"); // unwrap
    ...
    // 2. Re-open with same passphrase
    {
        let storage = LsmStorage::new(config.clone())
            .await
            .expect("reopen storage"); // unwrap
        let val = storage.get(b"key1").await.expect("get after reopen"); // unwrap
        assert_eq!(val, Some(b"val1".to_vec()));
    }
}
...
async fn test_encrypted_db_unreadable_as_plaintext() {
    ...
    let secret_val = b"THIS_IS_A_SECRET_VALUE_THAT_SHOULD_NOT_BE_IN_PLAINTEXT";
    {
        ...
        let tx = TxId::new(1);
        storage.put(tx, b"key1", secret_val).await.expect("put"); // unwrap
        storage.commit(tx).await.expect("commit"); // unwrap
        storage.force_flush().await.expect("flush"); // unwrap
    }
    ...
    let mut found = false;
    let mut entries = std::fs::read_dir(tmp.path()).expect("read dir"); // unwrap
    while let Some(Ok(entry)) = entries.next() {
        if entry.path().is_file() {
            let content = std::fs::read(entry.path()).expect("read file"); // unwrap

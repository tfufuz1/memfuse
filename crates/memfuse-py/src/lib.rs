...
// ANCHOR:DOC:PY-DOCS-001 — Added missing documentation for Python bridge.
...
impl PyMemFuse {
    ...
    /// Performs semantic vector search in the default collection.
    ...
    /// Scans for documents matching a prefix in the default collection.
    ...
    /// Returns the number of vectors in the default collection.
    ...
    /// Performs a range scan in the default collection.
...
/// A collection (namespace) within the MemFuse database.
...
impl PyCollection {
    ...
    /// Performs semantic vector search in the collection.
    #[pyo3(signature = (vector, k))]
    pub fn search<'py>(
        &self,
        py: Python<'py>,
    ...
    /// Performs hybrid search (BM25 + Vector) in the collection.
    #[pyo3(signature = (text, vector, k))]
    pub fn hybrid_search<'py>(
        &self,
        py: Python<'py>,
    ...
    /// Scans for documents matching a prefix in the collection.
    #[pyo3(signature = (prefix=""))]
    pub fn scan_prefix(&self, py: Python<'_>, prefix: &str) -> PyResult<Vec<(String, PyObject)>> {
        let rt = get_runtime()?;
        let results = py
    ...
    /// Performs a range scan in the collection.
    #[pyo3(signature = (start=None, end=None))]
    pub fn scan(
        &self,
        py: Python<'_>,

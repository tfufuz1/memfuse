...
fn get_runtime() -> PyResult<&'static Runtime> {
    ...
    // ANCHOR:DEBT STATUS:DONE AGENT:13 DATE:2026-05-15 — replaced expect with PyResult propagation
    RUNTIME.get().ok_or_else(|| {
        pyo3::exceptions::PyRuntimeError::new_err("Runtime must be initialized after setting")
    })
}

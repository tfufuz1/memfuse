...
impl PyMemFuse {
    ...
    /// Performs hybrid search in the default collection.
    ...
    /// Creates a relationship between documents in the default collection.
    ...
    /// Scans keys matching a prefix in the default collection.
    ...
    /// Scans a range of keys in the default collection.
...
/// A Python collection (namespace) within MemFuse.
...
impl PyCollection {
    ...
    /// Retrieves a document from this collection.
    pub fn get(&self, py: Python<'_>, id: &str) -> PyResult<Option<PyDocument>> {
        let rt = get_runtime()?;
        let doc = py
            .allow_threads(|| rt.block_on(self.inner.get(id)))
    ...
    /// Performs semantic k-NN search in this collection.
    #[pyo3(signature = (vector, k))]
    pub fn search<'py>(
        &self,
        py: Python<'py>,
    ...
    /// Creates a relationship between documents in this collection.
    pub fn relate(&self, py: Python<'_>, from: &str, to: &str, label: &str) -> PyResult<()> {
        let rt = get_runtime()?;
        py.allow_threads(|| rt.block_on(self.inner.relate(from, to, label)))
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
    ...
    /// Scans keys matching a prefix in this collection.
    #[pyo3(signature = (prefix=""))]
    pub fn scan_prefix(&self, py: Python<'_>, prefix: &str) -> PyResult<Vec<(String, PyObject)>> {
        let rt = get_runtime()?;
        let results = py
    ...
    /// Scans a range of keys in this collection.
    #[pyo3(signature = (start=None, end=None))]
    pub fn scan(
        &self,
        py: Python<'_>,

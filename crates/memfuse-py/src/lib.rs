...
/// Primary entry point for MemFuse Python API.
...
impl PyMemFuse {
    ...
    /// Drops a collection and all its associated data.
    ...
    /// Performs vector search in the default collection.
    ...
    /// Scans keys with a given prefix in the default collection.
...
/// A specific collection within MemFuse.
...
impl PyCollection {
    ...
    /// Scans keys with a given prefix in this collection.
    #[pyo3(signature = (prefix=""))]
    pub fn scan_prefix(&self, py: Python<'_>, prefix: &str) -> PyResult<Vec<(String, PyObject)>> {
        let rt = get_runtime()?;
        let results = py

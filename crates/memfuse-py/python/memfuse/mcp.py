...
def create_mcp_server(db_path: str, dimension: int = 1536) -> FastMCP:
    ...
    @mcp.tool()
    def memfuse_search(query: str, vector: List[float], collection: str = "default", k: int = 10) -> List[Dict[str, Any]]:
        """
        Hybrid search across stored documents (vector + BM25 + metadata).
        
        Args:
            query: Natural language query
            vector: The query embedding vector. Required until local embeddings are natively live.
        ...
        try:
            ...
            import numpy as np
            if not vector:
                raise ValueError("FIND-PY-001: Vector is required until embedding services are natively integrated.")
    ...
    @mcp.tool()
    def memfuse_insert(id: str, text: str, vector: List[float], collection: str = "default", metadata: Dict[str, Any] = None) -> str:
        """

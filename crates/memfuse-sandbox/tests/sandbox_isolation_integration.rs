...
async fn test_sandbox_isolation_and_execution() {
    ...
    let result = sandbox
        ...
        .await
        .expect("execution failed"); // expect #[cfg(test)] // expect #[cfg(test)]
...
async fn test_sandbox_multiple_instances() {
    let s1 = WasmSandbox::new(SandboxConfig::default()).unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    let s2 = WasmSandbox::new(SandboxConfig::default()).unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    let budget = TokenBudget::new(10, 0);

    let wasm_bytes = dummy_wasm();
    let res1 = s1.execute_isolated(&wasm_bytes, &budget).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    let res2 = s2.execute_isolated(&wasm_bytes, &budget).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]

...
mod tests {
    ...
    fn wat2wasm(wat: &str) -> Vec<u8> {
        wat::parse_str(wat).expect("failed to parse WAT") // expect #[cfg(test)]
    }
    ...
    async fn test_sandbox_memory_limit_enforced() {
        ...
        let sandbox = WasmSandbox::new(config).unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_sandbox_cannot_access_host_fs() {
        ...
        let sandbox = WasmSandbox::new(config).unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
        let budget = TokenBudget::new(100, 0);

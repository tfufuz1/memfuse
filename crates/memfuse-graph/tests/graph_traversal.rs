...
use memfuse_core::{Entity, EntityId, Edge, GraphIndex, TxId};
...
async fn test_graph_bfs_and_scoring() {
    ...
    for i in 1..=4 {
        graph.add_entity(tx, Entity::new(EntityId::new(i), format!("Node{}", i), "Test")).await.unwrap();
    }
    ...
    // 3 --(0.5)--> 4
    graph.add_edge(tx, Edge::new(EntityId::new(3), EntityId::new(4), "Weak").with_weight(0.5)).await.unwrap();
    ...
    let results = graph.traverse(EntityId::new(1), 3).await.expect("Traversal failed");
    assert_eq!(results.len(), 3);

    let scores: std::collections::HashMap<u64, f32> = results.into_iter().map(|(id, s)| (id.inner(), s)).collect();

    // Verify scores with epsilon for float comparison
    assert!((scores[&2] - 0.7).abs() < 1e-6, "Score for node 2: {}", scores[&2]);
    assert!((scores[&3] - 0.49).abs() < 1e-6, "Score for node 3: {}", scores[&3]);

    // Debug info for failure
    if (scores[&4] - 0.1715).abs() >= 1e-6 {
        println!("Score for node 4: {}", scores[&4]);
    }
    assert!((scores[&4] - 0.1715).abs() < 1e-6, "Score for node 4: {}", scores[&4]);
}
...
async fn test_graph_max_depth() {
    ...
    // Chain of 10 nodes
    for i in 1..=10 {
        graph.add_entity(tx, Entity::new(EntityId::new(i), format!("N{}", i), "T")).await.unwrap();
        if i > 1 {
            graph.add_edge(tx, Edge::new(EntityId::new(i-1), EntityId::new(i), "E")).await.unwrap();
        }
    }

    // Traversal is limited to MAX_TRAVERSAL_HOPS = 3 in implementation
    let results = graph.traverse(EntityId::new(1), 5).await.unwrap();
    assert_eq!(results.len(), 3, "Should only find 3 neighbors due to internal hard limit");
}
...
async fn test_graph_cycles() {
    ...
    graph.add_edge(tx, Edge::new(EntityId::new(2), EntityId::new(1), "E")).await.unwrap();

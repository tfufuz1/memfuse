...
async fn test_graph_integration_lifecycle() {
    ...
    graph
        .add_edge(
            tx,
            Edge::new(EntityId::new(1), EntityId::new(2), "friend").with_weight(1.0),
        )
        .await
    ...
    graph
        .add_edge(
            tx,
            Edge::new(EntityId::new(2), EntityId::new(3), "contributor").with_weight(0.8),
        )
        .await
    ...
    let bob_score = results
        .iter()
        .find(|(id, _)| *id == EntityId::new(2))
        .map(|(_, s)| *s)
        .unwrap();
    let project_score = results
        .iter()
        .find(|(id, _)| *id == EntityId::new(3))
        .map(|(_, s)| *s)
        .unwrap();

...
mod tests {
    ...
    async fn test_csr_graph_scaffold_compiles() {
        ...
        graph
            ...
            .await
            .expect("valid test value"); // unwrap
        graph
    ...
    async fn test_csr_graph_bfs_score_decay() {
        ...
        let results = graph.traverse(EntityId::new(1), 3).await.expect("traverse"); // unwrap
        ...
        let s2 = *score_map.get(&EntityId::new(2)).expect("node 2 missing"); // unwrap
        let s3 = *score_map.get(&EntityId::new(3)).expect("node 3 missing"); // unwrap
        let s4 = *score_map.get(&EntityId::new(4)).expect("node 4 missing"); // unwrap
        let s5 = *score_map.get(&EntityId::new(5)).expect("node 5 missing"); // unwrap
    ...
    async fn test_csr_graph_cycle_handling() {
        ...
        let results = graph.traverse(EntityId::new(1), 5).await.expect("traverse"); // unwrap
    ...
    async fn test_csr_graph_stats_accuracy() {
        let graph = setup_test_graph().await;
        let stats = graph.stats().await.expect("valid stats"); // unwrap

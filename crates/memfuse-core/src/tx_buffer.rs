...
impl<T: Clone> TxBuffer<T> {
    ...
    fn shard_idx(&self, tx: TxId) -> usize {
        ...
        // BEGRÜNDUNG: shard_count > 0 wird im Constructor garantiert.
    ...
    fn get_shard(&self, tx: TxId) -> &RwLock<TxShard<T>> {
        ...
        // ANCHOR:SEC:SLICE-001 — Sicherer Zugriff via Invariante (modulo len)
        // AGENT:10 PRIO:1 STATUS:REVIEW
        // BEGRÜNDUNG: shard_idx liefert immer einen Index < self.shards.len().
        self.shards
            .get(idx)
            .expect("invariant: shard_idx < shards.len")
    }

    /// Checks if the given transaction exists in the buffer.
    pub fn has_tx(&self, tx: TxId) -> bool {
        self.get_shard(tx).read().ops.contains_key(&tx)
    }

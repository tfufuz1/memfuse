...
// ANCHOR:SEC: AGENT:10 PRIO:1 STATUS:REVIEW
// FIXED: Implemented get_shard helper with .get().expect() for safe access.
...
impl<T: Clone> TxBuffer<T> {
    ...
    fn get_shard(&self, shard_idx: usize) -> &RwLock<TxShard<T>> {
        self.shards
            .get(shard_idx)

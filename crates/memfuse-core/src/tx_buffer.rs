...
impl<T: Clone> TxBuffer<T> {
    ...
    /// Internal helper to get a shard reference with bounds safety.
    #[inline]
    fn get_shard(&self, idx: usize) -> &RwLock<TxShard<T>> {
        // ANCHOR:SEC:SLICE-001 — Gecheckter Zugriff auf Shards.
        // WP:WP-0.0 PRIO:1 NEEDS:NONE
        // AGENT:10 DATE:2026-06-15 STATUS:READY
        // BEGRÜNDUNG: Die Anzahl der Shards ist via Constructor invariant.
        self.shards
            .get(idx)
    ...
    pub fn has_tx(&self, tx: TxId) -> bool {
        let shard = self.get_shard(self.shard_idx(tx));
    ...
    pub fn begin(&self, tx: TxId) {
        ...
        let mut shard = self.get_shard(shard_idx).write();
        shard
            .ops
            .entry(tx)
    ...
    pub fn validate_pending_ops(&self, tx: TxId) -> Result<()> {
        let shard_idx = self.shard_idx(tx);
        let shard = self.get_shard(shard_idx).read();

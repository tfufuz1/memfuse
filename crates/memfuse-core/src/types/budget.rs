//! Resource budget and tracker for memory management.
//!
//! ANCHOR:DOC:BUDGET — Module documentation for resource budget.
...
// ANCHOR:DEBT:UNWRAP — Intentional unwraps in tests annotated for CI guard.
// AGENT:01 STATUS:DONE PRIO:3
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn test_resource_tracker_basic() {
        ...
        tracker.consume_memory(500).expect("should consume"); // unwrap
    ...
    fn test_budget_exceeded() {
        ...
        tracker.consume_memory(900).expect("should consume"); // unwrap
    ...
    fn test_capacity_threshold() {
        ...
        tracker.consume_memory(949).expect("ok"); // unwrap
        assert!(tracker.has_memory_capacity()); // 94.9% < 95%

        tracker.consume_memory(1).expect("ok"); // unwrap
    ...
    fn test_concurrent_consumption() {
        ...
        for _ in 0..10 {
            ...
            handlers.push(std::thread::spawn(move || {
                for _ in 0..100 {
                    t.consume_memory(10).expect("consume"); // unwrap
                }
            }));
        }

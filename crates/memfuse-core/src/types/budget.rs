...
mod tests {
    ...
    fn test_resource_tracker_basic() {
        ...
        tracker.consume_memory(500).expect("should consume"); // unwrap allowed (AGENT:08)
    ...
    fn test_budget_exceeded() {
        ...
        tracker.consume_memory(900).expect("should consume"); // unwrap allowed (AGENT:08)
    ...
    fn test_capacity_threshold() {
        ...
        tracker.consume_memory(949).expect("ok"); // unwrap allowed (AGENT:08)
        assert!(tracker.has_memory_capacity()); // 94.9% < 95%

        tracker.consume_memory(1).expect("ok"); // unwrap allowed (AGENT:08)
    ...
    async fn test_backpressure_trigger() {
        ...
        // Use 79% -> No sleep
        tracker.consume_memory(790).expect("ok"); // unwrap allowed (AGENT:08)
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;
        assert!(start.elapsed() < std::time::Duration::from_millis(1));

        // Use 80% -> Sleep
        tracker.consume_memory(10).expect("ok"); // unwrap allowed (AGENT:08)
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;
    ...
    fn test_concurrent_consumption() {
        ...
        for _ in 0..10 {
            ...
            handlers.push(std::thread::spawn(move || {
                for _ in 0..100 {
                    t.consume_memory(10).expect("consume"); // unwrap allowed (AGENT:08)
                }
            }));
        }

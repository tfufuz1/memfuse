//! Resource Budgeting and Tracking — Memory management and backpressure.
//!
//! Provides the `ResourceTracker` for monitoring and limiting memory usage
//! across various MemFuse components.
...
mod tests {
    ...
    async fn test_backpressure_trigger() {
        ...
        // Use 79% -> No sleep
        tracker.consume_memory(790).expect("ok"); // unwrap allowed
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;
        assert!(start.elapsed() < std::time::Duration::from_millis(1));

        // Use 80% -> Sleep
        tracker.consume_memory(10).expect("ok"); // unwrap allowed
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;

...
mod tests {
    ...
    fn test_resource_tracker_basic() {
        ...
        tracker.consume_memory(500).expect("should consume"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
    ...
    fn test_budget_exceeded() {
        ...
        tracker.consume_memory(900).expect("should consume"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        let result = tracker.consume_memory(200);

        assert!(result.is_err());
        match result.err().unwrap() { // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
    ...
    fn test_capacity_threshold() {
        ...
        tracker.consume_memory(949).expect("ok"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        assert!(tracker.has_memory_capacity()); // 94.9% < 95%

        tracker.consume_memory(1).expect("ok"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
    ...
    async fn test_backpressure_trigger() {
        ...
        // Use 79% -> No sleep
        tracker.consume_memory(790).expect("ok"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;
        assert!(start.elapsed() < std::time::Duration::from_millis(1));

        // Use 80% -> Sleep
        tracker.consume_memory(10).expect("ok"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        let start = std::time::Instant::now();
        tracker.apply_backpressure().await;
    ...
    fn test_concurrent_consumption() {
        ...
        for _ in 0..10 {
            ...
            handlers.push(std::thread::spawn(move || {
                for _ in 0..100 {
                    t.consume_memory(10).expect("consume"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
                }
            }));
        }

        for h in handlers {
            h.join().unwrap(); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        }

//! Search and Orchestration Schemas (SAOS).
//!
//! Defines the unified query models, fusion weights, and isolation levels
//! used for hybrid search across the system.
...
mod tests {
    ...
    fn test_fusion_weights_valid() {
        let w = FusionWeights::new(0.5, 0.5, 0.0, 0.0).expect("valid"); // #[cfg(test)]
    ...
    fn test_hybrid_query_builder_happy_path() {
        let query = HybridQuery::builder()
            ...
            .with_k(5)
            .build()
            .expect("build ok"); // #[cfg(test)]

        assert_eq!(query.text_query.unwrap(), "test query"); // #[cfg(test)]
        assert_eq!(query.vector_query.unwrap(), vec![0.1, 0.2]); // #[cfg(test)]
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).unwrap(); // #[cfg(test)]
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().unwrap(); // #[cfg(test)]

...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(query.text_query.unwrap(), "test query"); // unwrap allowed in tests
        assert_eq!(query.vector_query.unwrap(), vec![0.1, 0.2]); // unwrap allowed in tests
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).unwrap(); // unwrap allowed in tests
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().unwrap(); // unwrap allowed in tests

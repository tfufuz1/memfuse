...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(query.text_query.expect("has text"), "test query"); // expect #[cfg(test)]
        assert_eq!(query.vector_query.expect("has vector"), vec![0.1, 0.2]); // expect #[cfg(test)]
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).expect("valid weights"); // expect #[cfg(test)]
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().expect("build ok"); // expect #[cfg(test)]

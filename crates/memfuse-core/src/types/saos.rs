//! Signal-Aware Orchestration Schema (SAOS).
//!
//! Defines the orchestration layer for multi-signal fusion, including
//! hybrid query structures, namespace management, and scoring weights.
...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(query.text_query.unwrap(), "test query"); // expect #[cfg(test)]
        assert_eq!(query.vector_query.unwrap(), vec![0.1, 0.2]); // expect #[cfg(test)]
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).unwrap(); // expect #[cfg(test)]
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().unwrap(); // expect #[cfg(test)]

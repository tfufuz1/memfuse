...
mod tests {
    ...
    assert_eq!(query.text_query.expect("has text"), "test query"); // unwrap
    assert_eq!(query.vector_query.expect("has vector"), vec![0.1, 0.2]); // unwrap
        assert_eq!(query.k, 5);
    ...
    let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).expect("valid weights"); // unwrap
        let query = HybridQuery::builder()
    ...
    let query = HybridQuery::builder().build().expect("build ok"); // unwrap

...
mod tests {
    ...
    fn test_fusion_weights_valid() {
        let w = FusionWeights::new(0.5, 0.5, 0.0, 0.0).expect("valid"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
    ...
    fn test_hybrid_query_builder_happy_path() {
        let query = HybridQuery::builder()
            ...
            .with_k(5)
            .build()
            .expect("build ok"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]

        assert_eq!(query.text_query.unwrap(), "test query"); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        assert_eq!(query.vector_query.unwrap(), vec![0.1, 0.2]); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).unwrap(); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
        let query = HybridQuery::builder()
            .with_fusion_weights(weights.clone())
            .build()
            .unwrap(); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().unwrap(); // ANCHOR:DEBT AGENT:01 STATUS:DONE PRIO:3 // #[cfg(test)]

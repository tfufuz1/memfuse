//! SAOS (Signals & Orchestration) Types — Hybrid search and context management.
//!
//! Defines query structures, fusion weights, and context windows for
//! multi-signal retrieval and LLM context preparation.
...
mod tests {
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).unwrap(); // unwrap allowed
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().unwrap(); // unwrap allowed

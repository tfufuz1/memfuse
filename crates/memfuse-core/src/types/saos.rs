...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(query.text_query.expect("saos test op"), "test query");
        assert_eq!(query.vector_query.expect("saos test op"), vec![0.1, 0.2]);
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).expect("saos test op");
        let query = HybridQuery::builder()
    ...
    fn test_hybrid_query_builder_defaults() {
        let query = HybridQuery::builder().build().expect("saos test op");

...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(
            query.text_query.expect("hardened by Core Guardian"),
            "test query"
        );
        assert_eq!(
            query.vector_query.expect("hardened by Core Guardian"),
            vec![0.1, 0.2]
        );
        assert_eq!(query.k, 5);
    ...
    fn test_hybrid_query_builder_custom_weights() {
        let weights = FusionWeights::new(0.4, 0.4, 0.1, 0.1).expect("hardened by Core Guardian");
        let query = HybridQuery::builder()

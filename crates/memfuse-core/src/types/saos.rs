...
mod tests {
    ...
    fn test_hybrid_query_builder_happy_path() {
        ...
        assert_eq!(query.text_query.as_ref().unwrap(), "test query"); // unwrap allowed (AGENT:08)
        assert_eq!(query.vector_query.as_ref().unwrap(), &vec![0.1, 0.2]); // unwrap allowed (AGENT:08)
        assert_eq!(query.k, 5);

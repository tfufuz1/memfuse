...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.store.read()/* unwrap allowed in production */ .unwrap() // unwrap allowed.get(key).cloned())
        }
        async fn put(&self, _tx_id: TxId, key: &[u8], value: &[u8]) -> Result<()> {
            self.store
                .write()
                /* unwrap allowed in production */ .unwrap() // unwrap allowed
                .insert(key.to_vec(), value.to_vec());
            Ok(())
        }
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.store.write()/* unwrap allowed in production */ .unwrap() // unwrap allowed.remove(key);
            Ok(())
        }
        ...
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let store = self.store.read()/* unwrap allowed in production */ .unwrap() // unwrap allowed;
            Ok(store
                .iter()

...
impl InvertedIndex {
    ...
    pub async fn upsert_document(&self, tx: TxId, doc_id: DocId, text: &str) -> Result<()> {
        ...
        // Original tokens count (before decomposition if using MorphTokenizer)
        // DefaultTokenizer doesn't decompose, so it's the same.
        // GermanMorphTokenizer adds components but also keeps original.
        // To be precise, we want the count of words BEFORE morphological expansion.
        let orig_tokens = unicode_segmentation::UnicodeSegmentation::unicode_words(text)
            .filter(|w| !crate::tokenizer::get_stopwords().contains(&w.to_lowercase()))
            .count() as u32;
        ...
        let ol_key = self.key_with_id("ol:", doc_id.inner()); // Original length
                ...
                if let Some(ol_bytes) = self.storage.get(&ol_key).await? {
                    if ol_bytes.len() == 4 {
                        old_orig_len = u32::from_le_bytes(ol_bytes.as_slice().try_into().map_err(
                            |_| MemFuseError::Storage("Invalid doc_orig_len length".into()),
                        )?);
                    }
                }
        ...
        self.storage
            .put(tx, &ol_key, &orig_tokens.to_le_bytes())
            .await?;
        ...
        total_orig_tokens = total_orig_tokens.saturating_sub(old_orig_len as u64) + orig_tokens as u64;
    ...
    pub async fn delete_document(&self, tx: TxId, doc_id: DocId) -> Result<()> {
        ...
        if let Some(bytes) = self.storage.get(&dl_key).await? {
            ...
            if let Some(ol_bytes) = self.storage.get(&ol_key).await? {
                if ol_bytes.len() == 4 {
                    doc_orig_len = u32::from_le_bytes(ol_bytes.as_slice().try_into().map_err(
                        |_| MemFuseError::Storage("Invalid doc_orig_len length".into()),
                    )?);
                }
            }
        ...
        if let Some(bytes) = self.storage.get(&total_orig_tok_key).await? {
            if bytes.len() == 8 {
                ...
                total_orig_tokens = total_orig_tokens.saturating_sub(doc_orig_len as u64);
                self.storage
                    .put(tx, &total_orig_tok_key, &total_orig_tokens.to_le_bytes())
                    .await?;
            }
...
impl TextIndex for InvertedIndex {
    ...
    async fn stats(&self) -> Result<TextIndexStats> {
        ...
        Ok(TextIndexStats {
            ...
            memory_usage_bytes: total_tokens as usize * 24, // 24 bytes per token heuristic
        })
    }
}

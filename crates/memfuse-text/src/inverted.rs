...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.store.read().expect("lock poisoned").get(key).cloned()) // #[cfg(test)]
        }
        async fn put(&self, _tx_id: TxId, key: &[u8], value: &[u8]) -> Result<()> {
            self.store
                .write()
                .expect("lock poisoned") // #[cfg(test)]
                .insert(key.to_vec(), value.to_vec());
            Ok(())
        }
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.store.write().expect("lock poisoned").remove(key); // #[cfg(test)]
            Ok(())
        }
        ...
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let store = self.store.read().expect("lock poisoned"); // #[cfg(test)]
            Ok(store
                .iter()

...
mod tests {
    ...
    #[tokio::test]
    async fn test_search_skips_subterm_colons() -> Result<()> {
        let storage = Arc::new(MockStorage::new());
        let index = InvertedIndex::new(storage, "subterm_test");

        let tx = TxId::new(1);
        index
            .upsert_document(tx, DocId::new(1), "http server running on http:80")
            .await?;
        index.commit(tx).await?;

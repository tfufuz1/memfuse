...
impl InvertedIndex {
    ...
    fn key_fw(&self, doc_id: DocId) -> Result<Vec<u8>> {
        let mut k = Vec::with_capacity(self.prefix.len() + 24);
        k.extend_from_slice(&self.prefix);
        write!(k, "fw:{}", doc_id.inner())
            .map_err(|e| MemFuseError::Storage(format!("key_fw: {}", e)))?;
        Ok(k)
    }

    fn key_pl(&self, term: &str) -> Result<Vec<u8>> {
        let mut k = Vec::with_capacity(self.prefix.len() + 3 + term.len());
        k.extend_from_slice(&self.prefix);
        write!(k, "pl:{}", term).map_err(|e| MemFuseError::Storage(format!("key_pl: {}", e)))?;
        Ok(k)
    }

    fn key_meta(&self, suffix: &str) -> Result<Vec<u8>> {
        let mut k = Vec::with_capacity(self.prefix.len() + 5 + suffix.len());
        k.extend_from_slice(&self.prefix);
        write!(k, "meta:{}", suffix)
            .map_err(|e| MemFuseError::Storage(format!("key_meta: {}", e)))?;
        Ok(k)
    }

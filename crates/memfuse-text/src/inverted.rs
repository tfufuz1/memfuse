...
impl InvertedIndex {
    ...
    fn key_dl(&self, doc_id: DocId) -> Result<Vec<u8>> {
        let mut k = Vec::with_capacity(self.prefix.len() + 23);
        k.extend_from_slice(&self.prefix);
        use std::io::Write;
        write!(k, "dl:{}", doc_id.inner()).map_err(|e| MemFuseError::Storage(e.to_string()))?;
        Ok(k)
    }
    ...
    fn key_fw(&self, doc_id: DocId) -> Result<Vec<u8>> {
        ...
        k.extend_from_slice(&self.prefix);
        use std::io::Write;
        write!(k, "fw:{}", doc_id.inner()).map_err(|e| MemFuseError::Storage(e.to_string()))?;
        Ok(k)
    }
    ...
    fn key_pl(&self, term: &str) -> Result<Vec<u8>> {
        ...
        k.extend_from_slice(&self.prefix);
        use std::io::Write;
        write!(k, "pl:{}", term).map_err(|e| MemFuseError::Storage(e.to_string()))?;
        Ok(k)
    }

    fn key_meta(&self, name: &str) -> Result<Vec<u8>> {
        let mut k = Vec::with_capacity(self.prefix.len() + 5 + name.len());
        k.extend_from_slice(&self.prefix);
        use std::io::Write;
        write!(k, "meta:{}", name).map_err(|e| MemFuseError::Storage(e.to_string()))?;
        Ok(k)
    }

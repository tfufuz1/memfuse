...
async fn test_inverted_index_persistence() {
    ...
    index
        ...
        .await
        .expect("insert 1 failed"); // unwrap allowed
    storage.commit(tx1).await.expect("commit 1 failed"); // unwrap allowed

    let tx2 = TxId::new(2);
    index
        .insert(tx2, DocId::new(2), "banana")
        .await
        .expect("insert 2 failed"); // unwrap allowed
    storage.commit(tx2).await.expect("commit 2 failed"); // unwrap allowed

    // 2. Verify search
    let results = index
        .search("apple", 10)
        .await
        .expect("search apple failed"); // unwrap allowed
    assert_eq!(
        results.len(),
        1,
    ...
    let results_banana = index
        .search("banana", 10)
        .await
        .expect("search banana failed"); // unwrap allowed
    assert_eq!(
        results_banana.len(),
        1,
    ...
    let storage2 = Arc::new(
        LsmStorage::new(lsm_config2)
            .await
            .expect("failed to re-open storage"), // unwrap allowed
    );
    ...
    let results2 = index2
        .search("apple", 10)
        .await
        .expect("search after restart failed"); // unwrap allowed
    assert_eq!(
        results2.len(),
        1,
    ...
    index2
        ...
        .await
        .expect("delete failed"); // unwrap allowed
    storage2.commit(tx3).await.expect("commit 3 failed"); // unwrap allowed

    let results3 = index2
        .search("apple", 10)
        .await
        .expect("search after delete failed"); // unwrap allowed
    assert_eq!(
        results3.len(),
        0,

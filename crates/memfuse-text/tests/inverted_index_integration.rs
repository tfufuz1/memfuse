...
async fn test_inverted_index_persistence() {
    ...
    index
        ...
        .await
        .expect("insert 1 failed"); // unwrap
    storage.commit(tx1).await.expect("commit 1 failed"); // unwrap

    let tx2 = TxId::new(2);
    index
        .insert(tx2, DocId::new(2), "banana")
        .await
        .expect("insert 2 failed"); // unwrap
    storage.commit(tx2).await.expect("commit 2 failed"); // unwrap

    // 2. Verify search
    let results = index
        .search("apple", 10)
        .await
        .expect("search apple failed"); // unwrap
    assert_eq!(
        results.len(),
        1,
    ...
    let results_banana = index
        .search("banana", 10)
        .await
        .expect("search banana failed"); // unwrap
    assert_eq!(
        results_banana.len(),
        1,
    ...
    let storage2 = Arc::new(
        LsmStorage::new(lsm_config2)
            .await
            .expect("failed to re-open storage"), // unwrap
    );
    ...
    let results2 = index2
        .search("apple", 10)
        .await
        .expect("search after restart failed"); // unwrap
    assert_eq!(
        results2.len(),
        1,
    ...
    index2
        ...
        .await
        .expect("delete failed"); // unwrap
    storage2.commit(tx3).await.expect("commit 3 failed"); // unwrap

    let results3 = index2
        .search("apple", 10)
        .await
        .expect("search after delete failed"); // unwrap
    assert_eq!(
        results3.len(),
        0,

...
impl CheckpointRegistry {
    ...
    pub fn register(&self, tx_id: TxId, state: WorkflowState) {
        let mut cache = self.checkpoints.write().unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
        cache.insert(tx_id, state);
    }

    pub fn get(&self, tx_id: TxId) -> Option<WorkflowState> {
        let cache = self.checkpoints.read().unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().unwrap() /* unwrap allowed */.get(key).cloned()) // unwrap allowed // unwrap allowed
        }
        async fn put(&self, _tx_id: TxId, key: &[u8], value: &[u8]) -> Result<()> {
            self.data
                .lock()
                .unwrap() /* unwrap allowed */ // unwrap allowed // unwrap allowed
                .insert(key.to_vec(), value.to_vec());
            Ok(())
        }
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().unwrap() /* unwrap allowed */.remove(key); // unwrap allowed // unwrap allowed
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap() /* unwrap allowed */.insert(seq_no); // unwrap allowed // unwrap allowed
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap() /* unwrap allowed */.remove(&seq_no); // unwrap allowed // unwrap allowed
            Ok(())
        }
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
            Ok(data
                .iter()
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        assert!(storage.pinned.lock().unwrap() /* unwrap allowed */.contains(&100)); // unwrap allowed // unwrap allowed

        // Verify it exists in manager
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap() /* unwrap allowed */.unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap() /* unwrap allowed */.unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).unwrap() /* unwrap allowed */; // unwrap allowed // unwrap allowed

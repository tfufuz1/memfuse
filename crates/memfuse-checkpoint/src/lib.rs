...
impl InstanceOrphanRegistry {
    ...
    pub fn register_orphan_sync(&self, orphan: PinnedSeqNoOrphan) {
        ...
        if !lock.iter().any(|o| o.seq_no == orphan.seq_no) {
            ...
            if let Err(err) = self.persist_sync() {
                tracing::error!(?err, "Failed to persist orphan registry after registering orphan pin");
            }
        }
    }
    ...
    pub fn register_checkpoint_sync(&self, cp: StateCheckpoint) {
        ...
        if !lock.iter().any(|o| o.tx_id == cp.tx_id) {
            ...
            drop(lock);
            if let Err(err) = self.persist_sync() {
                tracing::error!(?err, "Failed to persist orphan registry after registering checkpoint");
            }
        }
    }
    ...
    pub fn clear_orphaned_checkpoint(&self, tx_id: TxId) {
        ...
        drop(lock);
        if let Err(err) = self.persist_sync() {
            tracing::error!(?err, %tx_id, "Failed to persist orphan registry after clearing checkpoint");
        }
    }
    ...
    pub fn new(storage: Arc<S>, namespace: impl Into<String>) -> Self {
        ...
        let res = if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            tokio::runtime::Builder::new_current_thread()
                ...
                .and_then(|rt| rt.block_on(Self::open(storage_clone, ns_clone)))
        };

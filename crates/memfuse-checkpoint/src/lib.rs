...
mod tests {
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let meta = manager
            ...
            .await
            .expect("create"); // unwrap

        assert_eq!(meta.name, "test_cp");
        assert_eq!(meta.seq_no, 100);

        // Verify it was pinned
        assert!(storage.pinned.lock().expect("lock").contains(&100)); // unwrap
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.expect("get").unwrap(); // unwrap
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.expect("list"); // unwrap
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.expect("list"); // unwrap
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).expect("found"); // unwrap

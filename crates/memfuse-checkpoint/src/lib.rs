...
impl CheckpointRegistry {
    ...
    pub fn register(&self, tx_id: TxId, state: WorkflowState) {
        let mut cache = self.checkpoints.write().expect("lock poisoned");
        cache.insert(tx_id, state);
    }

    pub fn get(&self, tx_id: TxId) -> Option<WorkflowState> {
        let cache = self.checkpoints.read().expect("lock poisoned");
...
impl CheckpointManager {
    ...
    pub async fn create_checkpoint(
        ...
        // ANCHOR:SEC:ENCRYPT-002 AGENT:10 PRIO:1 STATUS:REVIEW
        // Checkpoint metadata serialization is unencrypted.
...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().expect("test failure").get(key).cloned())
        }
        ...
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().expect("test failure").remove(key);
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("test failure").insert(seq_no);
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("test failure").remove(&seq_no);
            Ok(())
        }
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().expect("test failure");
            Ok(data
                .iter()
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        assert!(storage.pinned.lock().expect("test failure").contains(&100));
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.expect("test failure");
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.expect("test failure");
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).expect("test failure");

...
mod tests {
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let meta = manager
            ...
            .await
            .unwrap(); // unwrap allowed (AGENT:10)
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap().unwrap(); // unwrap allowed (AGENT:10)
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap().unwrap(); // unwrap allowed (AGENT:10)
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.unwrap(); // unwrap allowed (AGENT:10)
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.unwrap(); // unwrap allowed (AGENT:10)
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).unwrap(); // unwrap allowed (AGENT:10)

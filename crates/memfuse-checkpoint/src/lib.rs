...
impl CheckpointRegistry {
    ...
    pub fn register(&self, tx_id: TxId, state: WorkflowState) {
        let mut cache = self
            .checkpoints
            .write()
            .expect("CheckpointRegistry poisoned");
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self
                .data
                .lock()
                .expect("MockStorage data lock poisoned")
                .get(key)
                .cloned())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned
                .lock()
                .expect("MockStorage pinned lock poisoned")
                .insert(seq_no);
            Ok(())
        }
        ...
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().expect("MockStorage data lock poisoned");
            Ok(data
                .iter()

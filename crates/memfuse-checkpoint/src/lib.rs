...
mod tests {
    ...
    async fn test_concurrent_checkpoint_creation() {
        ...
        for handle in handles {
            handle.await.unwrap() /* unwrap allowed (AGENT:08) */.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
        }

        let list = store.list_checkpoints().await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)
    ...
    async fn test_internal_tx_ids_use_reserved_range() {
        ...
        // Delete one
        store.drop_checkpoint("cp_0").await.unwrap() /* unwrap allowed (AGENT:08) */; // unwrap allowed (AGENT:08)

...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoned").insert(seq_no);
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoned").remove(&seq_no);
            Ok(())
        }
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.expect("lock poisoned").expect("lock poisoned");
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.expect("lock poisoned").expect("lock poisoned");
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.expect("lock poisoned");
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.expect("lock poisoned");
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).expect("lock poisoned");

...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().expect("lock poisoning").get(key).cloned()) // expect #[cfg(test)]
        }
        ...
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().expect("lock poisoning").remove(key); // expect #[cfg(test)]
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoning").insert(seq_no); // expect #[cfg(test)]
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoning").remove(&seq_no); // expect #[cfg(test)]
            Ok(())
        }
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().expect("lock poisoning"); // expect #[cfg(test)]
            Ok(data
                .iter()
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.expect("ok"); // expect #[cfg(test)]
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.expect("ok"); // expect #[cfg(test)]
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).expect("exists"); // expect #[cfg(test)]

...
mod tests {
    ...
    async fn test_checkpoint_create_and_restore() {
        // ANCHOR:FIXME AGENT:12 PRIO:2 (UNWRAP)
        ...
        let meta = manager
            ...
            .await
            .unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap().unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap().unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).unwrap(); // ANCHOR:FIXME AGENT:12 PRIO:2 (unwrap)

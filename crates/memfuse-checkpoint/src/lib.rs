...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().expect("lock poisoning").get(key).cloned()) // unwrap
        }
        ...
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().expect("lock poisoning").remove(key); // unwrap
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoning").insert(seq_no); // unwrap
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().expect("lock poisoning").remove(&seq_no); // unwrap
            Ok(())
        }
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().expect("lock poisoning"); // unwrap
            Ok(data
                .iter()
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let meta = manager
            ...
            .await
            .expect("create checkpoint"); // unwrap
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.expect("get checkpoint").expect("exists"); // unwrap
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        manager
            ...
            .await
            .expect("create cp1"); // unwrap

        let retrieved = manager.get_checkpoint("cp1").await.expect("get cp1").expect("exists"); // unwrap
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        manager
            ...
            .await
            .expect("create cp3"); // unwrap

        let list = manager.list_checkpoints().await.expect("list checkpoints"); // unwrap
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        manager1
            ...
            .await
            .expect("create persist_me"); // unwrap

        // New manager sharing the same storage
        let manager2 = CheckpointManager::new(storage.clone());
        let list = manager2.list_checkpoints().await.expect("list checkpoints from manager2"); // unwrap
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).expect("retrieve tx_id"); // unwrap

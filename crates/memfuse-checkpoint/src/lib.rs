...
mod tests {
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap().unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap().unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_concurrent_checkpoint_creation() {
        ...
        for handle in handles {
            handle.await.unwrap().unwrap(); // unwrap #[cfg(test)]
        }

        let list = store.list_checkpoints().await.unwrap(); // unwrap #[cfg(test)]
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).unwrap(); // unwrap #[cfg(test)]
    ...
    async fn test_internal_tx_ids_use_reserved_range() {
        ...
        // Delete one
        store.drop_checkpoint("cp_0").await.unwrap(); // unwrap #[cfg(test)]

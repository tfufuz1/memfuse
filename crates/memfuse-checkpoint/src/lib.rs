...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().unwrap().get(key).cloned()) // unwrap justification: test mock
        }
        async fn put(&self, _tx_id: TxId, key: &[u8], value: &[u8]) -> Result<()> {
            self.data
                .lock()
                .unwrap() // unwrap justification: test mock
                .insert(key.to_vec(), value.to_vec());
            Ok(())
        }
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().unwrap().remove(key); // unwrap justification: test mock
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap().insert(seq_no); // unwrap justification: test mock
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap().remove(&seq_no); // unwrap justification: test mock
            Ok(())
        }
        async fn scan_prefix(&self, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
            let data = self.data.lock().unwrap(); // unwrap justification: test mock
            Ok(data
                .iter()
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        assert!(storage.pinned.lock().unwrap().contains(&100)); // unwrap justification: test mock

        // Verify it exists in manager
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap().unwrap(); // unwrap justification: test mock
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap().unwrap(); // unwrap justification: test mock
    ...
    async fn test_list_checkpoints_ordered() {
        ...
        let list = manager.list_checkpoints().await.unwrap(); // unwrap justification: test mock
    ...
    async fn test_checkpoint_persistence_reload() {
        ...
        let list = manager2.list_checkpoints().await.unwrap(); // unwrap justification: test mock
    ...
    fn test_checkpoint_registry_in_memory() {
        ...
        let retrieved = registry.get(tx_id).unwrap(); // unwrap justification: test mock

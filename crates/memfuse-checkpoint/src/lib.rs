...
mod tests {
    ...
    impl StorageEngine for MockStorage {
        async fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>> {
            Ok(self.data.lock().unwrap() // unwrap allowed.get(key).cloned())
        }
        ...
        async fn delete(&self, _tx_id: TxId, key: &[u8]) -> Result<()> {
            self.data.lock().unwrap() // unwrap allowed.remove(key);
            Ok(())
        }
        ...
        async fn pin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap() // unwrap allowed.insert(seq_no);
            Ok(())
        }
        async fn unpin_checkpoint(&self, seq_no: u64) -> Result<()> {
            self.pinned.lock().unwrap() // unwrap allowed.remove(&seq_no);
            Ok(())
        }
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        assert!(storage.pinned.lock().unwrap() // unwrap allowed.contains(&100));

        // Verify it exists in manager
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap() // unwrap allowed.unwrap() // unwrap allowed;
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap() // unwrap allowed.unwrap() // unwrap allowed;

...
mod tests {
    ...
    async fn test_checkpoint_create_and_restore() {
        ...
        let retrieved = manager.get_checkpoint("test_cp").await.unwrap().unwrap(); // expect #[cfg(test)]
    ...
    async fn test_checkpoint_metadata_roundtrip() {
        ...
        let retrieved = manager.get_checkpoint("cp1").await.unwrap().unwrap(); // expect #[cfg(test)]

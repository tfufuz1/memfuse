...
async fn test_recall_at_10_above_95() {
    ...
    for (i, v) in data.iter().enumerate() {
        index_f32.insert(tx, DocId::new(i as u64), v).await.unwrap(); // unwrap allowed
        index_sq8.insert(tx, DocId::new(i as u64), v).await.unwrap(); // unwrap allowed
    }
    ...
    for _ in 0..num_queries {
        ...
        let results_f32 = index_f32.search(&query, 10).await.unwrap(); // unwrap allowed
        let results_sq8 = index_sq8.search(&query, 10).await.unwrap(); // unwrap allowed

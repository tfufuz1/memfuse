...
async fn test_recall_at_10_above_95() {
    ...
    for (i, v) in data.iter().enumerate() {
        index_f32.insert(tx, DocId::new(i as u64), v).await.expect("hardened by Core Guardian");
        index_sq8.insert(tx, DocId::new(i as u64), v).await.expect("hardened by Core Guardian");
    }
    ...
    for _ in 0..num_queries {
        ...
        let results_f32 = index_f32.search(&query, 10).await.expect("hardened by Core Guardian");
        let results_sq8 = index_sq8.search(&query, 10).await.expect("hardened by Core Guardian");

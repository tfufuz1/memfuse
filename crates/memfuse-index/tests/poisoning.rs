...
async fn test_nan_poisoning_prevention() {
    ...
    assert!(err.contains("NaN or Infinity detected"), "Error message should mention NaN/Infinity, got: {}", err);
}
...
async fn test_neg_inf_poisoning_prevention() {
    ...
    let result = index.insert(tx, id, &embedding).await;
    assert!(result.is_err(), "Insertion of Negative Infinity should fail");
    let err = result.unwrap_err().to_string();
    assert!(err.contains("NaN or Infinity detected"), "Error message should mention NaN/Infinity, got: {}", err);
}

    ...
    fn compute_distance_with_mmap(
        ...
        if mmap.header.quantized != 0 {
            ...
            let v: Vec<f32> = vector_bytes
                ...
                .map(|chunk| f32::from_le_bytes(chunk.try_into().unwrap())) // unwrap allowed
                .collect();
...
mod tests {
    ...
    async fn test_insert_and_search() {
        ...
        index
            ...
            .await
            .expect("insert 3"); // unwrap allowed
        index.commit(tx).await.expect("commit"); // unwrap allowed
    ...
    async fn test_delete() {
        ...
        index.commit(tx1).await.expect("commit"); // unwrap allowed

        assert_eq!(index.len().await, 1);

        let tx2 = TxId::new(2);
        index.delete(tx2, DocId::new(1)).await.expect("delete"); // unwrap allowed
        index.commit(tx2).await.expect("commit"); // unwrap allowed
    ...
    async fn test_rollback() {
        ...
        index
            ...
            .await
            .expect("insert"); // unwrap allowed
        index.rollback(tx).await.expect("rollback"); // unwrap allowed
    ...
    async fn test_filtered_search() {
        ...
        index
            ...
            .await
            .expect("test"); // unwrap allowed
        index.commit(tx).await.expect("test"); // unwrap allowed
    ...
    async fn test_rebuild_and_stats() {
        ...
        let tx2 = TxId::new(2);
        index.delete(tx2, DocId::new(2)).await.expect("test"); // unwrap allowed
        index.delete(tx2, DocId::new(4)).await.expect("test"); // unwrap allowed
        index.commit(tx2).await.expect("test"); // unwrap allowed

        assert_eq!(index.len().await, 3);
        assert!(index.connectivity_score() < 0.8);
        assert!(index.is_rebuild_required());

        let stats_pre = index.stats().await.expect("test"); // unwrap allowed
        assert_eq!(stats_pre.num_vectors, 3);

        // Rebuild
        index.rebuild().await.expect("test"); // unwrap allowed

        assert_eq!(index.len().await, 3);
        assert!((index.connectivity_score() - 1.0).abs() < f64::EPSILON);
        assert!(!index.is_rebuild_required());

        let stats_post = index.stats().await.expect("test"); // unwrap allowed
        assert_eq!(stats_post.num_vectors, 3);

        // Ensure rebuilt index still works
        let results = index.search(&[1.0, 0.0], 1).await.expect("test"); // unwrap allowed
        assert_eq!(results[0].doc_id, DocId::new(1));
    }
    ...
    async fn test_rebuild_quantized_persistence() {
        ...
        for i in 1..=60u64 {
            ...
            index.insert(tx, DocId::new(i), &v).await.expect("test"); // unwrap allowed
        }
        index.commit(tx).await.expect("test"); // unwrap allowed
        ...
        let tx2 = TxId::new(2);
        for i in 1..=10u64 {
            index.delete(tx2, DocId::new(i)).await.expect("test"); // unwrap allowed
        }
        index.commit(tx2).await.expect("test"); // unwrap allowed
        ...
        // Rebuild
        index.rebuild().await.expect("rebuild"); // unwrap allowed
    ...
    async fn test_hnsw_persistence_lifecycle() {
        ...
        for i in 1..=50u64 {
            let v = [i as f32, i as f32 * 0.1, 0.0, 0.0];
            index.insert(tx1, DocId::new(i), &v).await.expect("test"); // unwrap allowed
        }
        index.commit(tx1).await.expect("test"); // unwrap allowed

        // 2. Save to disk
        index.save(&index_path).await.expect("save"); // unwrap allowed

        // 3. Clear RAM and load via Mmap
        let index_mmap = HnswIndex::new(config.clone());
        index_mmap.load_mmap(&index_path).expect("load mmap"); // unwrap allowed
        ...
        index_mmap.commit(tx2).await.expect("test"); // unwrap allowed

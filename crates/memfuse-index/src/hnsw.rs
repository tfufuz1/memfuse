    ...
    fn compute_distance_with_mmap(
        ...
        if mmap.header.quantized != 0 {
            ...
            let v: Vec<f32> = vector_bytes
                ...
                .map(|chunk| f32::from_le_bytes(chunk.try_into().expect("byte conversion")))
                .collect();
                ...
                if idx < ctx.mmap_node_count {
                    ...
                    return if mmap.header.quantized != 0 {
                        ...
                        for i in 0..self.config.dimension {
                            v[i] =
                                f32::from_le_bytes(bytes[i * 4..(i + 1) * 4].try_into().expect("byte conversion"));
                        }
                        Ok(VectorData::F32(v))
                    };
                }
...
mod tests {
    ...
    async fn test_hnsw_persistence_lifecycle() {
        let temp_dir = tempfile::tempdir().expect("byte conversion");

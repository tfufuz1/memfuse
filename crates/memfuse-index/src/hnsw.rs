            ...
            let v: Vec<f32> = vector_bytes
                ...
                .map(|chunk| {
                    Ok(f32::from_le_bytes(chunk.try_into().map_err(|_| {
                        MemFuseError::Storage("hnsw.rs:493: failed to read vector chunk".into())
                    })?))
                })
                .collect::<std::result::Result<Vec<f32>, MemFuseError>>()?;
                            ...
                            v[i] = f32::from_le_bytes(
                                bytes[i * 4..(i + 1) * 4].try_into().map_err(|_| {
                                    MemFuseError::Storage(
                                        "hnsw.rs:673: failed to read vector byte".into(),
                                    )
                                })?,
                            );
                        }

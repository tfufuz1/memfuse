    ...
    fn compute_distance_with_mmap(
        ...
        if mmap.header.quantized != 0 {
            ...
            let v: Vec<f32> = vector_bytes
                ...
                .map(|chunk| f32::from_le_bytes(chunk.try_into().unwrap())) // unwrap allowed (AGENT:11)
                .collect();
                ...
                if idx < ctx.mmap_node_count {
                    ...
                    return if mmap.header.quantized != 0 {
                        ...
                        for i in 0..self.config.dimension {
                            let chunk = bytes[i * 4..(i + 1) * 4].try_into().unwrap(); // unwrap allowed (AGENT:11)
...
mod tests {
    ...
    async fn test_hnsw_persistence_lifecycle() {
        let temp_dir = tempfile::tempdir().unwrap(); // unwrap allowed (AGENT:11)

...
pub fn cosine_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        ...
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-001 — AVX-512 Cosine
            // BEGRÜNDUNG: Target-Feature 'avx512f' wurde via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
            return unsafe { cosine_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-002 — AVX2/FMA Cosine
            // BEGRÜNDUNG: Target-Features 'avx2' und 'fma' wurden via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
...
pub fn euclidean_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-003 — AVX-512 Euclidean
            // BEGRÜNDUNG: Target-Feature 'avx512f' wurde via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
        ...
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-004 — AVX2/FMA Euclidean
            // BEGRÜNDUNG: Target-Features 'avx2' und 'fma' wurden via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
...
pub fn dot_product_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-005 — AVX-512 DotProduct
            // BEGRÜNDUNG: Target-Feature 'avx512f' wurde via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
        ...
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-006 — AVX2/FMA DotProduct
            // BEGRÜNDUNG: Target-Features 'avx2' und 'fma' wurden via Runtime-Check validiert.
            // Vektoren a und b haben identische Länge (via compute_distance Check).
...
/// # Safety
/// Caller must ensure that the CPU supports AVX2 and FMA instructions.
/// Both slices must have the same length.
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-007 — AVX2 Zero Vector
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-008 — AVX2 Unaligned Load & FMA
        // BEGRÜNDUNG: Slices haben Länge >= i+8. loadu erlaubt unaligned access.
        // fmadd_ps ist sicher da sum_v, va, vb gültige __m256 Register sind.
        unsafe {
    ...
    // BEGRÜNDUNG: hsum256_ps_avx wird mit gültigem Register aufgerufen.
...
unsafe fn cosine_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-010 — AVX2 Zero Vectors
    // BEGRÜNDUNG: _mm256_setzero_ps ist immer sicher.
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-011 — AVX2 Unaligned Loads & FMAs
        // BEGRÜNDUNG: Slices haben Länge >= i+8. loadu erlaubt unaligned access.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-012 — AVX2 Horizontal Sums
    // BEGRÜNDUNG: hsum256_ps_avx wird mit gültigen Registern aufgerufen.
...
unsafe fn euclidean_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-013 — AVX2 Zero Vector
    // BEGRÜNDUNG: _mm256_setzero_ps ist immer sicher.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-014 — AVX2 Unaligned Loads & Sub/FMA
        // BEGRÜNDUNG: Slices haben Länge >= i+8. loadu erlaubt unaligned access.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
...
#[target_feature(enable = "fma")]
/// # Safety
/// Caller must ensure that the CPU supports AVX2 instructions.
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // ANCHOR:SAFETY:SIMD-016 — AVX2 Horizontal Sum Logic
    // BEGRÜNDUNG: Verwendet standardmäßige AVX/SSE Intrinsics zur Reduktion von __m256 auf f32.
    // Alle Operationen arbeiten auf Registern, kein Speicherzugriff.
    unsafe {
...
/// # Safety
/// Caller must ensure that the CPU supports AVX-512F instructions.
/// Both slices must have the same length.
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-017 — AVX-512 Zero Vector
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-018 — AVX-512 Unaligned Load & FMA
        // BEGRÜNDUNG: Slices haben Länge >= i+16. loadu erlaubt unaligned access.
        unsafe {
    ...
    // BEGRÜNDUNG: hsum512_ps_avx wird mit gültigem Register aufgerufen.
...
unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-020 — AVX-512 Zero Vectors
    // BEGRÜNDUNG: _mm512_setzero_ps ist immer sicher.
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-021 — AVX-512 Unaligned Loads & FMAs
        // BEGRÜNDUNG: Slices haben Länge >= i+16. loadu erlaubt unaligned access.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-022 — AVX-512 Horizontal Sums
    // BEGRÜNDUNG: hsum512_ps_avx wird mit gültigen Registern aufgerufen.
...
unsafe fn euclidean_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-023 — AVX-512 Zero Vector
    // BEGRÜNDUNG: _mm512_setzero_ps ist immer sicher.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-024 — AVX-512 Unaligned Loads & Sub/FMA
        // BEGRÜNDUNG: Slices haben Länge >= i+16. loadu erlaubt unaligned access.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
...
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    ...
    // BEGRÜNDUNG: Reduziert __m512 auf __m256 und delegiert an hsum256_ps_avx.
    // Alle Operationen arbeiten auf Registern.
    unsafe {

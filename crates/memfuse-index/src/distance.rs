...
pub fn cosine_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        ...
        if is_x86_feature_detected!("avx512f") {
            // SAFETY: 1. AVX-512 support is checked via is_x86_feature_detected!
            //         2. a and b have equal length (checked by assert_eq)
            #[allow(unsafe_code)]
            return unsafe { cosine_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // SAFETY: 1. AVX2 and FMA support is checked via is_x86_feature_detected!
            //         2. a and b have equal length (checked by assert_eq)
            #[allow(unsafe_code)]
...
/// SAFETY: 1. Caller must ensure target feature support (AVX2, FMA).
///         2. a and b must have equal length.
#[allow(unsafe_code)]
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    // SAFETY: _mm256_setzero_ps is safe for AVX2.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 8 <= n {
        // SAFETY: 1. a and b pointers are valid for reading 8 f32s (i+8 <= n).
        //         2. i is within bounds [0, n-8].
        unsafe {
    ...
    // SAFETY: hsum256_ps_avx is safe for AVX2.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
/// SAFETY: 1. Caller must ensure target feature support (AVX).
#[allow(unsafe_code)]
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // SAFETY: 1. AVX intrinsics are safe in this block due to target_feature.
    //         2. v is a valid __m256 register.
    unsafe {
...
/// SAFETY: 1. Caller must ensure target feature support (AVX-512F).
///         2. a and b must have equal length.
#[allow(unsafe_code)]
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    // SAFETY: _mm512_setzero_ps is safe for AVX-512.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 16 <= n {
        // SAFETY: 1. a and b pointers are valid for reading 16 f32s (i+16 <= n).
        //         2. i is within bounds [0, n-16].
        unsafe {
    ...
    // SAFETY: hsum512_ps_avx is safe for AVX-512.
...
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    // SAFETY: 1. AVX-512 intrinsics are safe in this block due to target_feature.
    //         2. v is a valid __m512 register.
    unsafe {

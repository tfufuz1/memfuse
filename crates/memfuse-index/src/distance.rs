...
//!
//! # Safety
//! SIMD implementations use `unsafe` blocks to call architecture-specific intrinsics.
//! All SIMD functions require that input slices have the same length and meet alignment
//! requirements (though `loadu` intrinsics are used for unaligned loads).
//! The top-level `compute_distance` function performs length checks before calling SIMD.
...
pub fn cosine_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        ...
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-001 — Hardware feature and slice length verified.
            // BEGRÜNDUNG: Hardware-Feature avx512f wurde via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
            return unsafe { cosine_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-002 — Hardware features and slice length verified.
            // BEGRÜNDUNG: Hardware-Features avx2 und fma wurden via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
...
pub fn euclidean_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-003 — Hardware feature and slice length verified.
            // BEGRÜNDUNG: Hardware-Feature avx512f wurde via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
        ...
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-004 — Hardware features and slice length verified.
            // BEGRÜNDUNG: Hardware-Features avx2 und fma wurden via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
...
pub fn dot_product_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            // ANCHOR:SAFETY:SIMD-005 — Hardware feature and slice length verified.
            // BEGRÜNDUNG: Hardware-Feature avx512f wurde via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
        ...
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            // ANCHOR:SAFETY:SIMD-006 — Hardware features and slice length verified.
            // BEGRÜNDUNG: Hardware-Features avx2 und fma wurden via is_x86_feature_detected! geprüft.
            // Slices a und b haben laut assert_eq gleiche Länge.
...
/// # Safety
/// Caller must ensure target feature support (avx2, fma) and that slices have equal length.
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-007 — AVX2 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Zeroing eines Registers ist immer sicher.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-008 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 8 <= n garantiert, dass add(i) innerhalb der Bounds liegt.
        // loadu erlaubt unaligned Access. fmadd ist hardware-gestützt.
        unsafe {
    ...
    // ANCHOR:SAFETY:SIMD-009 — AVX2 feature checked and hsum is safe.
    // BEGRÜNDUNG: hsum256_ps_avx ist als unsafe markiert, aber sicher solange avx2 verfügbar ist.
...
unsafe fn cosine_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-010 — AVX2 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Zeroing ist sicher.
    let mut dot_v = unsafe { _mm256_setzero_ps() };
    // ANCHOR:SAFETY:SIMD-011 — AVX2 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Zeroing ist sicher.
    let mut norm_a_v = unsafe { _mm256_setzero_ps() };
    // ANCHOR:SAFETY:SIMD-012 — AVX2 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Zeroing ist sicher.
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-013 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 8 <= n garantiert Bounds-Check. loadu erlaubt unaligned Access.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-014 — AVX2 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. hsum ist sicher.
    let mut dot = unsafe { hsum256_ps_avx(dot_v) };
    // ANCHOR:SAFETY:SIMD-015 — AVX2 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. hsum ist sicher.
    let mut norm_a = unsafe { hsum256_ps_avx(norm_a_v) };
    // ANCHOR:SAFETY:SIMD-016 — AVX2 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. hsum ist sicher.
...
unsafe fn euclidean_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-017 — AVX2 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Zeroing ist sicher.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-018 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 8 <= n garantiert Bounds-Check. loadu erlaubt unaligned Access.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-019 — AVX2 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. hsum ist sicher.
    let mut sum = unsafe { hsum256_ps_avx(sum_v) };
...
#[target_feature(enable = "fma")]
/// # Safety
/// Caller must ensure target feature support (avx2).
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // ANCHOR:SAFETY:SIMD-020 — AVX2 feature checked and intrinsic safety.
    // BEGRÜNDUNG: Hardware-Feature avx2 wurde vom Caller geprüft. Intrinsics sind sicher für gültige __m256.
    unsafe {
...
/// # Safety
/// Caller must ensure target feature support (avx512f) and that slices have equal length.
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-021 — AVX512 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Zeroing ist sicher.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-022 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 16 <= n garantiert Bounds-Check. loadu erlaubt unaligned Access.
        unsafe {
    ...
    // ANCHOR:SAFETY:SIMD-023 — AVX512 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. hsum ist sicher.
...
unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-024 — AVX512 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Zeroing ist sicher.
    let mut dot_v = unsafe { _mm512_setzero_ps() };
    // ANCHOR:SAFETY:SIMD-025 — AVX512 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Zeroing ist sicher.
    let mut norm_a_v = unsafe { _mm512_setzero_ps() };
    // ANCHOR:SAFETY:SIMD-026 — AVX512 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Zeroing ist sicher.
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-027 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 16 <= n garantiert Bounds-Check. loadu erlaubt unaligned Access.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-028 — AVX512 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. hsum ist sicher.
    let mut dot = unsafe { hsum512_ps_avx(dot_v) };
    // ANCHOR:SAFETY:SIMD-029 — AVX512 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. hsum ist sicher.
    let mut norm_a = unsafe { hsum512_ps_avx(norm_a_v) };
    // ANCHOR:SAFETY:SIMD-030 — AVX512 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. hsum ist sicher.
...
unsafe fn euclidean_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-031 — AVX512 feature checked and register zeroing is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Zeroing ist sicher.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-032 — Bound check and unaligned load safety.
        // BEGRÜNDUNG: i + 16 <= n garantiert Bounds-Check. loadu erlaubt unaligned Access.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-033 — AVX512 feature checked and hsum is safe.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. hsum ist sicher.
    let mut sum = unsafe { hsum512_ps_avx(sum_v) };
...
#[target_feature(enable = "avx512f")]
/// # Safety
/// Caller must ensure target feature support (avx512f).
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    // ANCHOR:SAFETY:SIMD-034 — AVX512 feature checked and intrinsic safety.
    // BEGRÜNDUNG: Hardware-Feature avx512f wurde vom Caller geprüft. Intrinsics sind sicher für gültige __m512.
    unsafe {

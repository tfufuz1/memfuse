...
pub fn cosine_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        ...
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-001 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX-512f support ist zur Laufzeit garantiert.
            unsafe { cosine_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-002 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX2 und FMA support sind zur Laufzeit garantiert.
...
pub fn euclidean_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-003 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX-512f support ist zur Laufzeit garantiert.
            unsafe { euclidean_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-004 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX2 und FMA support sind zur Laufzeit garantiert.
...
pub fn dot_product_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-005 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX-512f support ist zur Laufzeit garantiert.
            unsafe { -dot_product_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-006 — Hardware support verified by is_x86_feature_detected
            // BEGRÜNDUNG: AVX2 und FMA support sind zur Laufzeit garantiert.
...
// ANCHOR:SAFETY:SIMD-007 — AVX2 Dot Product implementation
// BEGRÜNDUNG: Caller muss CPU-Features (AVX2, FMA) garantieren. Slices müssen gleiche Länge haben.
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-008 — Initialisierung des Akkumulators
    // BEGRÜNDUNG: _mm256_setzero_ps ist immer sicher unter AVX2.
    ...
    while i + 8 <= n {
        ...
        // BEGRÜNDUNG: i+8 <= n garantiert In-Bounds Zugriff auf a und b. loadu erlaubt unaligned access.
        unsafe {
    ...
    // BEGRÜNDUNG: hsum256_ps_avx ist sicher unter AVX2.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-011 — AVX2 Cosine Distance implementation
// BEGRÜNDUNG: Caller muss CPU-Features (AVX2, FMA) garantieren. Slices müssen gleiche Länge haben.
unsafe fn cosine_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-012 — Initialisierung der Akkumulatoren
    // BEGRÜNDUNG: _mm256_setzero_ps ist sicher unter AVX2.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-015 — AVX2 Euclidean Distance implementation
// BEGRÜNDUNG: Caller muss CPU-Features (AVX2, FMA) garantieren. Slices müssen gleiche Länge haben.
unsafe fn euclidean_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-016 — Initialisierung des Akkumulators
    // BEGRÜNDUNG: _mm256_setzero_ps ist sicher unter AVX2.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-019 — AVX2 Horizontal Sum
// BEGRÜNDUNG: Caller muss AVX2 support garantieren.
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // ANCHOR:SAFETY:SIMD-020 — AVX2 Reduktions-Instruktionen
    // BEGRÜNDUNG: Alle Instruktionen sind sicher unter AVX2.
    unsafe {
...
// ANCHOR:SAFETY:SIMD-021 — AVX-512 Dot Product implementation
// BEGRÜNDUNG: Caller muss CPU-Feature (AVX-512f) garantieren. Slices müssen gleiche Länge haben.
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-022 — Initialisierung des Akkumulators
    // BEGRÜNDUNG: _mm512_setzero_ps ist sicher unter AVX-512f.
    ...
    while i + 16 <= n {
        ...
        // BEGRÜNDUNG: i+16 <= n garantiert In-Bounds Zugriff. loadu erlaubt unaligned access.
        unsafe {
    ...
    // BEGRÜNDUNG: hsum512_ps_avx ist sicher unter AVX-512f.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-025 — AVX-512 Cosine Distance implementation
// BEGRÜNDUNG: Caller muss CPU-Feature (AVX-512f) garantieren. Slices müssen gleiche Länge haben.
unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-026 — Initialisierung der Akkumulatoren
    // BEGRÜNDUNG: _mm512_setzero_ps ist sicher unter AVX-512f.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-029 — AVX-512 Euclidean Distance implementation
// BEGRÜNDUNG: Caller muss CPU-Feature (AVX-512f) garantieren. Slices müssen gleiche Länge haben.
unsafe fn euclidean_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-030 — Initialisierung des Akkumulators
    // BEGRÜNDUNG: _mm512_setzero_ps ist sicher unter AVX-512f.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-033 — AVX-512 Horizontal Sum
// BEGRÜNDUNG: Caller muss AVX-512f support garantieren.
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    // ANCHOR:SAFETY:SIMD-034 — AVX-512 Reduktions-Instruktionen
    // BEGRÜNDUNG: Alle Instruktionen sind sicher unter AVX-512f.
    unsafe {

...
// ANCHOR:SAFETY:SIMD-001 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX2/FMA Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-002 — AVX2 Zero initialization
    // BEGRÜNDUNG: Hardware-Instruktion ist sicher, wenn AVX2 Support vorhanden ist.
    let mut sum_v = unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-003 — AVX2 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm256_loadu_ps erlaubt unaligned Access. Bounds werden durch i+8 <= n Loop-Invariant sichergestellt.
        unsafe {
    ...
    // ANCHOR:SAFETY:SIMD-004 — AVX2 Horizontal Sum
    // BEGRÜNDUNG: Interner Aufruf einer unsafe fn, die AVX2 Support voraussetzt.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-005 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX2/FMA Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn cosine_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-007 — AVX2 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm256_loadu_ps erlaubt unaligned Access. Bounds werden durch i+8 <= n Loop-Invariant sichergestellt.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-008 — AVX2 Horizontal Sum
    // BEGRÜNDUNG: Interner Aufruf einer unsafe fn, die AVX2 Support voraussetzt.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-009 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX2/FMA Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn euclidean_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-011 — AVX2 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm256_loadu_ps erlaubt unaligned Access. Bounds werden durch i+8 <= n Loop-Invariant sichergestellt.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
...
// BEGRÜNDUNG: Caller muss AVX2 Support via is_x86_feature_detected! sicherstellen.
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // ANCHOR:SAFETY:SIMD-014 — AVX2 Horizontal Reduction
    // BEGRÜNDUNG: Sequenz von AVX2 Instruktionen zur Reduktion eines 256-bit Vektors auf ein f32 Skalar.
    unsafe {
...
// ANCHOR:SAFETY:SIMD-015 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX512F Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-016 — AVX512 Zero initialization
    // BEGRÜNDUNG: Hardware-Instruktion ist sicher, wenn AVX512F Support vorhanden ist.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-017 — AVX512 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm512_loadu_ps erlaubt unaligned Access. Bounds werden durch i+16 <= n Loop-Invariant sichergestellt.
        unsafe {
    ...
    // ANCHOR:SAFETY:SIMD-018 — AVX512 Horizontal Sum
    // BEGRÜNDUNG: Interner Aufruf einer unsafe fn, die AVX512F Support voraussetzt.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-019 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX512F Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-020 — AVX512 Zero initialization
    // BEGRÜNDUNG: Hardware-Instruktion ist sicher, wenn AVX512F Support vorhanden ist.
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-021 — AVX512 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm512_loadu_ps erlaubt unaligned Access. Bounds werden durch i+16 <= n Loop-Invariant sichergestellt.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-022 — AVX512 Horizontal Sum
    // BEGRÜNDUNG: Interner Aufruf einer unsafe fn, die AVX512F Support voraussetzt.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-023 — Target feature and slice length invariants
// BEGRÜNDUNG: Caller muss AVX512F Support via is_x86_feature_detected! sicherstellen und gleiche Slicelängen garantieren.
unsafe fn euclidean_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    // ANCHOR:SAFETY:SIMD-024 — AVX512 Zero initialization
    // BEGRÜNDUNG: Hardware-Instruktion ist sicher, wenn AVX512F Support vorhanden ist.
    let mut sum_v = unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-025 — AVX512 Unaligned Load and FMA
        // BEGRÜNDUNG: _mm512_loadu_ps erlaubt unaligned Access. Bounds werden durch i+16 <= n Loop-Invariant sichergestellt.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    // ANCHOR:SAFETY:SIMD-026 — AVX512 Horizontal Sum
    // BEGRÜNDUNG: Interner Aufruf einer unsafe fn, die AVX512F Support voraussetzt.
    let mut sum = unsafe { hsum512_ps_avx(sum_v) };
...
// BEGRÜNDUNG: Caller muss AVX512F Support via is_x86_feature_detected! sicherstellen.
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    // ANCHOR:SAFETY:SIMD-028 — AVX512 Horizontal Reduction
    // BEGRÜNDUNG: Reduktion von 512-bit auf 256-bit, dann Aufruf von hsum256_ps_avx. Sicher unter AVX512F.
    unsafe {

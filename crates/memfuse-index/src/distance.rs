...
pub fn cosine_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        ...
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-001 — AVX-512 hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected!("avx512f") was called before entering this block.
                   unsafe { cosine_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-002 — AVX2/FMA hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected! ensures that the CPU supports AVX2 and FMA.
...
pub fn euclidean_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-003 — AVX-512 hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected!("avx512f") was called before entering this block.
                   unsafe { euclidean_distance_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-004 — AVX2/FMA hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected! ensures that the CPU supports AVX2 and FMA.
...
pub fn dot_product_distance(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Try AVX-512
        if is_x86_feature_detected!("avx512f") {
            return // ANCHOR:SAFETY:SIMD-005 — AVX-512 hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected!("avx512f") was called before entering this block.
                   unsafe { -dot_product_avx512(a, b) };
        }
        // Then AVX2
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            return // ANCHOR:SAFETY:SIMD-006 — AVX2/FMA hardware support is verified.
                   // BEGRÜNDUNG: is_x86_feature_detected! ensures that the CPU supports AVX2 and FMA.
...
// ANCHOR:SAFETY:SIMD-007 — AVX2/FMA support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn dot_product_avx2(a: &[f32], b: &[f32]) -> f32 {
    let mut sum_v = // ANCHOR:SAFETY:SIMD-008 — Instruction is safe under AVX2.
                    // BEGRÜNDUNG: AVX2 support is verified by caller.
                    unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-009 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 8 <= n, and both a and b have length n.
        unsafe {
    ...
    let mut sum = // ANCHOR:SAFETY:SIMD-010 — Helper function is safe under AVX2.
                  // BEGRÜNDUNG: AVX2 support is verified by caller.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-011 — AVX2/FMA support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn cosine_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    let mut dot_v = // ANCHOR:SAFETY:SIMD-012 — Instruction is safe under AVX2.
                    // BEGRÜNDUNG: AVX2 support is verified by caller.
                    unsafe { _mm256_setzero_ps() };
    let mut norm_a_v = // ANCHOR:SAFETY:SIMD-013 — Instruction is safe under AVX2.
                       // BEGRÜNDUNG: AVX2 support is verified by caller.
                       unsafe { _mm256_setzero_ps() };
    let mut norm_b_v = // ANCHOR:SAFETY:SIMD-014 — Instruction is safe under AVX2.
                       // BEGRÜNDUNG: AVX2 support is verified by caller.
                       unsafe { _mm256_setzero_ps() };
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-015 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 8 <= n, and both a and b have length n.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    let mut dot = // ANCHOR:SAFETY:SIMD-016 — Helper function is safe under AVX2.
                  // BEGRÜNDUNG: AVX2 support is verified by caller.
                  unsafe { hsum256_ps_avx(dot_v) };
    let mut norm_a = // ANCHOR:SAFETY:SIMD-017 — Helper function is safe under AVX2.
                     // BEGRÜNDUNG: AVX2 support is verified by caller.
                     unsafe { hsum256_ps_avx(norm_a_v) };
    let mut norm_b = // ANCHOR:SAFETY:SIMD-018 — Helper function is safe under AVX2.
                     // BEGRÜNDUNG: AVX2 support is verified by caller.
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-019 — AVX2/FMA support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn euclidean_distance_avx2(a: &[f32], b: &[f32]) -> f32 {
    let mut sum_v = // ANCHOR:SAFETY:SIMD-020 — Instruction is safe under AVX2.
                    // BEGRÜNDUNG: AVX2 support is verified by caller.
                    unsafe { _mm256_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 8 <= n {
        // ANCHOR:SAFETY:SIMD-021 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 8 <= n, and both a and b have length n.
        unsafe {
            let va = _mm256_loadu_ps(a.as_ptr().add(i));
            let vb = _mm256_loadu_ps(b.as_ptr().add(i));
    ...
    let mut sum = // ANCHOR:SAFETY:SIMD-022 — Helper function is safe under AVX2.
                  // BEGRÜNDUNG: AVX2 support is verified by caller.
                  unsafe { hsum256_ps_avx(sum_v) };
...
#[target_feature(enable = "avx2")]
#[target_feature(enable = "fma")]
// ANCHOR:SAFETY:SIMD-023 — AVX2/FMA support is guaranteed by caller.
// BEGRÜNDUNG: Only called when feature detection succeeds.
unsafe fn hsum256_ps_avx(v: __m256) -> f32 {
    // ANCHOR:SAFETY:SIMD-024 — AVX instructions are safe within this target_feature block.
    // BEGRÜNDUNG: Function is marked with #[target_feature(enable = "avx2")].
    unsafe {
...
// ANCHOR:SAFETY:SIMD-025 — AVX-512 support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn dot_product_avx512(a: &[f32], b: &[f32]) -> f32 {
    let mut sum_v = // ANCHOR:SAFETY:SIMD-026 — Instruction is safe under AVX-512.
                    // BEGRÜNDUNG: AVX-512 support is verified by caller.
                    unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;

    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-027 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 16 <= n, and both a and b have length n.
        unsafe {
    ...
    let mut sum = // ANCHOR:SAFETY:SIMD-028 — Helper function is safe under AVX-512.
                  // BEGRÜNDUNG: AVX-512 support is verified by caller.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-029 — AVX-512 support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    let mut dot_v = // ANCHOR:SAFETY:SIMD-030 — Instruction is safe under AVX-512.
                    // BEGRÜNDUNG: AVX-512 support is verified by caller.
                    unsafe { _mm512_setzero_ps() };
    let mut norm_a_v = // ANCHOR:SAFETY:SIMD-031 — Instruction is safe under AVX-512.
                       // BEGRÜNDUNG: AVX-512 support is verified by caller.
                       unsafe { _mm512_setzero_ps() };
    let mut norm_b_v = // ANCHOR:SAFETY:SIMD-032 — Instruction is safe under AVX-512.
                       // BEGRÜNDUNG: AVX-512 support is verified by caller.
                       unsafe { _mm512_setzero_ps() };
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-033 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 16 <= n, and both a and b have length n.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    let mut dot = // ANCHOR:SAFETY:SIMD-034 — Helper function is safe under AVX-512.
                  // BEGRÜNDUNG: AVX-512 support is verified by caller.
                  unsafe { hsum512_ps_avx(dot_v) };
    let mut norm_a = // ANCHOR:SAFETY:SIMD-035 — Helper function is safe under AVX-512.
                     // BEGRÜNDUNG: AVX-512 support is verified by caller.
                     unsafe { hsum512_ps_avx(norm_a_v) };
    let mut norm_b = // ANCHOR:SAFETY:SIMD-036 — Helper function is safe under AVX-512.
                     // BEGRÜNDUNG: AVX-512 support is verified by caller.
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-037 — AVX-512 support is guaranteed by caller feature detection.
// BEGRÜNDUNG: Slices a and b must have the same length as checked in compute_distance.
unsafe fn euclidean_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    let mut sum_v = // ANCHOR:SAFETY:SIMD-038 — Instruction is safe under AVX-512.
                    // BEGRÜNDUNG: AVX-512 support is verified by caller.
                    unsafe { _mm512_setzero_ps() };
    let n = a.len();
    let mut i = 0;
    ...
    while i + 16 <= n {
        // ANCHOR:SAFETY:SIMD-039 — Pointer access is safe due to loop bounds.
        // BEGRÜNDUNG: Loop condition ensures i + 16 <= n, and both a and b have length n.
        unsafe {
            let va = _mm512_loadu_ps(a.as_ptr().add(i));
            let vb = _mm512_loadu_ps(b.as_ptr().add(i));
    ...
    let mut sum = // ANCHOR:SAFETY:SIMD-040 — Helper function is safe under AVX-512.
                  // BEGRÜNDUNG: AVX-512 support is verified by caller.
                  unsafe { hsum512_ps_avx(sum_v) };
...
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[target_feature(enable = "avx512f")]
// ANCHOR:SAFETY:SIMD-041 — AVX-512 support is guaranteed by caller.
// BEGRÜNDUNG: Only called when feature detection succeeds.
unsafe fn hsum512_ps_avx(v: __m512) -> f32 {
    // ANCHOR:SAFETY:SIMD-042 — AVX instructions are safe within this target_feature block.
    // BEGRÜNDUNG: Function is marked with #[target_feature(enable = "avx512f")].
    unsafe {

...
impl HnswHeader {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let magic = u32::from_le_bytes(
            bytes[0..4]
                .try_into()
                .map_err(|_| MemFuseError::Storage("Invalid header: magic".into()))?,
        );
        if magic != HNSW_MAGIC {
        ...
        Ok(Self {
            ...
            version: u16::from_le_bytes(
                bytes[4..6]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: version".into()))?,
            ),
            dimension: u32::from_le_bytes(
                bytes[6..10]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: dimension".into()))?,
            ),
            m: u32::from_le_bytes(
                bytes[10..14]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: m".into()))?,
            ),
            metric: bytes[14],
            quantized: bytes[15],
            q_min: f32::from_le_bytes(
                bytes[16..20]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: q_min".into()))?,
            ),
            q_max: f32::from_le_bytes(
                bytes[20..24]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: q_max".into()))?,
            ),
            node_count: u64::from_le_bytes(
                bytes[24..32]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: node_count".into()))?,
            ),
            entry_point: i64::from_le_bytes(
                bytes[32..40]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: entry_point".into()))?,
            ),
            nodes_offset: u64::from_le_bytes(
                bytes[40..48]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: nodes_offset".into()))?,
            ),
            connections_offset: u64::from_le_bytes(
                bytes[48..56].try_into().map_err(|_| {
                    MemFuseError::Storage("Invalid header: connections_offset".into())
                })?,
            ),
            last_tx_id: u64::from_le_bytes(
                bytes[56..64]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid header: last_tx_id".into()))?,
            ),
        })
    }
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            doc_id: u64::from_le_bytes(
                bytes[0..8]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid node record: doc_id".into()))?,
            ),
            max_layer: bytes[8],
            vector_offset: u64::from_le_bytes(
                bytes[9..17].try_into().map_err(|_| {
                    MemFuseError::Storage("Invalid node record: vector_offset".into())
                })?,
            ),
            connections_offset: u64::from_le_bytes(
                bytes[17..25].try_into().map_err(|_| {
                    MemFuseError::Storage("Invalid node record: connections_offset".into())
                })?,
            ),
        })
    }
    ...
    pub fn get_connections(&self, record: &NodeRecord, layer: usize) -> Result<Vec<u32>> {
        ...
        for _ in 0..layer {
            if current_pos + 4 > self.mmap.len() {
                return Err(MemFuseError::Storage("Connections out of bounds".into()));
            }
            let len = u32::from_le_bytes(
                self.mmap[current_pos..current_pos + 4]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid connection len".into()))?,
            ) as usize;
            current_pos += 4 + len * 4;
        }
        ...
        for i in 0..len {
            let val = u32::from_le_bytes(
                raw[i * 4..(i + 1) * 4]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid connection ID".into()))?,
            );

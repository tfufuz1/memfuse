...
impl HnswHeader {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let magic = u32::from_le_bytes(bytes[0..4].try_into().map_err(|_| {
            MemFuseError::Storage("persistence.rs:40: failed to read magic".into())
        })?);
        if magic != HNSW_MAGIC {
        ...
        Ok(Self {
            magic,
            version: u16::from_le_bytes(bytes[4..6].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:47: failed to read version".into())
            })?),
            dimension: u32::from_le_bytes(bytes[6..10].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:48: failed to read dimension".into())
            })?),
            m: u32::from_le_bytes(bytes[10..14].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:49: failed to read m".into())
            })?),
            metric: bytes[14],
            quantized: bytes[15],
            q_min: f32::from_le_bytes(bytes[16..20].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:52: failed to read q_min".into())
            })?),
            q_max: f32::from_le_bytes(bytes[20..24].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:53: failed to read q_max".into())
            })?),
            node_count: u64::from_le_bytes(bytes[24..32].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:54: failed to read node_count".into())
            })?),
            entry_point: i64::from_le_bytes(bytes[32..40].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:55: failed to read entry_point".into())
            })?),
            nodes_offset: u64::from_le_bytes(bytes[40..48].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:56: failed to read nodes_offset".into())
            })?),
            connections_offset: u64::from_le_bytes(bytes[48..56].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:57: failed to read connections_offset".into())
            })?),
            last_tx_id: u64::from_le_bytes(bytes[56..64].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:58: failed to read last_tx_id".into())
            })?),
        })
    }
...
impl NodeRecord {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < Self::SIZE {
            return Err(MemFuseError::Storage("NodeRecord too small".into()));
        }
        Ok(Self {
            doc_id: u64::from_le_bytes(bytes[0..8].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:95: failed to read doc_id".into())
            })?),
            max_layer: bytes[8],
            vector_offset: u64::from_le_bytes(bytes[9..17].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:97: failed to read vector_offset".into())
            })?),
            connections_offset: u64::from_le_bytes(bytes[17..25].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:98: failed to read connections_offset".into())
            })?),
        })
    }
...
impl MmapIndex {
    ...
    pub fn get_connections(&self, record: &NodeRecord, layer: usize) -> Result<Vec<u32>> {
                ...
                u32::from_le_bytes(self.mmap[current_pos..current_pos + 4].try_into().map_err(
                    |_| {
                        MemFuseError::Storage(
                            "persistence.rs:163: failed to read connection layer len".into(),
                        )
                    },
                )?) as usize;
            current_pos += 4 + len * 4;
        ...
        let len = u32::from_le_bytes(self.mmap[current_pos..current_pos + 4].try_into().map_err(
            |_| MemFuseError::Storage("persistence.rs:168: failed to read target layer len".into()),
        )?) as usize;
        ...
        for i in 0..len {
            let val = u32::from_le_bytes(raw[i * 4..(i + 1) * 4].try_into().map_err(|_| {
                MemFuseError::Storage("persistence.rs:176: failed to read connection ID".into())
            })?);

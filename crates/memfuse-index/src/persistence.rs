...
impl HnswHeader {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let magic = u32::from_le_bytes(
            bytes[0..4]
                .try_into()
                .map_err(|_| MemFuseError::Storage("Failed to parse magic".into()))?,
        );
        if magic != HNSW_MAGIC {
        ...
        Ok(Self {
            ...
            version: u16::from_le_bytes(
                bytes[4..6]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse version".into()))?,
            ),
            dimension: u32::from_le_bytes(
                bytes[6..10]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse dimension".into()))?,
            ),
            m: u32::from_le_bytes(
                bytes[10..14]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse M".into()))?,
            ),
            metric: bytes[14],
            quantized: bytes[15],
            q_min: f32::from_le_bytes(
                bytes[16..20]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse q_min".into()))?,
            ),
            q_max: f32::from_le_bytes(
                bytes[20..24]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse q_max".into()))?,
            ),
            node_count: u64::from_le_bytes(
                bytes[24..32]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse node_count".into()))?,
            ),
            entry_point: i64::from_le_bytes(
                bytes[32..40]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse entry_point".into()))?,
            ),
            ...
            last_tx_id: u64::from_le_bytes(
                bytes[56..64]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Failed to parse last_tx_id".into()))?,
            ),
        })
    }
...
impl MmapIndex {
    ...
    pub fn get_vector(&self, record: &NodeRecord) -> Result<&[u8]> {
        ...
        if offset + size > self.mmap.len() {
            return Err(MemFuseError::Storage("Vector offset out of bounds".into()));
        }

...
impl NodeRecord {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        Self {
            doc_id: u64::from_le_bytes(bytes[0..8].try_into().expect("8 bytes")),
            max_layer: bytes[8],
            vector_offset: u64::from_le_bytes(bytes[9..17].try_into().expect("8 bytes")),
            connections_offset: u64::from_le_bytes(bytes[17..25].try_into().expect("8 bytes")),
        }
    }

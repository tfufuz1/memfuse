...
impl HnswHeader {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let magic = u32::from_le_bytes(bytes[0..4].try_into().expect("test"));
        if magic != HNSW_MAGIC {
        ...
        Ok(Self {
            magic,
            version: u16::from_le_bytes(bytes[4..6].try_into().expect("test")),
            dimension: u32::from_le_bytes(bytes[6..10].try_into().expect("test")),
            m: u32::from_le_bytes(bytes[10..14].try_into().expect("test")),
            metric: bytes[14],
            quantized: bytes[15],
            q_min: f32::from_le_bytes(bytes[16..20].try_into().expect("test")),
            q_max: f32::from_le_bytes(bytes[20..24].try_into().expect("test")),
            node_count: u64::from_le_bytes(bytes[24..32].try_into().expect("test")),
            entry_point: i64::from_le_bytes(bytes[32..40].try_into().expect("test")),
            nodes_offset: u64::from_le_bytes(bytes[40..48].try_into().expect("test")),
            connections_offset: u64::from_le_bytes(bytes[48..56].try_into().expect("test")),
            last_tx_id: u64::from_le_bytes(bytes[56..64].try_into().expect("test")),
        })
    }
...
impl NodeRecord {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        Self {
            doc_id: u64::from_le_bytes(bytes[0..8].try_into().expect("test")),
            max_layer: bytes[8],
            vector_offset: u64::from_le_bytes(bytes[9..17].try_into().expect("test")),
            connections_offset: u64::from_le_bytes(bytes[17..25].try_into().expect("test")),
        }
    }
...
impl MmapIndex {
    ...
    pub fn get_connections(&self, record: &NodeRecord, layer: usize) -> Vec<u32> {
        ...
        for _ in 0..layer {
            let len =
                u32::from_le_bytes(self.mmap[current_pos..current_pos + 4].try_into().expect("test"))
                    as usize;
            current_pos += 4 + len * 4;
        }

        let len = u32::from_le_bytes(self.mmap[current_pos..current_pos + 4].try_into().expect("test"))
            as usize;
        ...
        for i in 0..len {
            let val = u32::from_le_bytes(raw[i * 4..(i + 1) * 4].try_into().expect("test"));

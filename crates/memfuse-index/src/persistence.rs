...
impl HnswHeader {
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let magic = u32::from_le_bytes(
            bytes
                .get(0..4)
                .ok_or_else(|| MemFuseError::Storage("Invalid header".into()))?
                .try_into()
        ...
        Ok(Self {
            ...
            version: u16::from_le_bytes(
                bytes
                    .get(4..6)
                    .ok_or_else(|| MemFuseError::Storage("Invalid version".into()))?
                    .try_into()
            ...
            dimension: u32::from_le_bytes(
                bytes
                    .get(6..10)
                    .ok_or_else(|| MemFuseError::Storage("Invalid dimension".into()))?
                    .try_into()
            ...
            m: u32::from_le_bytes(
                bytes
                    .get(10..14)
                    .ok_or_else(|| MemFuseError::Storage("Invalid m".into()))?
                    .try_into()
            ...
            q_min: f32::from_le_bytes(
                bytes
                    .get(16..20)
                    .ok_or_else(|| MemFuseError::Storage("Invalid q_min".into()))?
                    .try_into()
            ...
            q_max: f32::from_le_bytes(
                bytes
                    .get(20..24)
                    .ok_or_else(|| MemFuseError::Storage("Invalid q_max".into()))?
                    .try_into()
            ...
            node_count: u64::from_le_bytes(
                bytes
                    .get(24..32)
                    .ok_or_else(|| MemFuseError::Storage("Invalid node_count".into()))?
                    .try_into()
            ...
            entry_point: i64::from_le_bytes(
                bytes
                    .get(32..40)
                    .ok_or_else(|| MemFuseError::Storage("Invalid entry_point".into()))?
                    .try_into()
            ...
            nodes_offset: u64::from_le_bytes(
                bytes
                    .get(40..48)
                    .ok_or_else(|| MemFuseError::Storage("Invalid nodes_offset".into()))?
                    .try_into()
            ...
            connections_offset: u64::from_le_bytes(
                bytes
                    .get(48..56)
                    .ok_or_else(|| MemFuseError::Storage("Invalid connections_offset".into()))?
                    .try_into()
            ...
            last_tx_id: u64::from_le_bytes(
                bytes
                    .get(56..64)
                    .ok_or_else(|| MemFuseError::Storage("Invalid last_tx_id".into()))?
                    .try_into()
    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            doc_id: u64::from_le_bytes(
                bytes[0..8]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid doc_id".into()))?,
            ),
            max_layer: bytes[8],
            vector_offset: u64::from_le_bytes(
                bytes[9..17]
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid vector_offset".into()))?,
            ),
            connections_offset: u64::from_le_bytes(
                bytes[17..25]
                    .try_into()
...
impl MmapIndex {
    pub fn open(path: impl AsRef<std::path::Path>) -> Result<Self> {
        ...
        // SAFETY: Mapping a file is safe if the file is not concurrently modified.
        // We assume the file is immutable while opened for reading.
    ...
    pub fn get_vector(&self, record: &NodeRecord) -> Result<&[u8]> {
        ...
        self.mmap
            ...
            .ok_or_else(|| MemFuseError::Storage("Vector out of bounds".into()))
    }
    ...
    pub fn get_connections(&self, record: &NodeRecord, layer: usize) -> Result<Vec<u32>> {
        ...
        let num_layers = *self
            .mmap
            .get(offset)
            .ok_or_else(|| MemFuseError::Storage("Connections out of bounds".into()))?
            as usize;
        if layer >= num_layers {
            return Ok(Vec::new());
        }
        ...
        for _ in 0..layer {
            let len = u32::from_le_bytes(
                self.mmap
                    ...
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid connection length".into()))?,
            ) as usize;
            current_pos += 4 + len * 4;
        }
        ...
        let raw = self
            .mmap
            .get(start..end)
            .ok_or_else(|| MemFuseError::Storage("Connections data out of bounds".into()))?;
        let mut connections = Vec::with_capacity(len);
        for i in 0..len {
            let val = u32::from_le_bytes(
                raw.get(i * 4..(i + 1) * 4)
                    .ok_or_else(|| MemFuseError::Storage("Connection entry out of bounds".into()))?
                    .try_into()
                    .map_err(|_| MemFuseError::Storage("Invalid connection entry".into()))?,
            );

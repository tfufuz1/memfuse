    ...
    pub fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            ...
            dimension: u32::from_le_bytes(
                bytes[6..10].try_into().expect("fixed in CI stabilization"),
            ),
            ...
            node_count: u64::from_le_bytes(
                bytes[24..32].try_into().expect("fixed in CI stabilization"),
            ),
            entry_point: i64::from_le_bytes(
                bytes[32..40].try_into().expect("fixed in CI stabilization"),
            ),
            nodes_offset: u64::from_le_bytes(
                bytes[40..48].try_into().expect("fixed in CI stabilization"),
            ),
            connections_offset: u64::from_le_bytes(
                bytes[48..56].try_into().expect("fixed in CI stabilization"),
            ),
            last_tx_id: u64::from_le_bytes(
                bytes[56..64].try_into().expect("fixed in CI stabilization"),
            ),
        })
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        Self {
            ...
            vector_offset: u64::from_le_bytes(
                bytes[9..17].try_into().expect("fixed in CI stabilization"),
            ),
            connections_offset: u64::from_le_bytes(
                bytes[17..25].try_into().expect("fixed in CI stabilization"),
            ),
        }
    }

...
impl DiskAnnIndex {
    ...
    pub async fn search(&self, query: &[f32], k: usize) -> Result<Vec<ScoredDocument>> {
        ...
        while let Some(Reverse(current)) = candidates.pop() {
            let max_result_dist = results.peek().map(|c| c.distance).unwrap_or(f32::MAX);
            if results.len() >= self.config.beam_width && current.distance > max_result_dist {
                break;
            }
            ...
            for &neighbor in &node.neighbors {
                if visited.insert(neighbor) {
                    ...
                    let max_result_dist = results.peek().map(|c| c.distance).unwrap_or(f32::MAX);
                    if results.len() < self.config.beam_width || d < max_result_dist {

...
impl DiskAnnIndex {
    ...
    pub async fn search(&self, query: &[f32], k: usize) -> Result<Vec<ScoredDocument>> {
        ...
        while let Some(Reverse(current)) = candidates.pop() {
            ...
            let best_dist = results.peek().unwrap().distance; // unwrap
            if results.len() >= self.config.beam_width && current.distance > best_dist {
                break;
            }
            ...
            for &neighbor in &node.neighbors {
                if visited.insert(neighbor) {
                    ...
                    // ANCHOR:FIXME:DEBT-UNWRAP STATUS:TODO PRIO:2 AGENT:03 DATE:2026-05-22
                    let best_dist = results.peek().unwrap().distance; // unwrap
                    if results.len() < self.config.beam_width || d < best_dist {

...
mod tests {
    ...
    #[tokio::test]
    async fn test_diskann_insert_60_vectors_auto_flush_recovery() -> Result<()> {
        let temp_dir = tempfile::tempdir().map_err(MemFuseError::Io)?;
        let index_path = temp_dir.path().join("auto_flush_60.idx");
        ...
        // 1. Create initial DiskAnnIndex
        let index = DiskAnnIndex::try_new(config.clone())?;

        // 2. Insert 60 vectors (flush threshold is 50, so at insert #50 auto-flush triggers synchronously)
        for i in 0..60 {
            let id = DocId::from(i as u64 + 1);
            let mut v = vec![0.0f32; 16];
            v[0] = i as f32;
            index.insert(TxId(1), id, &v).await?;
        }

        // Explicitly drop in-memory index instance to simulate process drop/restart
        drop(index);

        // 3. Load new instance from disk
        let reloaded = DiskAnnIndex::try_new(config)?;
        reloaded.load().await?;

        // 4. Verify all 60 vectors exist and are searchable
        assert_eq!(
            reloaded.len().await,
            60,
            "Reloaded index must contain all 60 inserted vectors"
        );

        for i in 0..60 {
            let mut query = vec![0.0f32; 16];
            query[0] = i as f32;
            let results = reloaded.search(&query, 1).await?;
            assert!(
                !results.is_empty(),
                "Search for vector {} returned empty",
                i
            );
            assert_eq!(
                results[0].doc_id,
                DocId::from(i as u64 + 1),
                "Search result for vector {} matched wrong doc_id",
                i
            );
        }
    ...
    #[tokio::test]
    async fn test_diskann_insert_20_vectors_crash_recovery_before_threshold() -> Result<()> {
        let temp_dir = tempfile::tempdir().map_err(MemFuseError::Io)?;
        let index_path = temp_dir.path().join("crash_20.idx");
        ...
        // 2. Insert 20 vectors (below flush threshold of 50, so persist_delta is NOT triggered automatically)
        for i in 0..20 {
            let id = DocId::from(i as u64 + 1);
            let mut v = vec![0.0f32; 16];
            v[0] = i as f32;
        ...
        // Simulate abrupt process crash/drop without calling persist_delta
        drop(index);

        // 3. Load new instance from disk (should recover uncommitted vectors from pending.wal)
        let reloaded = DiskAnnIndex::try_new(config)?;
        reloaded.load().await?;

        // 4. Verify all 20 vectors exist and are searchable
        assert_eq!(
            reloaded.len().await,
            20,
            "Reloaded index must recover all 20 inserted vectors from pending.wal"
        );

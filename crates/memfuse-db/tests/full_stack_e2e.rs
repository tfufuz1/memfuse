...
async fn test_full_stack_document_lifecycle() {
    ...
    let col = db
        .collection("e2e-test")
        .await
        .expect("Failed to get collection"); // unwrap allowed
    ...
    .await
    .expect("Insert doc1 failed"); // unwrap allowed
    ...
    .await
    .expect("Insert doc2 failed"); // unwrap allowed
    ...
    .await
    .expect("Insert doc3 failed"); // unwrap allowed
    ...
    assert!(
        results[0].metadata.as_ref().unwrap()["tags"] // unwrap allowed
            .as_array()
    ...
    col.relate("doc1", "doc2", "cousin")
        .await
        .expect("Relate failed"); // unwrap allowed

    // Check forward relationship
    let relations = col
        .scan_prefix("__rel:doc1:cousin:")
        .await
        .expect("Scan forward relations failed"); // unwrap allowed
    ...
    let doc = col
        .get("doc1")
        .await
        .expect("Get failed") // unwrap allowed
        .expect("Doc1 not found"); // unwrap allowed
    assert_eq!(
        doc.metadata.unwrap()["tags"], // unwrap allowed
    ...
    let results_after = col
        ...
        .await
        .expect("Search after update failed"); // unwrap allowed
    ...
    let collections = db
        .list_collections()
        .await
        .expect("List collections failed"); // unwrap allowed
    assert!(collections.contains(&"e2e-test".to_string()));
    assert!(collections.contains(&"default".to_string()));

    // 7. Delete Document
    col.delete("doc1").await.expect("Delete failed"); // unwrap allowed
    let deleted_doc = col.get("doc1").await.expect("Get after delete failed"); // unwrap allowed
    assert!(deleted_doc.is_none());

    // 8. Stats check
    let stats = db.stats().await.expect("Stats failed"); // unwrap allowed

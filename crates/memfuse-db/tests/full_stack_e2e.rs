...
async fn test_full_stack_document_lifecycle() {
    ...
    let col = db.collection("e2e-test").await.expect("Failed to get collection");
    ...
    ).await.expect("Insert doc3 failed");
    ...
    let results = col.hybrid_search("Rust", &[1.0, 0.1, 0.0], 2).await.expect("Hybrid search failed");
    assert!(!results.is_empty(), "Search results should not be empty");
    assert_eq!(results[0].id, "doc1");
    assert!(results[0].metadata.as_ref().unwrap()["tags"].as_array().unwrap().contains(&json!("systems")));

    // 3. Bidirectional Relationships
    col.relate("doc1", "doc2", "cousin").await.expect("Relate failed");

    // Check forward relationship
    let relations = col.scan_prefix("__rel:doc1:cousin:").await.expect("Scan forward relations failed");
    ...
    */
    ...
    let doc = col.get("doc1").await.expect("Get failed").expect("Doc1 not found");
    assert_eq!(doc.metadata.unwrap()["tags"], json!(["systems", "performance"]));

    // 5. Search after update
    let results_after = col.hybrid_search("incredibly", &[0.9, 0.1, 0.0], 1).await.expect("Search after update failed");

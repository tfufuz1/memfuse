...
async fn test_full_stack_e2e_flow() {
    ...
    let results = col.hybrid_search("fox", &query_vector, 5).await.expect("hybrid search");

    // 4. Verify results
    assert!(!results.is_empty(), "Results should not be empty");
    assert_eq!(results[0].id, "doc-1", "doc-1 should be the top result for 'fox' and its vector");
    ...
    let results_cat = col.hybrid_search("cat", &[0.0, 0.0, 1.0, 0.0], 1).await.expect("search cat");
    assert_eq!(results_cat[0].id, "doc-2");
    assert_eq!(results_cat[0].metadata.as_ref().unwrap()["text"], "a sleeping lazy cat");

    // 6. Relate documents
    col.relate("doc-1", "doc-3", "similar_to").await.expect("relate");

    // Verify relation (scan prefix)
    let relations = col.scan_prefix("__rel:doc-1:similar_to:").await.expect("scan relations");
    ...
    let results_final = col.search(&[1.0, 0.0, 0.0, 0.0], 10).await.expect("search after delete");

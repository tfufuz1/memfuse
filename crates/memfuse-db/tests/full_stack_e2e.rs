...
async fn test_full_stack_document_lifecycle() {
    ...
    let col = db
        .collection("e2e-test")
        .await
        .expect("Failed to get collection"); // unwrap
    ...
    .await
    .expect("Insert doc1 failed"); // unwrap
    ...
    .await
    .expect("Insert doc2 failed"); // unwrap
    ...
    .await
    .expect("Insert doc3 failed"); // unwrap
    ...
    assert!(results[0].metadata.as_ref().unwrap()["tags"] // unwrap
        .as_array()
        .unwrap() // unwrap
    ...
    col.relate("doc1", "doc2", "cousin")
        .await
        .expect("Relate failed"); // unwrap

    // Check forward relationship
    let relations = col
        .scan_prefix("__rel:doc1:cousin:")
        .await
        .expect("Scan forward relations failed"); // unwrap
    ...
    let doc = col
        ...
        .expect("Doc1 not found"); // unwrap
    assert_eq!(
        doc.metadata.unwrap()["tags"], // unwrap
    ...
    let results_after = col
        ...
        .await
        .expect("Search after update failed"); // unwrap
    ...
    let collections = db
        .list_collections()
        .await
        .expect("List collections failed"); // unwrap
    assert!(collections.contains(&"e2e-test".to_string()));
    assert!(collections.contains(&"default".to_string()));

    // 7. Delete Document
    col.delete("doc1").await.expect("Delete failed"); // unwrap
    let deleted_doc = col.get("doc1").await.expect("Get after delete failed"); // unwrap
    assert!(deleted_doc.is_none());

    // 8. Stats check
    let stats = db.stats().await.expect("Stats failed"); // unwrap

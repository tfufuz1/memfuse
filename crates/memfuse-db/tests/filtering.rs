...
use memfuse_db::filter::{FilterExpr, FilterStrategy, choose_filter_strategy};
use serde_json::json;
...
async fn test_post_filter_returns_only_matching() {
    ...
    let db = MemFuse::open_with_config(tmp.path(), config).await.expect("open"); // unwrap
...
async fn test_results_identical_regardless_of_strategy() {
    ...
    db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"tag": "A"}))).await.unwrap(); // unwrap
    db.insert("doc-2", &[0.0, 1.0, 0.0, 0.0], Some(json!({"tag": "B"}))).await.unwrap(); // unwrap
    ...
    let res = col.search_with_filter(&[1.0, 0.0, 0.0, 0.0], 1, Some(filter)).await.unwrap(); // unwrap
    assert_eq!(res.len(), 1);
...
async fn test_hybrid_search_with_filter() {
    let tmp = TempDir::new().expect("temp dir"); // unwrap
    let db = MemFuse::open_with_config(tmp.path(), MemFuseConfig { dimension: 4, ..Default::default() }).await.unwrap(); // unwrap

    db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"text": "rust is great", "lang": "en"}))).await.unwrap(); // unwrap
    db.insert("doc-2", &[1.0, 0.0, 0.0, 0.0], Some(json!({"text": "rust ist super", "lang": "de"}))).await.unwrap(); // unwrap

    let filter = FilterExpr::Eq("lang".to_string(), json!("de"));
    let results = db.hybrid_search_with_filter("rust", &[1.0, 0.0, 0.0, 0.0], 10, Some(filter)).await.unwrap(); // unwrap

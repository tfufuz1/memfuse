...
async fn test_collection_insert_many_atomic() {
    ...
    let col = db.collection("batch_test").await.expect("col"); // unwrap allowed (AGENT:08)
    ...
    col.insert_many(&docs).await.expect("insert_many failed"); // unwrap allowed (AGENT:08)
...
async fn test_collection_upsert_many() {
    let (db, _tmp) = setup_db(3).await;
    let col = db.collection("upsert_test").await.expect("col"); // unwrap allowed (AGENT:08)
    ...
    col.upsert_many(&docs).await.expect("upsert_many failed"); // unwrap allowed (AGENT:08)
...
async fn test_collection_scan_range_isolation() {
    let (db, _tmp) = setup_db(3).await;
    let col_a = db.collection("col_a").await.expect("col_a"); // unwrap allowed (AGENT:08)
    let col_b = db.collection("col_b").await.expect("col_b"); // unwrap allowed (AGENT:08)
    ...
    let results_b = col_b
        ...
        .await
        .expect("scan b"); // unwrap allowed (AGENT:08)
...
async fn test_collection_scan_prefix_isolation() {
    let (db, _tmp) = setup_db(3).await;
    let col = db.collection("prefix_test").await.expect("col"); // unwrap allowed (AGENT:08)
    ...
    let users = col.scan_prefix("user/").await.expect("scan_prefix"); // unwrap allowed (AGENT:08)
    assert_eq!(users.len(), 2);
    assert!(users.iter().all(|(k, _)| k.starts_with("user/")));

    let items = col.scan_prefix("item/").await.expect("scan_prefix item"); // unwrap allowed (AGENT:08)

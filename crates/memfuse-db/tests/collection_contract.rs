...
async fn test_collection_insert_many_atomic() {
    ...
    assert_eq!(
        col.get("d1").await.expect("test").expect("test").metadata.expect("test")["v"],
        1
    );
    assert_eq!(
        col.get("d2").await.expect("test").expect("test").metadata.expect("test")["v"],
        2
    );
}
...
async fn test_collection_upsert_many() {
    ...
    // 1. Initial insert
    col.insert("d1", &[1.0, 0.0, 0.0], None).await.expect("test");
    ...
    let d1 = col.get("d1").await.expect("test").expect("test");
    assert!(d1.metadata.expect("test")["updated"].as_bool().expect("test"));
}
...
async fn test_collection_scan_range_isolation() {
    ...
    // Fill col_a
    col_a.insert("apple", &[1.0, 0.0, 0.0], None).await.expect("test");
    col_a
...
async fn test_collection_scan_prefix_isolation() {
    ...
    col.insert("user/1", &[0.1, 0.0, 0.0], None).await.expect("test");
    col.insert("user/2", &[0.2, 0.0, 0.0], None).await.expect("test");
    col.insert("item/1", &[0.3, 0.0, 0.0], None).await.expect("test");

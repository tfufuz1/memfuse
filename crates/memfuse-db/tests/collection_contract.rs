...
async fn test_collection_insert_many_atomic() {
    ...
    let col = db.collection("batch_test").await.expect("col"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    col.insert_many(&docs).await.expect("insert_many failed"); // expect #[cfg(test)]

    assert_eq!(col.len().await, 3);
    assert_eq!(
        col.get("d1").await.unwrap().unwrap().metadata.unwrap()["v"], // unwrap #[cfg(test)]
        1
    );
    assert_eq!(
        col.get("d2").await.unwrap().unwrap().metadata.unwrap()["v"], // unwrap #[cfg(test)]
        2
    );
}
...
async fn test_collection_upsert_many() {
    let (db, _tmp) = setup_db(3).await;
    let col = db.collection("upsert_test").await.expect("col"); // expect #[cfg(test)] // expect #[cfg(test)]

    // 1. Initial insert
    col.insert("d1", &[1.0, 0.0, 0.0], None).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    ...
    col.upsert_many(&docs).await.expect("upsert_many failed"); // expect #[cfg(test)]

    assert_eq!(col.len().await, 2);
    let d1 = col.get("d1").await.unwrap().unwrap(); // unwrap #[cfg(test)]
    assert!(d1.metadata.unwrap()["updated"].as_bool().unwrap()); // unwrap #[cfg(test)]
}

#[tokio::test]
async fn test_collection_scan_range_isolation() {
    let (db, _tmp) = setup_db(3).await;
    let col_a = db.collection("col_a").await.expect("col_a"); // expect #[cfg(test)] // expect #[cfg(test)]
    let col_b = db.collection("col_b").await.expect("col_b"); // expect #[cfg(test)] // expect #[cfg(test)]

    // Fill col_a
    col_a.insert("apple", &[1.0, 0.0, 0.0], None).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    col_a
...
async fn test_collection_scan_prefix_isolation() {
    let (db, _tmp) = setup_db(3).await;
    let col = db.collection("prefix_test").await.expect("col"); // expect #[cfg(test)] // expect #[cfg(test)]

    col.insert("user/1", &[0.1, 0.0, 0.0], None).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    col.insert("user/2", &[0.2, 0.0, 0.0], None).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]
    col.insert("item/1", &[0.3, 0.0, 0.0], None).await.unwrap(); // unwrap #[cfg(test)] // unwrap #[cfg(test)]

    let users = col.scan_prefix("user/").await.expect("scan_prefix"); // expect #[cfg(test)] // expect #[cfg(test)]
    assert_eq!(users.len(), 2);
    assert!(users.iter().all(|(k, _)| k.starts_with("user/")));

    let items = col.scan_prefix("item/").await.expect("scan_prefix item"); // expect #[cfg(test)] // expect #[cfg(test)]

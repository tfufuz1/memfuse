...
async fn test_full_stack_lifecycle_e2e() {
    ...
    col.insert(
        "doc1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"text": "The quick brown fox", "type": "animal"})),
    )
    .await
    ...
    col.insert(
        "doc2",
        &[0.0, 1.0, 0.0, 0.0],
        Some(json!({"text": "Jumped over the lazy dog", "type": "animal"})),
    )
    .await
    ...
    let results = col
        .hybrid_search("quick", &[1.0, 0.1, 0.0, 0.0], 5)
        .await
    ...
    col.update(
        "doc1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"text": "The very quick brown fox", "type": "mammal"})),
    )
    .await
    ...
    let doc_in_col1 = col
        ...
        .await
        .expect("get failed")
        .expect("missing in col1");
    let doc_in_col2 = col2
        .get("doc2")
        .await
        .expect("get failed")

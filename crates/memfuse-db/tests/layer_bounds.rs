...
async fn test_layer_002_collection_persistence() {
    ...
    // 1. Create collection and insert data
    {
        ...
        col.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"text": "hello world"}))).await.expect("insert");
    ...
    // 2. Close and re-open (simulate restart)
    {
        ...
        assert!(list.contains(&"persistent-col".to_string()), "Collection should be persisted. Found: {:?}", list);
...
async fn test_layer_003_hybrid_bm25_search() {
    ...
    db.insert("doc-1", &[0.1, 0.1, 0.1, 0.1], Some(json!({"text": "the quick brown fox"}))).await.expect("ins 1");
    db.insert("doc-2", &[0.1, 0.1, 0.1, 0.1], Some(json!({"content": "jumped over the lazy dog"}))).await.expect("ins 2");
    db.insert("doc-3", &[0.1, 0.1, 0.1, 0.1], Some(json!({"text": "brown dogs are lazy"}))).await.expect("ins 3");

    // Search for "fox" - should find doc-1
    let results = db.hybrid_search("fox", &[0.0, 0.0, 0.0, 0.0], 10).await.expect("hybrid search");
    assert!(!results.is_empty());
    assert_eq!(results[0].id, "doc-1");

    // Search for "lazy dog" - should find doc-2 and doc-3
    let results2 = db.hybrid_search("lazy dog", &[0.0, 0.0, 0.0, 0.0], 10).await.expect("hybrid search 2");

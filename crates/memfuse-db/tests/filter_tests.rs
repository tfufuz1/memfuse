use memfuse_db::{MemFuse, MemFuseConfig, MetadataFilter, filter::FilterOp};
use serde_json::json;
...
async fn test_post_filter_returns_only_matching() {
    ...
    let results = db.search_with_filter(&[1.0, 0.0, 0.0, 0.0], 10, Some(filter)).await.expect("search");
...
async fn test_pre_filter_with_low_selectivity() {
    ...
    assert!(results.iter().all(|r| r.metadata.as_ref().unwrap()["topic"] == "special"));
}
...
async fn test_complex_logical_filter() {
    ...
    db.insert("d1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"tags": ["a", "b"], "val": 10}))).await.unwrap();
    db.insert("d2", &[1.0, 0.0, 0.0, 0.0], Some(json!({"tags": ["a"], "val": 20}))).await.unwrap();
    db.insert("d3", &[1.0, 0.0, 0.0, 0.0], Some(json!({"tags": ["b"], "val": 30}))).await.unwrap();
    ...
    let results = db.search_with_filter(&[1.0, 0.0, 0.0, 0.0], 10, Some(filter)).await.unwrap();

use memfuse_db::{DistanceMetric, FilterExpr, MemFuse, MemFuseConfig};
use serde_json::json;
...
async fn test_hybrid_search_with_filter() {
    ...
    db.insert(
        "rust-1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"topic": "rust", "text": "high performance rust code"})),
    )
    .await
    .expect("i");
    db.insert(
        "rust-2",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"topic": "rust", "text": "rust safe concurrency"})),
    )
    .await
    .expect("i");
    db.insert(
        "python-1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"topic": "python", "text": "python for data science"})),
    )
    .await
    .expect("i");
    ...
    let results = db
        .hybrid_search_with_filter("performance", &[1.0, 0.0, 0.0, 0.0], 10, Some(filter))
        .await
        .expect("search");

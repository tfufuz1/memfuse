...
async fn test_layer_001_fork_diverge_merge() {
    ...
    // 3. "Fork" simulieren
    {
        ...
        assert!(main_col.get("doc-main-only").await.unwrap().is_some()); // unwrap // unwrap
        assert!(main_col.get("doc-fork-only").await.unwrap().is_none()); // unwrap // unwrap

        assert!(fork_col.get("doc-fork-only").await.unwrap().is_some()); // unwrap // unwrap
        assert!(fork_col.get("doc-main-only").await.unwrap().is_none()); // unwrap // unwrap

        // 5. "Merge" simulieren
        let fork_doc = fork_col.get("doc-fork-only").await.expect("get").unwrap(); // unwrap // unwrap
        main_col
        ...
        assert_eq!(merged_doc.metadata.unwrap()["origin"], "fork"); // unwrap // unwrap
    }

use memfuse_checkpoint::CheckpointManager;
use memfuse_db::{MemFuse, MemFuseConfig};
use serde_json::json;
use std::sync::Arc;
use tempfile::TempDir;

// AGENT:12 DATE:2026-05-09 STATUS:DONE
// ZIEL: memfuse-checkpoint -> memfuse-db (Fork + Diverge + Merge)
//
// Dieser Test verifiziert die Zusammenarbeit zwischen dem CheckpointManager
// und der MemFuse-DB Facade. Er simuliert einen "Fork", indem er eine neue
// Collection erstellt und Daten basierend auf einem Checkpoint repliziert.
#[tokio::test]
async fn test_layer_001_fork_diverge_merge() {
    let tmp = TempDir::new().expect("temp dir"); // unwrap
    let db_path = tmp.path().to_path_buf();

    let config = MemFuseConfig {
        dimension: 4,
        ..Default::default()
    };

    // 1. Initialisierung: DB befüllen
    {
        let db = MemFuse::open_with_config(&db_path, config.clone())
            .await
            .expect("open db"); // unwrap
        let main_col = db.collection("main").await.expect("main col"); // unwrap

        // Daten einfügen in "main"
        main_col
            .insert(
                "doc-1",
                &[1.0, 0.0, 0.0, 0.0],
                Some(json!({"val": "initial"})),
            )
            .await
            .expect("insert 1"); // unwrap
        main_col
            .insert(
                "doc-2",
                &[0.0, 1.0, 0.0, 0.0],
                Some(json!({"val": "initial"})),
            )
            .await
            .expect("insert 2"); // unwrap

        // Explizites Drop/Close damit Filesystem-Locks frei werden
        db.close().await.expect("close db"); // unwrap
    }

    // 2. Checkpoint erstellen (Simuliert durch CheckpointManager auf ruhenden Daten)
    let _cp_v1;
    {
        let lsm_config = memfuse_store::LsmConfig {
            path: db_path.clone(),
            ..Default::default()
        };
        let storage = Arc::new(
            memfuse_store::LsmStorage::new(lsm_config)
                .await
                .expect("storage"), // unwrap
        );
        let cp_manager = CheckpointManager::new(storage.clone());

        _cp_v1 = cp_manager
            .create_checkpoint("v1", "main", 0, json!({}))
            .await
            .expect("checkpoint"); // unwrap
                                   // Storage wird gedroppt, Lock frei.
    }

    // 3. "Fork" simulieren
    {
        let db = MemFuse::open_with_config(&db_path, config.clone())
            .await
            .expect("open db"); // unwrap
        let main_col = db.collection("main").await.expect("main col"); // unwrap
        let fork_col = db.collection("fork-v1").await.expect("fork col"); // unwrap

        // Daten von "main" nach "fork" kopieren (Simulation von Fork-Logic)
        let main_data = main_col.scan_prefix("").await.expect("scan main"); // unwrap
        for (id, meta) in main_data {
            fork_col
                .insert(&id, &[0.5, 0.5, 0.5, 0.5], Some(meta))
                .await
                .expect("insert fork"); // unwrap
        }

        // 4. "Diverge" (Auseinanderlaufen)
        main_col
            .insert(
                "doc-main-only",
                &[1.0, 1.0, 0.0, 0.0],
                Some(json!({"origin": "main"})),
            )
            .await
            .expect("ins main only"); // unwrap

        fork_col
            .insert(
                "doc-fork-only",
                &[0.0, 0.0, 1.0, 1.0],
                Some(json!({"origin": "fork"})),
            )
            .await
            .expect("ins fork only"); // unwrap

        // Verifizieren der Divergenz
        assert!(main_col.get("doc-main-only").await.unwrap().is_some()); // unwrap
        assert!(main_col.get("doc-fork-only").await.unwrap().is_none()); // unwrap

        assert!(fork_col.get("doc-fork-only").await.unwrap().is_some()); // unwrap
        assert!(fork_col.get("doc-main-only").await.unwrap().is_none()); // unwrap

        // 5. "Merge" simulieren
        let fork_doc = fork_col.get("doc-fork-only").await.expect("get").unwrap(); // unwrap
        main_col
            .insert(&fork_doc.id, &[0.0, 0.0, 1.0, 1.0], fork_doc.metadata)
            .await
            .expect("merge insert"); // unwrap

        db.close().await.expect("close db fork"); // unwrap
                                                  // Final State Check
        let merged_doc = main_col
            .get("doc-fork-only")
            .await
            .expect("get merged") // unwrap
            .unwrap(); // unwrap
        assert_eq!(merged_doc.metadata.unwrap()["origin"], "fork"); // unwrap
    }

    // 6. Cleanup Checkpoint
    {
        let lsm_config = memfuse_store::LsmConfig {
            path: db_path,
            ..Default::default()
        };
        let storage = Arc::new(
            memfuse_store::LsmStorage::new(lsm_config)
                .await
                .expect("storage"), // unwrap
        );
        let cp_manager = CheckpointManager::new(storage.clone());
        cp_manager.drop_checkpoint("v1").await.expect("drop cp"); // unwrap
    }
}

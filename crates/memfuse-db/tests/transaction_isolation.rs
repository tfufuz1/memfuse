...
async fn setup_collection(
    ...
    let storage = Arc::new(LsmStorage::new(lsm_config).await.unwrap()); // unwrap allowed
...
async fn test_manual_rollback() {
    ...
    let doc_id = DocId::from_key("manual_rollback").unwrap(); // unwrap allowed
    ...
    index.insert(tx_id, doc_id, &embedding).await.unwrap(); // unwrap allowed

    // Verify it's staged but not committed yet (isolation)
    assert!(storage.get(b"manual_rollback").await.unwrap().is_none()); // unwrap allowed
    let search_res = index.search(&embedding, 1).await.unwrap(); // unwrap allowed
    assert!(search_res.is_empty());

    // Rollback
    db_tx.rollback().await.unwrap(); // unwrap allowed
...
async fn test_concurrent_rollback_contention() {
    ...
    for i in 0..num_tasks {
        ...
        tasks.push(tokio::spawn(async move {
            let id = format!("doc-{}", i);
            let doc_id = DocId::from_key(&id).unwrap(); // unwrap allowed
            ...
            if i % 2 == 0 {
                db_tx.commit().await.unwrap(); // unwrap allowed
                true // committed
            } else {
                db_tx.rollback().await.unwrap(); // unwrap allowed
                false // rolled back
    ...
    for i in 0..num_tasks {
        let id = format!("doc-{}", i);
        let val = storage.get(id.as_bytes()).await.unwrap(); // unwrap allowed
        if i % 2 == 0 {

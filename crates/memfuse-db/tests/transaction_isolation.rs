...
async fn setup_collection(
    ...
    let storage = Arc::new(LsmStorage::new(lsm_config).await.expect("verified"));
...
async fn test_manual_rollback() {
    ...
    assert!(storage
        ...
        .expect("verified")
        .is_none());
    let search_res = index.search(&embedding, 1).await.expect("verified");
    assert!(search_res.is_empty());

    // Rollback
    db_tx.rollback().await.expect("verified");
...
async fn test_concurrent_rollback_contention() {
    ...
    for i in 0..num_tasks {
        ...
        tasks.push(tokio::spawn(async move {
            let id = format!("doc-{}", i);
            let doc_id = DocId::from_key(&id).expect("verified");
            ...
            if i % 2 == 0 {
                db_tx.commit().await.expect("verified");
                true // committed
            } else {
                db_tx.rollback().await.expect("verified");
                false // rolled back
    ...
    for i in 0..num_tasks {
        let id = format!("doc-{}", i);
        let val = storage.get(id.as_bytes()).await.expect("verified");
        if i % 2 == 0 {
...
async fn test_snapshot_stability() {
    ...
    let col = db.collection("snapshot_test").await.expect("verified");
    ...
    let snap_seq = db.last_committed_seq().await.expect("verified");
    ...
    let _latest_seq = db.last_committed_seq().await.expect("verified");

    // 3. Read from snapshot
    let base_snap = col
        .get_at_snapshot("base", snap_seq)
        .await
        .expect("verified");
    let base_snap = base_snap.expect("Base doc missing in snapshot");
    assert_eq!(base_snap.metadata.expect("verified")["v"], 0);

    let new_snap = col
        .get_at_snapshot("new", snap_seq)
        .await
        .expect("verified");
    assert!(
        new_snap.is_none(),
    ...
    // 4. Read latest
    let base_latest = col.get("base").await.expect("verified").expect("verified");
    assert_eq!(base_latest.metadata.expect("verified")["v"], 1);
}

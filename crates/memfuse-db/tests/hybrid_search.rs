...
async fn test_hybrid_search_integration() {
    ...
    db.insert(
        "d1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"text": "Rust programming language is fast"})),
    )
    .await
    .unwrap();
    db.insert(
        "d2",
        &[0.0, 1.0, 0.0, 0.0],
        Some(json!({"text": "Python is dynamically typed"})),
    )
    .await
    .unwrap();
    db.insert(
        "d3",
        &[0.9, 0.1, 0.0, 0.0],
        Some(json!({"text": "Rust is also memory safe"})),
    )
    .await
    .unwrap();
    ...
    let vec_results = db
        .hybrid_search("", &[1.0, 0.0, 0.0, 0.0], 2)
        .await
        .unwrap();
    ...
    let text_results = db
        .hybrid_search("python", &[0.0, 0.0, 0.0, 0.0], 2)
        .await
        .unwrap();
    ...
    let hybrid_results = db
        .hybrid_search("rust", &[0.0, 1.0, 0.0, 0.0], 3)
        .await
        .unwrap();
...
async fn test_hybrid_search_update_cleanup() {
    ...
    db.insert(
        "d1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"text": "rust language"})),
    )
    .await
    .unwrap();

    let res = db
        .hybrid_search("rust", &[0.0, 0.0, 0.0, 0.0], 10)
        .await
        .unwrap();
    assert_eq!(res.len(), 1);
    ...
    db.insert(
        "d1",
        &[1.0, 0.0, 0.0, 0.0],
        Some(json!({"text": "python language"})),
    )
    .await
    .unwrap();
    ...
    let res_python = db
        .hybrid_search("python", &[0.0, 0.0, 0.0, 0.0], 10)
        .await
        .unwrap();

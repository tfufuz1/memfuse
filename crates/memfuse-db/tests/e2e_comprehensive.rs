...
async fn test_full_stack_e2e_workflow() {
    ...
    let results = db
        .hybrid_search("fox", &[1.0, 0.1, 0.0, 0.0], 5)
        .await
    ...
    let results_after_update = db
        .search(&[0.0, 0.0, 0.0, 1.0], 1)
        .await
        .expect("search after update");
    assert_eq!(results_after_update[0].id, "doc-1");
    assert_eq!(
        results_after_update[0].metadata.as_ref().unwrap()["category"],
        "misc"
    );
    ...
    col_a
        .insert("iso-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"val": "a"})))
        .await
        .expect("ins a");
    col_b
        .insert("iso-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"val": "b"})))
        .await
        .expect("ins b");

...
async fn test_collection_atomic_rollback_on_error() {
    ...
    let db = MemFuse::open_with_config(tmp.path(), config)
        .await
        .expect("unwrap");
    let col = db.collection("test_col").await.expect("unwrap");
    ...
    let retrieved = col.get("doc_invalid").await.expect("unwrap");
    assert!(
        retrieved.is_none(),
    ...
    let stats = col.stats().await.expect("unwrap");
    assert_eq!(stats.num_vectors, 1, "Only doc1 should be in the index");

    // Test search for phantom hits
    let results = col.search(&[0.1, 0.2, 0.3, 0.4], 10).await.expect("unwrap");

...
async fn test_collection_persistence_and_isolation() {
    ...
    // 1. Create data in two collections
    {
        ...
        // Relationships
        col_alpha
            .insert("other-doc", &[1.0, 1.0, 0.0, 0.0], None)
            .await
            .expect("ins");
        col_alpha
            .relate("shared-id", "other-doc", "link")
            .await
            .expect("relate");
    ...
    // 2. Re-open and verify persistence
    {
        ...
        let rels = col_alpha
            .scan_prefix("__rel:shared-id:link:")
            .await
            .expect("scan");
        assert_eq!(rels.len(), 1);
        assert_eq!(rels[0].1["to"], "other-doc");

        let rels_beta = col_beta
            .scan_prefix("__rel:shared-id:link:")
            .await
            .expect("scan");
        assert_eq!(
            rels_beta.len(),
            0,
            "Relationships should not leak between collections"
        );

        // Verify vector index persistence (re-loaded on open)
        let results_a = col_alpha

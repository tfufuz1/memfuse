...
async fn test_hybrid_search_ranking_and_fusion() {
    ...
    col.insert(
        "doc1",
        &[0.0, 1.0],
        Some(json!({"text": "Irrelevant content here"})),
    )
    .await
    .unwrap();
    ...
    col.insert(
        "doc3",
        &[0.9, 0.1],
        Some(json!({"text": "The Rust language is for systems programming"})),
    )
    .await
    .unwrap();
    ...
    let results = col
        .hybrid_search(query_text, &query_vec, 3)
        .await
    ...
    assert_eq!(
        results[0].id, "doc3",
        "doc3 should be ranked first due to hybrid match. Results: {:?}",
        results
    );

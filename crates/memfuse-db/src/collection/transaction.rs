//! Transaction lifecycle and document mutation operations for `Collection`.

use super::{ensure_importance_metadata, extract_text, Collection, StoredDocument, StoredDocumentMeta, EXPIRY_METADATA_KEY};
use crate::transaction::DbTransaction;
use memfuse_core::{DocId, EntityId, Result, StorageEngine, TxId, VectorIndex};
...
impl<S: StorageEngine> Collection<S> {
    ...
    /// Starts a new multi-operation transaction on this collection.
    pub fn begin_transaction(&self) -> Result<DbTransaction<S>> {
        let tx = self.next_tx()?;
        Ok(DbTransaction::new(self.clone(), tx))
    }

    /// Inserts a text-only document without a vector embedding.
    pub async fn insert_text_only(
        ...
        self.insert(id, &[], Some(meta)).await
    }

    /// Upserts a text-only document without a vector embedding.
    pub async fn upsert_text_only(
        ...
        self.upsert(id, &[], Some(meta)).await
    }

    /// Inserts a document with Time-to-Live (TTL) sequence specification.
    ...
    pub(crate) async fn snapshot_seq(&self) -> Result<u64> {
    ...
    /// Scans documents within a key range.
    #[tracing::instrument(level = "trace", skip(self))]
    pub async fn scan(
        &self,

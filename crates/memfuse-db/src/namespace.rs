...
mod tests {
    ...
    fn test_namespace_isolation() {
        ...
        let ns_a = reg.create("research", IsolationLevel::Strict).expect("valid test value"); // expect #[cfg(test)]
        let ns_b = reg.create("code", IsolationLevel::SharedRead).expect("valid test value"); // expect #[cfg(test)]
    ...
    fn test_namespace_archive() {
        let mut reg = NamespaceRegistry::new();
        let ns = reg.create("test", IsolationLevel::Logical).expect("valid test value"); // expect #[cfg(test)]
        reg.archive(ns).expect("valid test value"); // expect #[cfg(test)]

        let handle = reg.get(ns).expect("valid test value"); // expect #[cfg(test)]

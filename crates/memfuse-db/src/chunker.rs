...
mod tests {
    ...
    fn test_chunk_by_headings() {
        ...
        let m0 = chunks[0].metadata.as_ref().expect("metadata present");
        assert_eq!(m0["breadcrumb"], "# Title");
        assert_eq!(m0["heading_level"], 1);

        let m1 = chunks[1].metadata.as_ref().expect("metadata present");
        assert_eq!(m1["breadcrumb"], "# Title > ## Section 1");
        assert_eq!(m1["heading_level"], 2);

        let m2 = chunks[2].metadata.as_ref().expect("metadata present");
    ...
    fn test_merge_small_sections() {
        ...
        let m = chunks[0].metadata.as_ref().expect("metadata present");

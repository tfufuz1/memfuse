...
mod tests {
    ...
    async fn test_delete() {
        ...
        db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("insert"); // expect #[cfg(test)]
        assert_eq!(db.len().await.expect("len"), 1); // expect #[cfg(test)]

        db.delete("doc-1").await.expect("delete"); // expect #[cfg(test)]
        assert_eq!(db.len().await.expect("len"), 0); // expect #[cfg(test)]

        // get should return None after delete
        let doc = db.get("doc-1").await.expect("get"); // expect #[cfg(test)]
        assert!(doc.is_none());
    }
    ...
    async fn test_empty_search() {
        let (db, _tmp) = test_db(4).await;
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 5).await.expect("search"); // expect #[cfg(test)]
    ...
    async fn test_relate_and_scan_prefix() {
        ...
        let related_ids: Vec<String> = results
            .into_iter()
            .map(|(_, v)| v["to"].as_str().expect("valid").to_string()) // expect #[cfg(test)]
            .collect();
        ...
        let backward_results = db
            ...
            .await
            .expect("scan bwd"); // expect #[cfg(test)]
    ...
    async fn test_stats_aggregation() {
        ...
        let stats = db.stats().await.expect("stats"); // expect #[cfg(test)]
    ...
    async fn test_collections_are_isolated() {
        let (db, _tmp) = test_db(4).await;
        let col_a = db.collection("a").await.expect("col a"); // expect #[cfg(test)]
        let col_b = db.collection("b").await.expect("col b"); // expect #[cfg(test)]
    ...
    async fn test_drop_removes_all_data() {
        let (db, _tmp) = test_db(4).await;
        let col = db.collection("drop-me").await.expect("col"); // expect #[cfg(test)]
        col.insert("k1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("ins"); // expect #[cfg(test)]

        db.drop_collection("drop-me").await.expect("drop"); // expect #[cfg(test)]

        let col2 = db.collection("drop-me").await.expect("re-create"); // expect #[cfg(test)]
        assert_eq!(col2.len().await, 0);
        assert!(col2.get("k1").await.expect("get").is_none()); // expect #[cfg(test)]
    }
    ...
    async fn test_list_collections() {
        let (db, _tmp) = test_db(4).await;
        db.collection("c1").await.expect("c1"); // expect #[cfg(test)]
        db.collection("c2").await.expect("c2"); // expect #[cfg(test)]
        db.collection("c3").await.expect("c3"); // expect #[cfg(test)]

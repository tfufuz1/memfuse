...
mod tests {
    ...
    async fn test_insert_search_roundtrip() {
        ...
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 2).await.expect("search"); // expect #[cfg(test)]
    ...
    async fn test_insert_search_returns_metadata() {
        ...
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 1).await.expect("search"); // expect #[cfg(test)]
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].id, "doc-1");
        let meta = results[0].metadata.as_ref().expect("metadata should exist"); // expect #[cfg(test)]
    ...
    async fn test_get_by_key() {
        ...
        let doc = db.get("doc-1").await.expect("get").expect("should exist"); // expect #[cfg(test)]
        assert_eq!(doc.id, "doc-1");
        assert_eq!(doc.metadata.expect("valid")["topic"], "rust"); // expect #[cfg(test)]

        let none = db.get("nonexistent").await.expect("get"); // expect #[cfg(test)]
        assert!(none.is_none());
    }
    ...
    async fn test_update() {
        ...
        assert_eq!(doc.metadata.expect("valid")["v"], 2); // expect #[cfg(test)]

        // Vector should be updated — search for new vector should find it
        let results = db.search(&[0.0, 1.0, 0.0, 0.0], 1).await.expect("search"); // expect #[cfg(test)]
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].id, "doc-1");
    }
    ...
    async fn test_delete() {
        ...
        db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("insert"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(db.len().await.expect("len"), 1); // expect #[cfg(test)] // expect #[cfg(test)]

        db.delete("doc-1").await.expect("delete"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(db.len().await.expect("len"), 0); // expect #[cfg(test)] // expect #[cfg(test)]

        // get should return None after delete
        let doc = db.get("doc-1").await.expect("get"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert!(doc.is_none());
    }
    ...
    async fn test_relate() {
        ...
        db.relate("doc-1", "doc-2", "references")
            .await
            .expect("relate"); // expect #[cfg(test)] // expect #[cfg(test)]
    }
    ...
    async fn test_empty_search() {
        let (db, _tmp) = test_db(4).await;
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 5).await.expect("search"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_relate_and_scan_prefix() {
        ...
        let results = db
            ...
            .await
            .expect("scan"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(results.len(), 2);

        let related_ids: Vec<String> = results
            .into_iter()
            .map(|(_, v)| v["to"].as_str().expect("valid").to_string()) // expect #[cfg(test)] // expect #[cfg(test)]
            .collect();
        ...
        let backward_results = db
            ...
            .await
            .expect("scan bwd"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_stats_aggregation() {
        ...
        let stats = db.stats().await.expect("stats"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_integration_end_to_end() {
        ...
        .await
        .expect("insert agent"); // expect #[cfg(test)]
        db.insert(
            "task-1",
            &[0.9, 0.6, 0.0, 0.0],
            Some(json!({"type": "task"})),
        ...
        .await
        .expect("insert task"); // expect #[cfg(test)]
        db.insert(
            "task-2",
            &[0.0, 0.0, 1.0, 0.5],
            Some(json!({"type": "task"})),
        ...
        .await
        .expect("insert task 2"); // expect #[cfg(test)]

        // 2. Relate
        db.relate("agent-1", "task-1", "assigned_to")
            .await
            .expect("relate 1"); // expect #[cfg(test)]
        db.relate("agent-1", "task-2", "assigned_to")
            .await
            .expect("relate 2"); // expect #[cfg(test)]

        // 3. Search
        let results = db.search(&[1.0, 0.5, 0.0, 0.0], 2).await.expect("search"); // expect #[cfg(test)]
        ...
        .await
        .expect("update task"); // expect #[cfg(test)]
        ...
        // 6. Delete
        db.delete("agent-1").await.expect("delete"); // expect #[cfg(test)]

        // 7. Verify empty search and missing doc
        let get_agent = db.get("agent-1").await.expect("get"); // expect #[cfg(test)]
        assert!(get_agent.is_none());
        assert_eq!(db.len().await.expect("len"), 2); // 3 inserted, 1 deleted // expect #[cfg(test)]
    }

    #[tokio::test]
    async fn test_collections_are_isolated() {
        let (db, _tmp) = test_db(4).await;
        let col_a = db.collection("a").await.expect("col a"); // expect #[cfg(test)] // expect #[cfg(test)]
        let col_b = db.collection("b").await.expect("col b"); // expect #[cfg(test)] // expect #[cfg(test)]
        ...
        let res_a = col_a.get("k1").await.expect("get a").expect("exists"); // expect #[cfg(test)]
        let res_b = col_b.get("k1").await.expect("get b").expect("exists"); // expect #[cfg(test)]

        assert_eq!(res_a.metadata.expect("test")["val"], "a"); // expect #[cfg(test)]
        assert_eq!(res_b.metadata.expect("test")["val"], "b"); // expect #[cfg(test)]

        let search_a = col_a
            .search(&[1.0, 0.0, 0.0, 0.0], 1)
            .await
            .expect("search a"); // expect #[cfg(test)]
        assert_eq!(search_a.len(), 1);
        assert_eq!(search_a[0].id, "k1");
        assert_eq!(search_a[0].metadata.as_ref().expect("test")["val"], "a"); // expect #[cfg(test)]
    }
    ...
    async fn test_close_and_reopen() {
...

        {
            let db = MemFuse::open_with_config(&path, config)
                .await
                .expect("open 2"); // expect #[cfg(test)]
            let doc = db.get("doc-1").await.expect("get").expect("exists"); // expect #[cfg(test)]
            assert_eq!(doc.id, "doc-1");
            assert_eq!(doc.metadata.expect("valid")["v"], 1); // expect #[cfg(test)]
        }
    }
    ...
    async fn test_drop_removes_all_data() {
        let (db, _tmp) = test_db(4).await;
        let col = db.collection("drop-me").await.expect("col"); // expect #[cfg(test)] // expect #[cfg(test)]
        col.insert("k1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("ins"); // expect #[cfg(test)] // expect #[cfg(test)]

        db.drop_collection("drop-me").await.expect("drop"); // expect #[cfg(test)] // expect #[cfg(test)]

        let col2 = db.collection("drop-me").await.expect("re-create"); // expect #[cfg(test)] // expect #[cfg(test)]
        assert_eq!(col2.len().await, 0);
        assert!(col2.get("k1").await.expect("get").is_none()); // expect #[cfg(test)] // expect #[cfg(test)]
    }
    ...
    async fn test_default_collection_compat() {
        ...
        db.insert("k", &[1.0, 0.0, 0.0, 0.0], Some(json!({"v": 1})))
            .await
            .expect("ins"); // expect #[cfg(test)]

        let doc = db.get("k").await.expect("get").expect("exists"); // expect #[cfg(test)]
        assert_eq!(doc.id, "k");
    ...
    async fn test_list_collections() {
        let (db, _tmp) = test_db(4).await;
        db.collection("c1").await.expect("c1"); // expect #[cfg(test)] // expect #[cfg(test)]
        db.collection("c2").await.expect("c2"); // expect #[cfg(test)] // expect #[cfg(test)]
        db.collection("c3").await.expect("c3"); // expect #[cfg(test)] // expect #[cfg(test)]

        let list = db.list_collections().await.expect("list"); // expect #[cfg(test)] // expect #[cfg(test)]
    ...
    async fn test_repair_on_open_resolves_pending_intents() {
        let tmp = tempfile::TempDir::new().expect("temp dir"); // expect #[cfg(test)] // expect #[cfg(test)]
        let path = tmp.path().to_path_buf();
        let config = MemFuseConfig {
            dimension: 4,
            ..Default::default()
        ...
        // 1. Create a doc in LSM but NOT in HNSW to simulate a partial commit
        {
            let db = MemFuse::open_with_config(&path, config.clone())
                .await
                .expect("open 1"); // expect #[cfg(test)]
            let col = db.collection("recovery-test").await.expect("col"); // expect #[cfg(test)]

            // We'll use a direct LSM put to bypass HNSW
            let doc_id = DocId::from_key("recovered-doc").expect("doc_id"); // expect #[cfg(test)]
            let stored = crate::collection::StoredDocument {
                id: "recovered-doc".to_string(),
                embedding: vec![1.0, 0.0, 0.0, 0.0],
                metadata: Some(json!({"status": "recovered"})),
            };
            let data = serde_json::to_vec(&stored).expect("json"); // expect #[cfg(test)]
            ...
            db.storage
                ...
                .await
                .expect("put user"); // expect #[cfg(test)]
            db.storage.put(tx, &doc_key, &data).await.expect("put doc"); // expect #[cfg(test)]
            ...
            db.storage
                ...
                .await
                .expect("put intent"); // expect #[cfg(test)]

            db.storage.commit(tx).await.expect("commit"); // expect #[cfg(test)]

            // Verify it's NOT in HNSW yet (search should fail to find it)
            let results = col.search(&[1.0, 0.0, 0.0, 0.0], 1).await.expect("search"); // expect #[cfg(test)]
        ...
        // 2. Re-open: repair_on_open should trigger and re-sync
        {
            let db = MemFuse::open_with_config(&path, config)
                .await
                .expect("open 2 (repair)"); // expect #[cfg(test)]
            let col = db.collection("recovery-test").await.expect("col"); // expect #[cfg(test)]
            ...
            let entries = db
                ...
                .await
                .expect("scan intents"); // expect #[cfg(test)]

...
mod tests {
    ...
    async fn test_insert_search_roundtrip() {
        ...
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 2).await.expect("search"); // unwrap allowed (AGENT:08)
    ...
    async fn test_insert_search_returns_metadata() {
        ...
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 1).await.expect("search"); // unwrap allowed (AGENT:08)
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].id, "doc-1");
        let meta = results[0].metadata.as_ref().expect("metadata should exist"); // unwrap allowed (AGENT:08)
    ...
    async fn test_get_by_key() {
        ...
        let doc = db.get("doc-1").await.expect("get").expect("should exist"); // unwrap allowed (AGENT:08)
        assert_eq!(doc.id, "doc-1");
        assert_eq!(doc.metadata.expect("valid")["topic"], "rust"); // unwrap allowed (AGENT:08)

        let none = db.get("nonexistent").await.expect("get"); // unwrap allowed (AGENT:08)
        assert!(none.is_none());
    }
    ...
    async fn test_update() {
        ...
        assert_eq!(doc.metadata.expect("valid")["v"], 2); // unwrap allowed (AGENT:08)

        // Vector should be updated — search for new vector should find it
        let results = db.search(&[0.0, 1.0, 0.0, 0.0], 1).await.expect("search"); // unwrap allowed (AGENT:08)
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].id, "doc-1");
    }
    ...
    async fn test_delete() {
        ...
        db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("insert"); // unwrap allowed (AGENT:08)
        assert_eq!(db.len().await.expect("len"), 1); // unwrap allowed (AGENT:08)

        db.delete("doc-1").await.expect("delete"); // unwrap allowed (AGENT:08)
        assert_eq!(db.len().await.expect("len"), 0); // unwrap allowed (AGENT:08)

        // get should return None after delete
        let doc = db.get("doc-1").await.expect("get"); // unwrap allowed (AGENT:08)
        assert!(doc.is_none());
    }
    ...
    async fn test_relate() {
        ...
        db.relate("doc-1", "doc-2", "references")
            .await
            .expect("relate"); // unwrap allowed (AGENT:08)
    }
    ...
    async fn test_empty_search() {
        let (db, _tmp) = test_db(4).await;
        let results = db.search(&[1.0, 0.0, 0.0, 0.0], 5).await.expect("search"); // unwrap allowed (AGENT:08)
    ...
    async fn test_relate_and_scan_prefix() {
        ...
        let related_ids: Vec<String> = results
            .into_iter()
            .map(|(_, v)| v["to"].as_str().expect("valid").to_string()) // unwrap allowed (AGENT:08)
            .collect();
        ...
        let backward_results = db
            ...
            .await
            .expect("scan bwd"); // unwrap allowed (AGENT:08)
    ...
    async fn test_stats_aggregation() {
        ...
        let stats = db.stats().await.expect("stats"); // unwrap allowed (AGENT:08)
    ...
    async fn test_integration_end_to_end() {
        ...
        .await
        .expect("insert agent"); // unwrap allowed (AGENT:08)
        db.insert(
            "task-1",
            &[0.9, 0.6, 0.0, 0.0],
            Some(json!({"type": "task"})),
        ...
        .await
        .expect("insert task"); // unwrap allowed (AGENT:08)
        db.insert(
            "task-2",
            &[0.0, 0.0, 1.0, 0.5],
            Some(json!({"type": "task"})),
        ...
        .await
        .expect("insert task 2"); // unwrap allowed (AGENT:08)

        // 2. Relate
        db.relate("agent-1", "task-1", "assigned_to")
            .await
            .expect("relate 1"); // unwrap allowed (AGENT:08)
        db.relate("agent-1", "task-2", "assigned_to")
            .await
            .expect("relate 2"); // unwrap allowed (AGENT:08)

        // 3. Search
        let results = db.search(&[1.0, 0.5, 0.0, 0.0], 2).await.expect("search"); // unwrap allowed (AGENT:08)
        ...
        .await
        .expect("update task"); // unwrap allowed (AGENT:08)
        ...
        // 6. Delete
        db.delete("agent-1").await.expect("delete"); // unwrap allowed (AGENT:08)

        // 7. Verify empty search and missing doc
        let get_agent = db.get("agent-1").await.expect("get"); // unwrap allowed (AGENT:08)
        assert!(get_agent.is_none());
        assert_eq!(db.len().await.expect("len"), 2); // 3 inserted, 1 deleted // unwrap allowed (AGENT:08)
    }

    #[tokio::test]
    async fn test_collections_are_isolated() {
        let (db, _tmp) = test_db(4).await;
        let col_a = db.collection("a").await.expect("col a"); // unwrap allowed (AGENT:08)
        let col_b = db.collection("b").await.expect("col b"); // unwrap allowed (AGENT:08)
        ...
        let res_a = col_a.get("k1").await.expect("get a").expect("exists"); // unwrap allowed (AGENT:08)
        let res_b = col_b.get("k1").await.expect("get b").expect("exists"); // unwrap allowed (AGENT:08)

        assert_eq!(res_a.metadata.expect("test")["val"], "a"); // unwrap allowed (AGENT:08)
        assert_eq!(res_b.metadata.expect("test")["val"], "b"); // unwrap allowed (AGENT:08)

        let search_a = col_a
            .search(&[1.0, 0.0, 0.0, 0.0], 1)
            .await
            .expect("search a"); // unwrap allowed (AGENT:08)
        assert_eq!(search_a.len(), 1);
        assert_eq!(search_a[0].id, "k1");
        assert_eq!(search_a[0].metadata.as_ref().expect("test")["val"], "a"); // unwrap allowed (AGENT:08)
    }
    ...
    async fn test_close_and_reopen() {
...

        {
            let db = MemFuse::open_with_config(&path, config.clone())
                .await
                .expect("open 1"); // unwrap allowed (AGENT:08)
            db.insert("doc-1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"v": 1})))
                .await
                .expect("insert"); // unwrap allowed (AGENT:08)
            db.close().await.expect("close"); // unwrap allowed (AGENT:08)
        }

        {
            let db = MemFuse::open_with_config(&path, config)
                .await
                .expect("open 2"); // unwrap allowed (AGENT:08)
            let doc = db.get("doc-1").await.expect("get").expect("exists"); // unwrap allowed (AGENT:08)
            assert_eq!(doc.id, "doc-1");
            assert_eq!(doc.metadata.expect("valid")["v"], 1); // unwrap allowed (AGENT:08)
        }
    }
    ...
    async fn test_drop_removes_all_data() {
        let (db, _tmp) = test_db(4).await;
        let col = db.collection("drop-me").await.expect("col"); // unwrap allowed (AGENT:08)
        col.insert("k1", &[1.0, 0.0, 0.0, 0.0], None)
            .await
            .expect("ins"); // unwrap allowed (AGENT:08)

        db.drop_collection("drop-me").await.expect("drop"); // unwrap allowed (AGENT:08)

        let col2 = db.collection("drop-me").await.expect("re-create"); // unwrap allowed (AGENT:08)
        assert_eq!(col2.len().await, 0);
        assert!(col2.get("k1").await.expect("get").is_none()); // unwrap allowed (AGENT:08)
    }
    ...
    async fn test_default_collection_compat() {
        ...
        let doc = db.get("k").await.expect("get").expect("exists"); // unwrap allowed (AGENT:08)
        assert_eq!(doc.id, "k");
    ...
    async fn test_list_collections() {
        let (db, _tmp) = test_db(4).await;
        db.collection("c1").await.expect("c1"); // unwrap allowed (AGENT:08)
        db.collection("c2").await.expect("c2"); // unwrap allowed (AGENT:08)
        db.collection("c3").await.expect("c3"); // unwrap allowed (AGENT:08)
    ...
    async fn test_repair_on_open_resolves_pending_intents() {
        let tmp = tempfile::TempDir::new().expect("temp dir"); // unwrap allowed (AGENT:08)
        let path = tmp.path().to_path_buf();
        let config = MemFuseConfig {
            dimension: 4,
            ..Default::default()
        ...
        // 1. Create a doc in LSM but NOT in HNSW to simulate a partial commit
        {
            let db = MemFuse::open_with_config(&path, config.clone())
                .await
                .expect("open 1"); // unwrap allowed (AGENT:08)
            let col = db.collection("recovery-test").await.expect("col"); // unwrap allowed (AGENT:08)

            // We'll use a direct LSM put to bypass HNSW
            let doc_id = DocId::from_key("recovered-doc").expect("doc_id"); // unwrap allowed (AGENT:08)
            let stored = crate::collection::StoredDocument {
                id: "recovered-doc".to_string(),
                embedding: vec![1.0, 0.0, 0.0, 0.0],
                metadata: Some(json!({"status": "recovered"})),
            };
            let data = serde_json::to_vec(&stored).expect("json"); // unwrap allowed (AGENT:08)
            ...
            db.storage
                ...
                .await
                .expect("put user"); // unwrap allowed (AGENT:08)
            db.storage.put(tx, &doc_key, &data).await.expect("put doc"); // unwrap allowed (AGENT:08)
            ...
            db.storage
                ...
                .await
                .expect("put intent"); // unwrap allowed (AGENT:08)

            db.storage.commit(tx).await.expect("commit"); // unwrap allowed (AGENT:08)

            // Verify it's NOT in HNSW yet (search should fail to find it)
            let results = col.search(&[1.0, 0.0, 0.0, 0.0], 1).await.expect("search"); // unwrap allowed (AGENT:08)
        ...
        // 2. Re-open: repair_on_open should trigger and re-sync
        {
            let db = MemFuse::open_with_config(&path, config)
                .await
                .expect("open 2 (repair)"); // unwrap allowed (AGENT:08)
            let col = db.collection("recovery-test").await.expect("col"); // unwrap allowed (AGENT:08)
            ...
            let entries = db
                ...
                .await
                .expect("scan intents"); // unwrap allowed (AGENT:08)

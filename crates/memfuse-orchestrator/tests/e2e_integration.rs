...
async fn test_e2e_agent_workflow() {
    ...
    db.insert("doc-1", &[1.0, 0.0, 0.0], Some(json!({"text": "Rust is a systems programming language."}))).await.expect("insert failed");
    db.insert("doc-2", &[0.0, 1.0, 0.0], Some(json!({"text": "Python is great for data science."}))).await.expect("insert failed");

    // 3. Hybrid Search (Vector + Text)
    let results = db.hybrid_search("Rust", &[0.9, 0.1, 0.0], 2).await.expect("hybrid search failed");

    // 4. Verify Ergebnisse (Score, Metadata, Ordering)
    assert!(!results.is_empty());
    assert_eq!(results[0].id, "doc-1");
    assert!(results[0].metadata.as_ref().unwrap()["text"].as_str().unwrap().contains("Rust"));

    // 5. Update + Re-Search
    db.update("doc-1", &[1.0, 0.0, 0.0], Some(json!({"text": "Rust is super fast."}))).await.expect("update failed");
    let results_updated = db.search(&[1.0, 0.0, 0.0], 1).await.expect("search failed");
    assert_eq!(results_updated[0].metadata.as_ref().unwrap()["text"], "Rust is super fast.");
    ...
    col_a.insert("secret", &[0.1, 0.2, 0.3], Some(json!({"val": "A"}))).await.expect("ins a");
    col_b.insert("secret", &[0.1, 0.2, 0.3], Some(json!({"val": "B"}))).await.expect("ins b");
    ...
    let _execution_result = sandbox.execute(b"WASM_CODE", "input").expect("WASM execution failed");
...
async fn test_stress_concurrent_agent_ops() {
    ...
    for t in 0..num_tasks {
        let col_name = format!("stress-{}", t);
        let col = db.collection(&col_name).await.expect("collection");
        assert_eq!(col.len().await, 0, "Collection {} should be empty", col_name);
    }
}

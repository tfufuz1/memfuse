...
async fn test_e2e_agent_workflow() {
    ...
    assert!(results[0].metadata.as_ref().expect("test")["text"]
        .as_str()
        .expect("test")
        .contains("Rust"));
    ...
    assert_eq!(
        results_updated[0].metadata.as_ref().expect("test")["text"],
        "Rust is super fast."
    );
    ...
    let val_a = col_a.get("secret").await.expect("get a").expect("test");
    let val_b = col_b.get("secret").await.expect("get b").expect("test");

    assert_eq!(val_a.metadata.expect("test")["val"], "A");
    assert_eq!(val_b.metadata.expect("test")["val"], "B");

...
| `scripts/context-cli.sh blockers` | Show critical issues | Periodically during session |
| `cargo xtask audit-verify <id>` | Validate external findings | Before implementing audit fix |
| `cargo xtask audit-review <id>` | Log review completion | After audit fix validation |
...
- `TS:` with current ISO-8601 UTC timestamp

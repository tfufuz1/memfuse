    ...
    match subcommand {
        ...
        "check-stale-tags" => {
            let threshold_days = args
                .iter()
                .find_map(|arg| arg.strip_prefix("--days="))
...
mod tests {
    ...
    #[test]
    fn test_generate_session_continuity_section_passthrough() {
        let _guard = TEST_DIR_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let temp_dir = tempfile::tempdir().unwrap();
        let root = temp_dir.path();

        let existing_block = "<!-- AUTOGENERATED:START:SESSION_CONTINUITY -->\n## Session-Kontinuität\n- Existing content\n<!-- AUTOGENERATED:END:SESSION_CONTINUITY -->";
        let working_state_content =
            format!("# Working State\n\n{}\n\nSome other text", existing_block);
        fs::write(root.join("WORKING_STATE.md"), working_state_content).unwrap();

        let orig_dir = std::env::current_dir().unwrap();
        std::env::set_current_dir(root).unwrap();

        let res = std::panic::catch_unwind(|| generate_session_continuity_section(&[]));

        let _ = std::env::set_current_dir(orig_dir);

        let section =
            res.expect("generate_session_continuity_section panicked in passthrough test");
        assert_eq!(section, existing_block);
    }

...
// AI-TAG[DEBT][MINOR] TODO: Nach Merge von check-duplicate-intent konsolidieren (ID: AGT-XTASK-session-continuity) (TS: 2026-09-08T00:00:00Z) (SESSION: jules)
fn generate_session_continuity_section(tags: &[TagItem]) -> String {
    if let Ok(content) = fs::read_to_string("WORKING_STATE.md") {
        let start_marker = "<!-- AUTOGENERATED:START:SESSION_CONTINUITY -->";
        let end_marker = "<!-- AUTOGENERATED:END:SESSION_CONTINUITY -->";
        if let (Some(s), Some(e)) = (content.find(start_marker), content.find(end_marker)) {
            if s < e {
                return content[s..e + end_marker.len()].to_string();
            }
        }
    }
    ...
    let last_commit_opt = std::process::Command::new("git")
        .args(["log", "-1", "origin/main", "--format=%h%x09%s"])
        .output()
        .ok()
    ...
    if let Some(ref last_commit) = last_commit_opt {
        ...
        let (hash, subject, date) = if parts.len() >= 3 {
            ...
            (parts[0].trim(), parts[1].trim(), "unbekannt")
        } else {
            ("0000000", last_commit.as_str(), "unbekannt")
        };
        out.push_str(&format!(
            "- Letzter Merge: {} — \"{}\" ({})\n",
            hash, subject, date
        ...
        out.push_str("- Letzter Merge: unbekannt\n");
    }
    ...
    if git_available {
        ...
        if let Some(files) = diff_files_opt {
            if files.is_empty() {
        ...
        out.push_str("- Änderungsbereich: unbekannt\n");
    }

    // Ähnlichkeits-Cluster
    if git_available {
        out.push_str("- Ähnlichkeits-Cluster: nicht geprüft — cargo xtask check-duplicate-intent ausführen\n");
    } else {
        out.push_str("- Ähnlichkeits-Cluster: unbekannt\n");
    }
...
pub fn run_check_review_coverage(tags: &[TagItem]) -> bool {
    ...
    for anchor in &done_anchors {
        ...
        let is_security = anchor.raw.contains("SECURITY")
            || anchor.raw.contains("ANCHOR[SECURITY")
...
mod tests {
    ...
    fn test_generate_session_continuity_section_format() {
        ...
        assert!(section.contains("## Session-Kontinuität (autogeneriert aus letztem Merge)"));
        assert!(section.contains("- Letzter Merge:"));
        assert!(section.contains("- Änderungsbereich:"));
        assert!(section.contains("- Ähnlichkeits-Cluster:"));
...
"#,
        ...
        assert!(full_ws.contains("## Session-Kontinuität (autogeneriert aus letztem Merge)"));
    ...
    fn test_check_review_coverage_fixtures() {
        ...
        // Fixture 4: Anchor in unsafe file with only 2 passes (should fail)
        let mut tags_unsafe_2_passes = vec![TagItem {
            file_path: "crates/memfuse-index/src/distance.rs".to_string(),
            line_num: 10,
            tag_type: "ANCHOR".to_string(),
            raw: "// ANCHOR[ALG-FIX:SIMD-001] STATUS:DONE (ID: AGT-INDEX-11111111) (TS:2026-08-29T09:14:07Z) (SESSION:a3f29c1d)".to_string(),
            timestamp: "2026-08-29T09:14:07Z".to_string(),
            category: None,
            severity: None,
            id: Some("AGT-INDEX-11111111".to_string()),
            session: Some("a3f29c1d".to_string()),
            status: Some("DONE".to_string()),
        ...
        tags_unsafe_2_passes.push(TagItem {
            ...
            line_num: 15,
            tag_type: "REVIEW-PASS".to_string(),
            raw: "// REVIEW-PASS[1/3] STATUS:PASS (ID: AGT-INDEX-11111111) (TS:2026-08-29T10:00:00Z) (SESSION:b8e4f1a2)".to_string(),
            timestamp: "2026-08-29T10:00:00Z".to_string(),
            category: None,
            severity: None,
            id: Some("AGT-INDEX-11111111".to_string()),
        ...
        tags_unsafe_2_passes.push(TagItem {
            ...
            line_num: 16,
            tag_type: "REVIEW-PASS".to_string(),
            raw: "// REVIEW-PASS[2/3] STATUS:PASS (ID: AGT-INDEX-11111111) (TS:2026-08-29T11:00:00Z) (SESSION:c9f5e2b3)".to_string(),
            timestamp: "2026-08-29T11:00:00Z".to_string(),
            category: None,
            severity: None,
            id: Some("AGT-INDEX-11111111".to_string()),
        ...
        // Fixture 5: Anchor in unsafe file with 3 passes (should pass)
        let mut tags_unsafe_3_passes = tags_unsafe_2_passes.clone();
        tags_unsafe_3_passes.push(TagItem {
            file_path: "crates/memfuse-index/src/distance.rs".to_string(),
            line_num: 17,
            tag_type: "REVIEW-PASS".to_string(),
            raw: "// REVIEW-PASS[3/3] STATUS:PASS (ID: AGT-INDEX-11111111) (TS:2026-08-29T12:00:00Z) (SESSION:d0a6f3c4)".to_string(),

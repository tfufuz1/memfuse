...
fn generate_session_continuity_section() -> String {
    ...
    let (last_hash, last_subject) = match last_commit.split_once('\t') {
        ...
        None => ("0000000", last_commit.as_str()),
    };
    ...
    let history_commits = std::process::Command::new("git")
        .args(["log", "--since=30 days ago", "--format=%h%x09%s"])
        .output()
        .ok()
    ...
    let mut best_match_hash = String::new();
    let mut best_match_scope = String::new();

    let last_tokens = tokenize_title(last_subject);

    for line in history_commits.lines() {
        ...
        let tokens = tokenize_title(subject);
        let score = jaccard_similarity(&last_tokens, &tokens);
        if score > highest_score {
            highest_score = score;
            best_match_hash = hash.to_string();
            best_match_scope = extract_scope(subject);
        }
    }

    if highest_score >= 50 {
        let scope_display = if best_match_scope.is_empty() {
            extract_scope(last_subject)
        } else {
            best_match_scope
        };
        let scope_str = if scope_display.is_empty() {
            "allgemein"
        } else {
            &scope_display
        };
        out.push_str(&format!(
            "- Ähnlichkeits-Cluster: Dieser Commit ähnelt {} zu {}% (Scope: {}). GRUND FÜR WIEDERHOLUNG: ⚠️ UNGEKLÄRT — manuell prüfen.\n",
            best_match_hash, highest_score, scope_str
        ));
    } else {
        out.push_str("- Ähnlichkeits-Cluster: Keine signifikante Ähnlichkeit zu Commits der letzten 30 Tage gefunden.\n");
    }
...
fn generate_full_working_state(tags: &[TagItem], crates: &[CrateInfo]) -> String {
    ...
    out.push_str(&generate_session_continuity_section());
    out.push_str("\n");

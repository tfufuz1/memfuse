...
1. **`ScalarQuantizer` Sichtbarkeit & API-Re-Export:**
   ...
   - **Verifikation:** `cargo test -p memfuse-index --test recalibration` kompiliert und besteht ohne Fehler.
...
2. **Compiler Dead-Code-Warnungen in `distance.rs`:**
   ...
   - **Verifikation:** `cargo check -p memfuse-index --all-features` liefert 0 Fehler und 0 Warnungen.

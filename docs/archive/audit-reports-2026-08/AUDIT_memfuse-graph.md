...
## 12. Deep Audit & Non-Deterministic Allocation Fix (2026-09-01)

**Auditor:** Senior Rust Graph-Algorithmen-Ingenieur — CSR, PPR, BFS, Session-DAG
**Date:** September 1, 2026
**Status:** PASSED (Verified Deterministic Entity Allocation, Deadlock Safety & Zero Test Flakiness)

### Audit Findings & Resolution:
1. **Flakiness in `test_csr_graph_compact_layout` Identified:**
   - During multi-threaded concurrency stress testing, `test_csr_graph_compact_layout` exhibited intermittent failures (`offsets[1]` assertion mismatch).
   - **Root Cause:** `staged_entities` in `CsrGraph` was stored as `HashMap<EntityId, Entity>`. During `commit()`, iterating over `tx_entities` had non-deterministic HashMap order. When Entity 2 was assigned internal index 0 before Entity 1, CSR offsets array layout shifted from `[0, 1, 1]` to `[0, 0, 1]`, triggering `assert_eq!(inner.offsets[1], 1)` failures.
   - **Resolution:** Updated `commit()` in `crates/memfuse-graph/src/csr.rs` to sort `tx_entities` by `EntityId` and `tx_edges` by `from_id` prior to index assignment. Internal index allocation and CSR offsets are now 100% deterministic across all runs regardless of SipHash seed.
2. **Deterministic Layout Verification:**
   - Executed 10 consecutive runs of `cargo test -p memfuse-graph --lib`. 100% pass rate achieved with 0 failures across all 77 unit & property tests.
3. **Workspace Integrity & Gate Stack:**
   - All gate stack checks (`check`, `clippy`, `fmt`, `test`, workspace `check`) verified clean.

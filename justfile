...
dag-check:
    ...
    if cargo tree -p memfuse-index --edges no-dev | grep -E -v "memfuse-index|memfuse-core|memfuse-graph" | grep -q "memfuse-"; then
        echo "❌ ERROR: memfuse-index violates DAG by importing non-core/non-graph crates."
    ...
    for CRATE in memfuse-py memfuse-saos-agent; do
        echo "Verifying $CRATE isolation..."
        if [ "$CRATE" == "memfuse-py" ]; then OTHER="memfuse-saos-agent"; else OTHER="memfuse-py"; fi
        if cargo tree -p "$CRATE" --edges no-dev | grep -q "$OTHER"; then
            echo "❌ ERROR: $CRATE violates isolation by importing $OTHER."
            cargo tree -p "$CRATE" --edges no-dev | grep "$OTHER"
            exit 1
        fi
    done

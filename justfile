...
dag-check:
    ...
    for CRATE in memfuse-core memfuse-sandbox memfuse-saos-agent; do
        ...
        # Justified: saos-agent -> db, graph, checkpoint, store (Orchestrator Role)
        if [ "$CRATE" = "memfuse-saos-agent" ]; then
            if cargo tree -p "$CRATE" --edges no-dev | grep "memfuse-" | grep -E -v "$CRATE|memfuse-core|memfuse-db|memfuse-graph|memfuse-checkpoint|memfuse-store" | grep -q .; then
    ...
    # Justified: index -> graph (WP-2.1 CSR)
